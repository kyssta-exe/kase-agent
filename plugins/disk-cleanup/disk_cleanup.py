"""disk_cleanup — ephemeral file cleanup for Kase.
...
Rules:
  ...
  - empty dirs    → always delete (under KASE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly KASE_HOME and /tmp/kase-*
Never touches: ~/.kase/logs/ or any system directory.
"""
...
try:
    ...
    def get_kase_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("KASE_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".kase").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$KASE_HOME/logs/``."""
    return get_kase_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$KASE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under KASE_HOME or ``/tmp/kase-*``.
    ...
    try:
        ...
        pass
    # Allow /tmp/kase-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("kase-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside KASE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under KASE_HOME (but leave KASE_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "kase", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under KASE_HOME (e.g. /tmp/kase-*) — fall through.
        pass
