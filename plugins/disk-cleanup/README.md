...
Auto-tracks and cleans up ephemeral files created during Kase
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$KASE_HOME` and `/tmp/kase-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `KASE_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `KASE_HOME` or `/tmp/kase-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$KASE_HOME/disk-cleanup/` is itself excluded
- `$KASE_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
