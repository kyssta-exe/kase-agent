# Kase Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: kase-achievements plugin
...
### `GET /api/plugins/kase-achievements/achievements`
...
### `POST /api/plugins/kase-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/kase-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/kase-achievements/overview`
...
- Persist plugin data under:
  - `~/.kase/plugins/kase-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.kase/plugins/kase-achievements/scan_checkpoint.json`
...
- `~/.kase/plugins/kase-achievements/`
