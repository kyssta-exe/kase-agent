...
def _is_tool_gateway_ready() -> bool:
    ...
    Reads ``read_kyssta_access_token`` and ``resolve_managed_tool_gateway``
    via :mod:`tools.web_tools` rather than direct imports, so unit tests
    that ``patch("tools.web_tools._read_kyssta_access_token", ...)`` see
    ...
    return _wt.resolve_managed_tool_gateway(
        "firecrawl", token_reader=_wt._read_kyssta_access_token
    ) is not None
...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``kase tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_kyssta_tools_enabled():
        return ""
    return (
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_kyssta_tools_enabled():
        message += (
            " With your Nous subscription you can also use the Tool Gateway — "
            "run `kase tools` and select Nous Subscription as the web provider."
        )
...
def _get_firecrawl_client() -> Any:
    ...
    ``_read_kyssta_access_token``, ``Firecrawl``) are also looked up via
