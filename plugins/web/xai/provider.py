...
``{title, url, description, position}`` rows every other Kase web
provider produces.
...
prefers Kase-managed xAI Grok OAuth (via ``kase auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.kase/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``kase tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `kase auth` to sign in with "
