...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with kase)
2. User-installed providers: ``$KASE_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$KASE_HOME/plugins/`` or None if unavailable."""
    try:
        from kase_constants import get_kase_home
        d = get_kase_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$KASE_HOME/plugins/<name>/``) directories.  Bundled takes
