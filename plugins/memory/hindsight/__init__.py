...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: kase)
...
Or via $KASE_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Kase can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $KASE_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_kase_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "kase": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "kase"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Kase config."""
    profile = config.get("profile", "kase")
    return str(profile or "kase")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Kase profile name (from agent_identity)
      {workspace} — Kase workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``kase-{user}`` with no user becomes ``kase``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Kase and the User"
    ...
    def save_config(self, values, hermes_home):
        """Write config to $KASE_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        from kase_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "kase")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "kase"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: kase-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Kase and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "kase"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "kase"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "kase", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "kase")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Kase and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Kase created/used that client on the shared async loop,
