...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/kase>
...
```bash
kase honcho setup    # full interactive wizard (cloud or local)
kase memory setup    # generic picker, also works
```

Or manually:
```bash
kase config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$KASE_HOME/honcho.json` | Profile-local (isolated Kase instances) |
| 2 | `~/.kase/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Kase profile: `kase` (default) or `kase.<profile>`.
...
| 4 | `per-session` strategy | Kase session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`kase`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`kase`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-kase`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening kase in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Kase session ID (timestamp + hex). Every `kase` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Kase profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`kase`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Kase profile: `kase` (default) or `kase.<profile>` (e.g. `kase -p coder` → host key `kase.coder`).
...
| Command | Description |
|---------|-------------|
| `kase honcho setup` | Full interactive setup wizard |
| `kase honcho status` | Show resolved config for active profile |
| `kase honcho enable` / `disable` | Toggle Honcho for active profile |
| `kase honcho mode <mode>` | Change recall or observation mode |
| `kase honcho peer --user <name>` | Update user peer name |
| `kase honcho peer --ai <name>` | Update AI peer name |
| `kase honcho tokens --context <N>` | Set context token budget |
| `kase honcho tokens --dialectic <N>` | Set dialectic max chars |
| `kase honcho map <name>` | Map current directory to a session name |
| `kase honcho sync` | Create host blocks for all Kase profiles |
