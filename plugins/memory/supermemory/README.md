...
Config file: `$KASE_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `kase` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `kase-{identity}` → `kase-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Kase profile:

```json
{
  "container_tag": "kase-{identity}"
}
```

For a profile named `coder`, this resolves to `kase-coder`. The default profile resolves to `kase-default`. Without `{identity}`, all profiles share the same container.
