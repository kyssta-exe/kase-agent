"""`kase meet node ...` subcommand tree.

Wired into the existing ``kase meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``kase meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="kase-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             kase meet node approve <name> ws://<host>:{args.port} {token}")
        try:
