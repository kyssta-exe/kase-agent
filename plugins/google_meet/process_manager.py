...
session-scoped state file under ``$KASE_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $KASE_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_kase_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this kase install, leave it first —
    ...
    env = os.environ.copy()
    env["KASE_MEET_URL"] = url
    env["KASE_MEET_OUT_DIR"] = str(out)
    env["KASE_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["KASE_MEET_HEADED"] = "1"
    if auth_state:
        env["KASE_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["KASE_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if KASE_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["KASE_MEET_MODE"] = mode
    if realtime_model:
        env["KASE_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["KASE_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["KASE_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["KASE_MEET_REALTIME_KEY"] = realtime_api_key
