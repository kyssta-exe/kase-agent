...
Let the kase join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where kase runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `kase meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `kase meet` CLI |
...
| `cli.py` | `kase meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$KASE_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `kase meet node {run,list,approve,remove,status,ping}` |
...
kase meet install                                      # pip + Chromium
kase meet setup                                        # preflight
kase meet auth                                         # optional
kase meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
kase meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.kase/.env
kase meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
kase meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
kase meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.kase/.env
kase meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, kase will **not** switch your system audio input automatically — the
...
kase plugins enable google_meet
kase meet node run --display-name my-mac --host 0.0.0.0 --port 18789
