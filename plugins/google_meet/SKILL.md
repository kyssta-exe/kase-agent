...
kase meet install                 # pip deps + Chromium (transcribe only)
kase meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
kase meet auth                    # optional; skips guest-lobby wait
kase meet setup                   # preflight checks
```

`kase meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or KASE_MEET_REALTIME_KEY in ~/.kase/.env
```
...
pip install playwright websockets && python -m playwright install chromium
kase plugins enable google_meet
kase meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
kase meet node approve my-mac ws://<mac-ip>:18789 <token>
kase meet node ping my-mac                   # confirm reachable
```

Run `kase meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Kase agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `kase meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `KASE_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `kase_meet_src`). |
...
Local:
```
$KASE_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$KASE_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `kase meet node approve`.
