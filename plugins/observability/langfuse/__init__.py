"""langfuse — Kase plugin for Langfuse observability.

Traces Kase conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Kase plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``kase plugins enable
observability/langfuse``, or by checking the box in the interactive
``kase plugins`` UI). At runtime the plugin also requires the
``langfuse`` SDK and credentials; if either is missing the hooks are inert.

Required env vars (set in ~/.kase/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Kase plugin system —
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Kase branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Kase also has a turn-scoped
    ...
    # messages is a list only for legacy Kase branches that fired
    # pre_llm_call with API messages directly. Current Kase fires
...
def register(ctx) -> None:
    ...
    # Kase versions.  pre_api_request / post_api_request fire per API
