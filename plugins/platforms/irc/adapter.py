"""
IRC Platform Adapter for Kase.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Kase agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "kase-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Kase")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Kase shutting down")
...
def check_requirements() -> bool:
    ...
    # kase setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `kase gateway setup` flow for the IRC platform.

    Lazy-imports ``kase_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Kase to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #kase — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.kase/.env")
    print_info("Restart the gateway for changes to take effect: kase gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "kase-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "kase-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Kase (cron)")
