"""SimpleX Chat platform adapter (Kase plugin).
...
This adapter ships as a Kase platform plugin under
``plugins/platforms/simplex/``. The Kase plugin loader scans the
...
discoverable and `kase setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Kase — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"kase-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``kase setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.kase/.env`` via ``kase_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("kase_cli.config not available; set SIMPLEX_* vars manually in ~/.kase/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `kase gateway status` without
