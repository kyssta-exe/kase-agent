"""
LINE Messaging API platform adapter for Kase.
...
to Kase. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``KASE_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            hermes_home = Path.home().joinpath(".kase").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``kase status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``kase status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``kase setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.kase/.env`` via ``kase_cli.config``.
    """
    print()
    ...
    try:
        from kase_cli.config import get_env_var, set_env_var
    except ImportError:
        print("kase_cli.config not available; set LINE_* vars manually in ~/.kase/.env")
        return
