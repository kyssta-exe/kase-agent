...
# ``model_tools`` import time, so ``kase status``, ``kase chat``, etc.
...
# ``kase_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Kase is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a kase MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${KASE_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _kase_home = _get_kase_home()
        except (ModuleNotFoundError, ImportError):
            _kase_home = _Path.home() / ".kase"
        self._thread_count_store = _ThreadCountStore(
            _kase_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Kase is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("KASE_HOME", str(_Path.home() / ".kase"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${KASE_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a kase MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Kase is thinking…' marker message.
        ...
        "Kase is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Kase is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Kase is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Kase is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Kase is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``kase setup``.

    The setup wizard at ``kase_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.kase/.env`` so the next
    gateway restart picks them up.
    """
    from kase_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. kase-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.kase/.env")
    print_info("Restart the gateway: kase gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``kase cron`` running as a
    separate process from ``kase gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Kase plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'kase[google_chat]'",
        ...
        # the "Kase is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Kase is thinking…' marker message "
