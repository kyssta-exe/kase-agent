...
Usage (via cron with --no-agent):
    ...
    kase cron create kase-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$KASE_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name kase-issues --repo Kyssta/kase --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.kase/.env to avoid the 60 req/hr
