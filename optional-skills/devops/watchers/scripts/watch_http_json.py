...
Usage (via cron with --no-agent):
    ...
    kase cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$KASE_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Kase-Watcher/1.0"})
