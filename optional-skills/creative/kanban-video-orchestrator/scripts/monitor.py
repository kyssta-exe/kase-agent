#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `kase kanban list` and
...
def hermes_available() -> bool:
    return shutil.which("kase") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed kase CLI."""
    try:
        out = subprocess.run(
            ["kase", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `kase kanban list`
    out = subprocess.run(
        ["kase", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not hermes_available():
        print("ERROR: 'kase' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
