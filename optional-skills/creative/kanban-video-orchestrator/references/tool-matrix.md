...
Maps each role archetype to the Kase skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public kase
...
## Kase skills relevant to video production

### Visual / rendering skills (`kase/skills/creative/`)
...
### Design / pre-production skills (`kase/skills/creative/`)
...
### Audio / media skills (`kase/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`kase/skills/devops/`)
...
These are **not** Kase skills but external CLIs / APIs that profiles invoke.
...
## Built-in Kase tools for media review

These are native Kase tools — not invoked via terminal but through their own
...
key is present in `~/.kase/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Kase profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.kase/.env` → environment variable.
...
When a new Kase-public video skill ships:
