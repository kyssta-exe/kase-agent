...
kase kanban watch --tenant <project-slug>

# Snapshot of the board
kase kanban list --tenant <project-slug>
kase kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
kase kanban stats --tenant <project-slug>

# Visual dashboard (browser)
kase dashboard

# Inspect a specific task (includes comments + events)
kase kanban show <task-id>

# Follow a single task's event stream
kase kanban tail <task-id>
```

Verify available subcommands with `kase kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `kase kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `kase kanban block <id>` then `kase kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `kase kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `kase kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `kase profile list` (profile exists?), `kase status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(kase kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
kase kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
kase kanban block <task-id>
kase kanban archive <task-id>

# Diagnose what happened
kase kanban show <task-id>      # task body, comments, recent events
kase kanban tail <task-id>      # follow the live event stream
kase kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `kase kanban create` for the director
...
while true; do
    clear
    kase kanban list --tenant <slug>
    echo "---"
    kase kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `kase kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `kase profile list` and start
