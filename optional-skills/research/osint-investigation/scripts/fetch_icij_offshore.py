...
$HERMES_OSINT_CACHE/icij/ (default: ~/.cache/kase-osint/icij/) and search
...
def _cache_dir() -> Path:
    ...
    return Path.home() / ".cache" / "kase-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "kase osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $HERMES_OSINT_CACHE/icij or ~/.cache/kase-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
