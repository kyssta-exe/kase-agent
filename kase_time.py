...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.kase/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Kase never
...
from kase_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Kase requires 3.9+)
