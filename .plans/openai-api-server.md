# OpenAI-Compatible API Server for Kase
...
SSE streaming. By exposing this endpoint, kase becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.kase/.env`
2. Run `kase gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with kase through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns kase as a model) |
...
Authorization: Bearer kase-api-key-here
...
But kase sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "kase"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured kase model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of kase's core prompt
...
Once implemented, kase works as a drop-in backend for:
