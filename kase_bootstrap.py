"""Windows UTF-8 bootstrap for Kase entry points.
...
should be imported at the very top of every Kase entry point
(``kase``, ``kase``, ``kase-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import kase_bootstrap``
# (or ``from kase_bootstrap import apply_windows_utf8_bootstrap``) at
