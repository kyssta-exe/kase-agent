...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      kase = self'.packages.default;
      hermesVenv = kase.hermesVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "kase-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "kase-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "kase-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${kase}/bin/kase || (echo "FAIL: kase binary missing"; exit 1)
          test -x ${kase}/bin/kase || (echo "FAIL: kase binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${kase}/bin/kase version 2>&1 | grep -qi "kase" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "kase-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in kase kase kase-acp; do
            test -x ${kase}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "kase-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking kase --help ==="
          ${kase}/bin/kase --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${kase}/bin/kase --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "kase-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${kase}/share/kase/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${kase}/share/kase/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "KASE_BUNDLED_SKILLS" ${kase}/bin/kase || \
            (echo "FAIL: KASE_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: KASE_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "kase-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${kase}/share/kase/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${kase}/share/kase/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HERMES_BUNDLED_PLUGINS" ${kase}/bin/kase || \
        ...
        bundled-tui = pkgs.runCommand "kase-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${kase}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${kase}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "KASE_TUI_DIR" ${kase}/bin/kase || \
            (echo "FAIL: KASE_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: KASE_TUI_DIR set in wrapper"
        ...
        kase-node = pkgs.runCommand "kase-node-version" { } ''
          set -e
          echo "=== Checking HERMES_NODE in wrapper ==="
          grep -q "HERMES_NODE" ${kase}/bin/kase || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${kase}/bin/kase)
        ...
        managed-guard = pkgs.runCommand "kase-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${kase}/bin/kase config set model foo
          check_blocked "config edit" ${kase}/bin/kase config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = kase.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "kase-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${hermesWithExtra}/bin/kase || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hermesWithExtra}/bin/kase || \
          ...
          if grep -q "PYTHONPATH" ${kase}/bin/kase; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = kase.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "kase-extra-dependency-groups" { } ''
          set -e
        ...
        in pkgs.runCommand "kase-config-roundtrip" {
