...
# container recreation. Environment variables are written to $KASE_HOME/.env
# and read by kase at startup — no container recreation needed for env changes.
#
# Tool resolution: the kase wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    kase = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-kase)
    deepConfigType = lib.types.mkOptionType {
      name = "kase-config-attrs";
      description = "Kase YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "kase-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "kase-documents" { } (
      ''
        mkdir -p $out
    ...
    # kase user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "kase-container-entrypoint" ''
      set -eu
      ...
      # Ensure KASE_HOME is owned by the target user.
      ...
      if [ -n "''${KASE_HOME:-}" ] && [ -d "$KASE_HOME" ]; then
        find "$KASE_HOME" \! -user "$HERMES_UID" -exec chown "$HERMES_UID:$HERMES_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/kase-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/kase-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/kase ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/kase
        chmod 0440 /etc/sudoers.d/kase
      fi
    ...
    # so they can update without recreation. Env vars go through $KASE_HOME/.env.
    ...
    # Default: /var/lib/kase/workspace → /data/workspace.
  ...
  in {
    options.services.kase = with lib; {
      enable = mkEnableOption "Kase gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = kase;
        description = "The kase package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/kase";
        description = "State directory. Contains .kase/ subdir (KASE_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Kase config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $KASE_HOME/.env at activation time.
          Kase reads this file on every startup via load_kase_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $KASE_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $KASE_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `kase gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the kase user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the kase plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Kase discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          kase.plugins entry-point group. Each package must be built
          with the same Python interpreter as kase (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in kase's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the kase CLI to environment.systemPackages and export
          KASE_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.kase symlink to the service
            stateDir. These users are automatically added to the kase group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.kase.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the kase CLI to system PATH and export KASE_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.kase/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.KASE_HOME = "${cfg.stateDir}/.kase";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.kase.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the kase user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.kase: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed kase
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure kase is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.kase        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.kase/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.kase/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.kase/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.kase/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.kase/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."kase-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.kase
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.kase ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.kase ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.kase -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.kase/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.kase/$_subdir"
            chmod 2770 "${cfg.stateDir}/.kase/$_subdir"
            find "${cfg.stateDir}/.kase/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.kase/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.kase/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.kase/config.yaml
            chmod 0640 ${cfg.stateDir}/.kase/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.kase/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.kase/.managed
          chmod 0644 ${cfg.stateDir}/.kase/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.kase/.container-mode <<'HERMES_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    hermes_bin=${containerDataDir}/current-package/bin/kase
    HERMES_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.kase/.container-mode
            chmod 0644 ${cfg.stateDir}/.kase/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.kase/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.kase";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.kase" ]; then
                  rm -f "${symlinkPath}"
                  echo "kase: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.kase -> stateDir/.kase for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.kase";
                target = "${cfg.stateDir}/.kase";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "kase: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.kase/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.kase/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.kase/auth.json
              fi
            ''}
          ''}
          ...
          # Kase reads $KASE_HOME/.env at startup via load_kase_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.kase/.env"
    ...
    HERMES_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.kase/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.kase/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.kase/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.kase = {
          ...
          serviceConfig = {
            ...
            # $KASE_HOME/.env by the activation script. load_kase_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/kase"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the kase group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.kase = {
          description = "Kase Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env KASE_HOME=${containerDataDir}/.kase \
                --env HERMES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/kase gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
