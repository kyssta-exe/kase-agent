...
This page covers all commands related to [Kase profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `kase profile` without a subcommand shows help.
...
```bash
kase profile use <name>
```

Sets `<name>` as the active profile. All subsequent `kase` commands (without `-p`) will use this profile.
...
kase profile use default
```

## `kase profile create`

```bash
kase profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `kase profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `kase update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
kase profile create mybot

# Clone config only from current profile
kase profile create work --clone

# Clone everything from current profile
kase profile create backup --clone-all

# Clone config from a specific profile
kase profile create work2 --clone --clone-from work
```

## `kase profile describe`

```bash
kase profile describe [<name>] [options]
```
...
kase profile describe researcher

# Set it explicitly
kase profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
kase profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
kase profile describe --all --auto
```

## `kase profile delete`

```bash
kase profile delete <name> [options]
```
...
```bash
kase profile delete mybot
kase profile delete mybot --yes
```
...
```bash
kase profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Kase home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ kase profile show work
Profile: work
Path:    ~/.kase/profiles/work
...
```bash
kase profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Kase installation.
...
```bash
kase profile alias work
# Creates/updates ~/.local/bin/work

kase profile alias work --name mywork
# Creates ~/.local/bin/mywork

kase profile alias work --remove
...
```bash
kase profile rename <old-name> <new-name>
```
...
```bash
kase profile rename mybot assistant
# ~/.kase/profiles/mybot → ~/.kase/profiles/assistant
...
```bash
kase profile export <name> [options]
```
...
```bash
kase profile export work
# Creates work.tar.gz in the current directory

kase profile export work -o ./work-2026-03-29.tar.gz
```

## `kase profile import`

```bash
kase profile import <archive> [options]
```
...
```bash
kase profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

kase profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`kase profile export` / `import` are still the right commands for
...
### `kase profile install`

```bash
kase profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `kase -p telemetry`). |
...
kase profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
kase profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
kase profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
kase profile install ./telemetry/
```

### `kase profile update`

```bash
kase profile update <name> [--force-config] [--yes]
```
...
```bash
kase profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Kase version, author, env var requirements, the source URL/path, and
...
`kase profile list` also shows the distribution name and version in a
`Distribution` column, and `kase profile show <name>` / `delete <name>`
...
kase profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
kase profile install https://github.com/your-org/internal-assistant.git
```
...
Kase version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Kase can clone from.
3. Tell recipients to run `kase profile install <your-repo-url>`.
...
```bash
kase -p <name> <command> [options]
kase --profile <name> <command> [options]
```

Global flag to run any Kase command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
kase -p work chat -q "Check the server status"
kase --profile dev gateway start
kase -p personal skills list
kase -p work config edit
```

## `kase completion`

```bash
kase completion <shell>
```
...
- `kase profile <TAB>` — subcommands (list, use, create, etc.)
- `kase profile use <TAB>` — profile names
- `kase -p <TAB>` — profile names
