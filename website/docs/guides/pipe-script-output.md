...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `kase send`."
---

# Pipe Script Output to Messaging Platforms

`kase send` is a small, scriptable CLI that pushes a message to any
messaging platform Kase is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | kase send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `kase
...
kase send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | kase send --to telegram:-1001234567890

# Send a file
kase send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
kase send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
kase send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
kase send --list

# Filter by platform
kase send --list telegram
```
...
Any platform Kase ships adapters for works as a target:
...
`kase send` resolves the message body in this order:

1. **Positional argument** — `kase send --to telegram "hi"`
2. **`--file PATH`** — `kase send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | kase send --to telegram`

When stdin is a TTY (no pipe), Kase does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  kase send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `kase send` reuses your Kase config, the same script works on
any host where Kase is installed — no need to export bot tokens into
...
`kase send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  kase send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | kase send \
...
```bash
./train.py --epochs 200 && \
  kase send --to telegram "training done" || \
  kase send --to telegram "training failed (exit $?)"
```
...
kase send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(kase send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `kase send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `kase send` calls
the platform's REST endpoint directly using credentials from
`~/.kase/.env` and `~/.kase/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `kase gateway start` and retry.
...
The listing is built from `~/.kase/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`kase gateway
...
| Approach | Multi-platform | Reuses Kase creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `kase send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`kase send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `kase send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `kase send` shares with cron delivery.
