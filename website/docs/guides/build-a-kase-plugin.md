...
title: "Build a Kase Plugin"
description: "Step-by-step guide to building a complete Kase plugin with tools, hooks, data files, and skills"
---

# Build a Kase Plugin

This guide walks through building a complete Kase plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Kase has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.kase/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/docs/user-guide/features/skills) — `kase skills tap add <repo>` · [Publishing a tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.kase/plugins/calculator
cd ~/.kase/plugins/calculator
```
...
This tells Kase: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Kase may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `kase my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Kase continues fine
...
```bash
HERMES_PLUGINS_DEBUG=1 kase plugins list
```
...
The same logs are always written to `~/.kase/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
kase logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `kase plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.kase/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.kase/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.kase/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.kase/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `kase plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Kase into installing arbitrary packages — only specs Kase itself ships are eligible |
...
auditable in `~/.kase/logs/agent.log`. Plugins load after built-in
...
- **The system prompt is Kase's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `kase <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for kase my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: kase my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for kase my-plugin."""
...
After registration, users can run `kase my-plugin status`, `kase my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `kase name` in a terminal |
...
Kase has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.kase/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `kase gateway status` without SDK instantiation.
...
Kase also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/docs/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Kase without any Python plugin. Declare them in `~/.kase/config.yaml`:
...
Kase connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/docs/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.kase/hooks/<name>/`:
...
kase skills search my-workflow --source myorg/skills-repo
kase skills install myorg/skills-repo/my-workflow
```
...
```toml
# pyproject.toml
[project.entry-points."kase.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install kase-plugin-calculator
# Plugin auto-discovered on next kase startup
```
...
```python
# Wrong — will break if Kase passes extra context
def handler(args):
    ...
