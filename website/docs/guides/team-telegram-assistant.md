...
This tutorial walks you through setting up a Telegram bot powered by Kase that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Kase installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.kase/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Kase itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Kase Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Kase. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u kase-gateway -f
...
kase gateway start
kase gateway stop
tail -f ~/.kase/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `kase gateway install` to update the plist.
:::
...
```bash
kase gateway stop && kase gateway start
```
...
kase pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.kase/.env`:
...
Control how much detail the bot shows when using tools. In `~/.kase/config.yaml`:
...
Customize how the bot communicates by editing `~/.kase/SOUL.md`:

For a full guide, see [Use SOUL.md with Kase](/docs/guides/use-soul-with-kase).
...
```bash
# From the CLI
kase cron list          # View all scheduled jobs
kase cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u kase-gateway` (Linux) or `~/.kase/logs/gateway.log` (macOS) |
| Cron job output | `~/.kase/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.kase/cron/jobs.json` |
| Pairing data | `~/.kase/pairing/` |
| Session history | `~/.kase/sessions/` |
