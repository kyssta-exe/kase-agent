---
sidebar_position: 8
title: "Use Voice Mode with Kase"
description: "A practical guide to setting up and using Kase voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Kase
...
- you want Kase sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Kase.
...
## Step 1: make sure normal Kase works first
...
Kase supports both local and cloud speech stacks.
...
### If you use `kase setup`

If you choose NeuTTS in the setup wizard, Kase checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Kase transcribes and responds
...
- asking Kase to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Kase loop.

## Tuning CLI behavior

### Silence threshold

If Kase starts/stops too aggressively, tune:
...
Kase stays a normal chat bot, but can speak replies.
...
- you want Kase to function like a portable research or ops assistant
...
Kase joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Kase detects speech boundaries
- transcripts are posted in the associated text channel
- Kase responds in text and audio
...
1. get text Kase working
2. install `kase[voice]`
