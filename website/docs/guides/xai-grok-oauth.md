...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in Kase — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

Kase supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and Kase automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Kase — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Kase opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.kase/auth.json` and refreshed automatically before they expire.
...
```bash
kase auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, or SSH sessions where no browser is available, Kase detects the remote environment and prints the authorization URL instead of opening a browser.
...
kase auth add xai-oauth --no-browser
...
If you don't have a regular SSH client (e.g. you're running Kase inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — Kase skips the loopback listener and lets you paste the failed callback URL straight from your browser:

```bash
kase auth add xai-oauth --manual-paste
# Or via the model picker:
kase model --manual-paste
```

See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/Kyssta/kase/issues/26923).

## How the Login Works

1. Kase opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Kase and the tokens are saved to `~/.kase/auth.json`.
4. From then on, Kase refreshes the access token in the background — you stay signed in until you `kase auth remove xai-oauth` or revoke access from your xAI account settings.
...
```bash
kase config set model.default grok-4.3
kase config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.kase/config.yaml` will contain:
...
```bash
kase --provider xai-oauth        # canonical
kase --provider grok-oauth       # alias
kase --provider x-ai-oauth       # alias
kase --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `kase tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search auto-enables when xAI credentials are present
The `x_search` toolset auto-enables whenever xAI credentials (a SuperGrok / X Premium+ OAuth token or `XAI_API_KEY`) are configured. Disable explicitly via `kase tools` → `🐦 X (Twitter) Search` (press space) if you don't want this. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok / X Premium+ OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
| `HERMES_INFERENCE_PROVIDER` | Force the active provider at runtime, e.g. `HERMES_INFERENCE_PROVIDER=xai-oauth kase`. |

## Troubleshooting

### Token expired — not re-logging in automatically

Kase refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Kase surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Kase marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `kase auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Kase raises a timeout error.

**Fix:** re-run `kase auth add xai-oauth` (or `kase model`). The flow starts fresh.

### State mismatch (possible CSRF)

Kase detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Kase prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
This is **not** a stale-token problem — re-running `kase model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/Kyssta/kase/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
kase config set model.provider xai
```
...
**Fix:** run `kase model` and pick the xAI Grok OAuth provider, or run `kase auth add xai-oauth`.
...
```bash
kase auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `kase auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `kase auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Kase is on a different machine than your browser
