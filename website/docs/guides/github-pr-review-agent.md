...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Kase  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Kase instantly when PRs are opened or updated.
:::
...
- **Kase installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  kase gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.kase/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Kase can access GitHub. Start a chat:
...
```
Run: gh pr list --repo Kyssta/kase --state open --limit 3
```
...
Still in the chat, ask Kase to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo Kyssta/kase
```
...
A skill gives Kase consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.kase/skills/code-review
```

Create `~/.kase/skills/code-review/SKILL.md`:
...
Verify it loaded — start `kase` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Kase your team's standards:
...
2. Teach Kase your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
kase gateway status    # Is the gateway running?
kase cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-kase-plugin)** — wrap the review logic into a shareable plugin
