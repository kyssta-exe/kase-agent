...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.kase/.env` at the end.
...
4. Fill in:
   - **Name:** `Kase Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `kase-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the Kase app
New-CsApplicationAccessPolicy `
  -Identity "Kase-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Kase meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Kase-Meeting-Pipeline-Policy" `
...
Put the three values you collected into `~/.kase/.env`:
...
Kase ships a Graph auth smoke-test. From your Kase install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.kase/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `kase gateway restart`.
