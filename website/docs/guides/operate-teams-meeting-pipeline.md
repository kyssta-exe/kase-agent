...
```bash
kase teams-pipeline token-health
kase teams-pipeline token-health --force-refresh
```
...
```bash
kase teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
kase teams-pipeline maintain-subscriptions
kase teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Kase cron (recommended if you already run the Kase gateway)

Kase ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.kase/scripts/`. First create the script:

```bash
mkdir -p ~/.kase/scripts
cat > ~/.kase/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec kase teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.kase/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
kase cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/kase-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Kase Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=kase
EnvironmentFile=/etc/kase/env
ExecStart=/usr/local/bin/kase teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/kase-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Kase Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now kase-teams-pipeline-maintain.timer
systemctl list-timers kase-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/kase teams-pipeline maintain-subscriptions >> /var/log/kase/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.kase/.env` at the top of a wrapper script that crontab calls.
...
```bash
kase teams-pipeline subscriptions   # should show expirationDateTime advanced
kase teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
kase teams-pipeline list
kase teams-pipeline list --status failed
kase teams-pipeline show <job-id>
```

### Replay a stored job

```bash
kase teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
kase teams-pipeline fetch --meeting-id <meeting-id>
kase teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `kase teams-pipeline maintain-subscriptions --dry-run`
- inspect `kase teams-pipeline list --status failed`
...
- run `kase teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `kase teams-pipeline run`
...
- [ ] `kase teams-pipeline validate` returns an OK snapshot
- [ ] `kase teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Kase cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
