---
sidebar_position: 7
title: "Use SOUL.md with Kase"
description: "How to use SOUL.md to shape Kase's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Kase

`SOUL.md` is the **primary identity** for your Kase instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Kase to feel like the same assistant every time you talk to it — or if you want to replace the Kase persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Kase should be
- what Kase should avoid stylistically
- how Kase should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Kase is and how Kase speaks
...
Kase now uses only the global SOUL file for the current instance:

```text
~/.kase/SOUL.md
```

If you run Kase with a custom home directory, it becomes:
...
Kase automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Kase does not overwrite it
- if the file exists but is empty, Kase adds nothing from it to the prompt

## How Kase uses it

When Kase starts a session, it reads `SOUL.md` from `KASE_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Kase feels.
...
Kase already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Kase should sound.

# Avoid
What Kase should not do.

# Defaults
How Kase should behave when ambiguity appears.
```
...
Then restart Kase or start a new session.
...
### I edited SOUL.md but Kase still sounds the same

Check:
- you edited `~/.kase/SOUL.md` or `$KASE_HOME/SOUL.md`
...
### Kase is ignoring parts of my SOUL.md
