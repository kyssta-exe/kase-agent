---
sidebar_position: 6
title: "Use MCP with Kase"
description: "A practical guide to connecting MCP servers to Kase, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Kase

This guide shows how to actually use MCP with Kase in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Kase tool
- you want Kase to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Kase to internal APIs, databases, or company systems without modifying Kase core

Do not use MCP when:
- a built-in Kase tool already solves the job well
...
- Kase remains the agent
- MCP servers contribute tools
- Kase discovers those tools at startup or reload time
...
If you installed Kase with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Kase banner/status should show MCP integration when configured
- ask Kase what tools it has available
...
## WSL2: bridge Kase in WSL to Windows Chrome

This is the practical setup when:

- Kase runs inside WSL2
...
In this setup, Kase does **not** connect to Chrome directly. Instead:

- Kase runs in WSL
- Kase starts a local stdio MCP server
...
```text
Kase (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Kase stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Kase core browser transport
...
```bash
kase mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
kase mcp test chrome-devtools-win
```

Then start a fresh Kase session or run:
...
Once loaded, Kase can use the MCP-prefixed browser tools directly. For example:
...
If Kase runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Kase from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Kase from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Kase:
...
So Kase will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Kase to reason over a bounded workspace.
...
Now Kase can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Kase core.
...
Because Kase now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
