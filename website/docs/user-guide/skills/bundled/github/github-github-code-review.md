...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.kase/.env ] && grep -q "^GITHUB_TOKEN=" ~/.kase/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.kase/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${KASE_HOME:-$HOME/.kase}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Kase. Code looks clean — good test coverage, no security concerns."
