...
- Give it a title like "kase-&lt;machine-name>"
...
elif [ -f ~/.kase/.env ] && grep -q "^GITHUB_TOKEN=" ~/.kase/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.kase/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
