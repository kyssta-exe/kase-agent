...
description: "Configure and use Honcho memory with Kase -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Kase -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `kase skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Kase

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Kase profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/kase#running-honcho-locally-with-kase

### Verify

```bash
kase honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Kase profile's AI peer
...
Honcho models conversations as interactions between **peers**. Kase creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Kase instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Kase run |
| `global` | Single session across all directories |

Manual override: `kase honcho map my-project-name`
...
Each Kase profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block kase.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `kase.coder` host block in `honcho.json`
...
```bash
kase honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Kase when Honcho memory is active.
...
Config file: `$KASE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `kase honcho setup`. Ensure `memory.provider: honcho` is in `~/.kase/config.yaml`.

### Memory not persisting across sessions
Check `kase honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `kase profile create <name> --clone`. For existing profiles: `kase honcho sync`.
...
| Command | Description |
|---------|-------------|
| `kase honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `kase honcho status` | Show resolved config, connection test, peer info for active profile |
| `kase honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `kase honcho disable` | Disable Honcho for the active profile |
| `kase honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `kase honcho peers` | Show peer identities across all profiles |
| `kase honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `kase honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `kase honcho sessions` | List known directory-to-session-name mappings |
| `kase honcho map <name>` | Map current working directory to a Honcho session name |
| `kase honcho identity` | Seed AI peer identity or show both peer representations |
| `kase honcho sync` | Create host blocks for all Kase profiles that don't have one yet |
| `kase honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Kase + Honcho |
| `kase memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `kase memory status` | Show active memory provider and config |
| `kase memory off` | Disable external memory provider |
