...
| | |
|---|---|
| Source | Optional — install with `kase skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Kase |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Kase terminal calls (desktop app flow only)
...
### Service Account (recommended for Kase)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.kase/.env` (the skill will prompt for this on first load).
...
## Kase Execution Pattern (desktop app flow)

Kase terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/kase-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/kase-op.sock"
