...
| | |
|---|---|
| Source | Optional — install with `kase skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Kase"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.kase/.env`:
...
```bash
CANVAS="python $KASE_HOME/skills/productivity/canvas/scripts/canvas_api.py"
