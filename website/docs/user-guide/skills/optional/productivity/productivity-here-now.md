...
| | |
|---|---|
| Source | Optional — install with `kase skills install official/productivity/here-now` |
...
bash "$PUBLISH" {file-or-dir} --client kase
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client kase
```
...
| `--client {name}`      | Agent name for attribution (e.g. `kase`)    |
