...
| | |
|---|---|
| Source | Optional — install with `kase skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Kase core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Kase native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Kase native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Kase native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Kase usage patterns
...
- Do not silently choose Parallel when Kase native tools are already sufficient.
