...
| | |
|---|---|
| Source | Optional — install with `kase skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Kase |
| License | MIT |
...
import upstream classes into Kase itself.
...
```bash
mkdir -p ~/.kase/cache/darwinian-evolver && cd ~/.kase/cache/darwinian-evolver
...
```bash
cd ~/.kase/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.kase/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.kase/skills/research/darwinian-evolver
DE_DIR=~/.kase/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Kase core.
   Custom driver scripts under `~/.kase/skills/...` are user-side and fine.
