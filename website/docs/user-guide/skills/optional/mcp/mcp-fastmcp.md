...
| | |
|---|---|
| Source | Optional — install with `kase skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Kase or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Kase. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.kase/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.kase/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Kase integration, either:

- configure the server in `~/.kase/config.yaml` using the `native-mcp` skill, or
...
### Kase cannot see the deployed server

The server-building part may be correct while the Kase config is not. Load the `native-mcp` skill and configure the server in `~/.kase/config.yaml`, then restart Kase.
