...
| | |
|---|---|
| Source | Optional — install with `kase skills install official/devops/pinggy-tunnel` |
| Path | `optional-skills/devops/pinggy-tunnel` |
| Version | `0.1.0` |
| Author | Teknium (teknium1), Kase |
| License | MIT |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. Kase' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
