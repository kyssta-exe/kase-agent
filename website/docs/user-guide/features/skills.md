...
All skills live in **`~/.kase/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Kase at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Kase to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.kase/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
kase chat --toolsets skills -q "What skills do you have?"
kase chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.kase/cache/chart-q4-2025.png
...
When a missing value is encountered, Kase asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `kase setup` or `~/.kase/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `kase config migrate` prompts for unconfigured settings, and `kase config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.kase/skills/                  # Single source of truth
...
If you maintain skills outside of Kase — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Kase to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.kase/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.kase/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Kase process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Kase ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.kase/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.kase/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `kase bundles list`.
...
kase bundles create research

# Overwrite an existing bundle
kase bundles create backend-dev --skill ... --force

# Delete a bundle
kase bundles delete backend-dev

# Re-scan ~/.kase/skill-bundles/ and report changes
kase bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.kase/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.kase/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
kase skills browse                              # Browse all hub skills (official first)
kase skills browse --source official            # Browse only official optional skills
kase skills search kubernetes                   # Search all sources
kase skills search react --source skills-sh     # Search the skills.sh directory
kase skills search https://mintlify.com/docs --source well-known
kase skills inspect openai/skills/k8s           # Preview before installing
kase skills install openai/skills/k8s           # Install with security scan
kase skills install official/security/1password
kase skills install skills-sh/vercel-labs/json-render/json-render-react --force
kase skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
kase skills list --source hub                   # List hub-installed skills
kase skills check                               # Check installed hub skills for upstream updates
kase skills update                              # Reinstall hub skills with upstream changes when needed
kase skills audit                               # Re-scan all hub skills for security
kase skills uninstall k8s                       # Remove a hub skill
kase skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
kase skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
kase skills publish skills/my-skill --to github --repo owner/repo
kase skills snapshot export setup.json          # Export skill config
kase skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Kase. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `kase skills search <query> --source skills-sh`. Kase resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Kase currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Kase repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Kase can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
kase skills search https://mintlify.com/docs --source well-known
kase skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
kase skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Kase can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
kase skills install openai/skills/k8s
kase skills tap add myorg/skills-repo
```
...
- Kase source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Kase supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Kase source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Kase can search and convert agent entries from LobeHub's public catalog into installable Kase skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Kase source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Kase integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Kase' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Kase source id: `browse-sh`
- Trust level: `community`

```bash
kase skills search airbnb --source browse-sh
kase skills inspect browse-sh/airbnb.com/search-listings-ddgioa
kase skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Kase fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
kase skills install https://sharethis.chat/SKILL.md
kase skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
kase skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `kase skills update` re-fetches from the same URL automatically when you want to refresh.
...
`kase skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Kase | Always trusted |
...
```bash
kase skills check          # Report which installed hub skills changed upstream
kase skills update         # Reinstall only the skills with updates available
kase skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Kase users add with `kase skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.kase.tags`, `version`, `author`, `platforms`, `metadata.kase.config`).
...
Kase discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Kase user can subscribe and install:

```bash
kase skills tap add my-org/kase-skills
kase skills search deploy
kase skills install my-org/kase-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.kase/.hub/taps.json`:
...
The `kase skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `kase skills tap list` shows the effective path per tap.
...
```bash
kase skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.kase/.hub/taps.json` (created on demand).

## Bundled skill updates (`kase skills reset`)

Kase ships with a set of bundled skills in `skills/` inside the repo. On install and on every `kase update`, a sync pass copies those into `~/.kase/skills/` and records a manifest at `~/.kase/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Kase recomputes the hash of your local copy and compares it to the origin hash:
...
kase skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `KASE_HOME`, so `kase -p coder skills reset <name>` only affects that profile.
:::
