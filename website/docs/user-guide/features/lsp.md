...
Kase runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Kase
...
1. Kase captures a baseline of current diagnostics for the file.
...
…). Kase auto-detects the binary on PATH or in
`<KASE_HOME>/lsp/bin/`.
...
`node_modules` tree — Kase installs both packages together when you
run `kase lsp install typescript` or auto-install fires on first
use.

## CLI

```
kase lsp status          # service state + per-server install status
kase lsp list            # registry, optionally --installed-only
kase lsp install <id>    # eagerly install one server
kase lsp install-all     # try every server with a known recipe
kase lsp restart         # tear down running clients
kase lsp which <id>      # print resolved binary path
```

`kase lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <KASE_HOME>/lsp/bin
...
When `install_strategy: auto`, Kase installs binaries into
`<KASE_HOME>/lsp/bin/`. NPM packages land in
`<KASE_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Kase-owned and is
...
Servers are kept alive for the life of the Kase process. There's
...
**`kase lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<KASE_HOME>/lsp/bin/`. Run
`kase lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `kase lsp status`**
...
When `kase lsp status` shows a `Backend warnings` section, install
...
Check `~/.kase/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `kase lsp restart` to clear the set;
