...
chat completions — use your Kase-managed provider subscription as their
...
| Use case | "Use Kase as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Kase stores
the refresh token in `~/.kase/auth.json` — the same place all Kase
...
```
Starting Kase proxy for Nous Portal
...
Model:      Kase-4-70B    # or Kase-4.3-36B, Kase-4-405B
```
...
```
Kase proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `kase login nous`. If you see
...
supports; check `portal.kyssta.com/models`.
...
INFERENCE_TEXT_MODEL=Kase-4-70B
```
...
```bash
kase proxy start --host 0.0.0.0 --port 8645
```
...
[portal.kyssta.com](https://portal.kyssta.com).
...
`kase_cli/proxy/adapters/<provider>.py` and registering it in
