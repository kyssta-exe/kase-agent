...
Kase can drive your Mac's desktop — clicking, typing, scrolling,
...
```
kase computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `kase computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `kase tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
- **`kase computer-use install --upgrade`** — manual force-refresh.
...
`kase computer-use status` shows the installed version next to the
binary path.
...
Kase applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.kase/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Kase applies four layers of optimisation:
...
`kase computer-use install` to fetch the cua-driver binary, or run
`kase tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/Kyssta/kase/blob/main/skills/apple/macos-computer-use/SKILL.md)
