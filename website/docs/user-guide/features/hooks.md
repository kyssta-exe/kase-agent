...
Kase has three hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.kase/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.kase/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.kase/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".kase" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.kase/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.kase/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.kase/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.kase/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".kase" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.kase/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.kase/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/kase-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.kase/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Kase does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.kase/BOOT.md` with natural-language startup instructions.
...
Create `~/.kase/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `kase cron list` and check if any scheduled jobs failed overnight.
...
**`~/.kase/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.kase/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.kase/hooks/boot-md/handler.py`**

```python
"""Run ~/.kase/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".kase" / "BOOT.md"
...
```bash
kase logs --follow --level INFO | grep boot-md
```
...
Delete `~/.kase/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of Kase shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.kase/hooks/`
...
See the **[Build a Plugin guide](/docs/guides/build-a-kase-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
Declare shell-script hooks in your `cli-config.yaml` and Kase will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`kase_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.kase/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.kase/agent-hooks/` (by convention) | `~/.kase/plugins/<name>/` | `~/.kase/hooks/<name>/` |
...
Each time the event fires, Kase spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Kase-canonical
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.kase/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kase/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.kase/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.kase/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.kase/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kase/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Kase event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.kase/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kase/agent-hooks/log-orchestration.sh
log=~/.kase/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Kase sees it, then persists the decision to `~/.kase/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `kase --accept-hooks chat`)
2. `KASE_ACCEPT_HOOKS=1` environment variable
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `kase hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

### The `kase hooks` CLI

| Command | What it does |
|---------|--------------|
| `kase hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `kase hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `kase hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `kase hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.kase/agent-hooks/` so the path is easy to audit.
- Re-run `kase hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
