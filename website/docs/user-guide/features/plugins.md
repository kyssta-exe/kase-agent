...
title: "Plugins"
description: "Extend Kase with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Kase has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/docs/developer-guide/adding-tools) page is for built-in Kase
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Kase Plugin](/docs/guides/build-a-kase-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.kase/plugins/` with a `plugin.yaml` and Python code:

```
~/.kase/plugins/my-plugin/
...
Start Kase — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.kase/plugins/hello-world/plugin.yaml`**
...
**`~/.kase/plugins/hello-world/__init__.py`**

```python
"""Minimal Kase plugin — registers a tool and a hook."""
...
Drop both files into `~/.kase/plugins/hello-world/`, restart Kase, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.kase/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting Kase.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `kase <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `kase plugins install` |
| Distribute via pip | `[project.entry-points."kase.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Kase — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
| User | `~/.kase/plugins/` | Personal plugins |
| Project | `.kase/plugins/` | Project-specific plugins (requires `HERMES_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `kase.plugins` entry_points | Distributed packages |
| Nix | `services.kase.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/docs/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Kase also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.kase/plugins/model-providers/<name>/` and `~/.kase/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

Sub-category plugins surface in `kase plugins list` and the interactive `kase plugins` UI under their **path-derived key** — e.g. `observability/langfuse`, `image_gen/openai`, `platforms/teams`. That key (not the bare manifest `name:`) is the value you pass to `kase plugins enable …` / `disable …` and the string to add under `plugins.enabled` in `config.yaml`.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `kase plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.kase/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
kase plugins                    # interactive toggle (space to check/uncheck)
kase plugins enable <name>      # add to allow-list
kase plugins disable <name>     # remove from allow-list + add to disabled
```

After `kase plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Kase' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.kase/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.kase/plugins/`.

### Migration for existing users

When you upgrade to a version of Kase that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.kase/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
Kase has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.kase/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Kase also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Kase Plugin](/docs/guides/build-a-kase-plugin) · [Adding Tools](/docs/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/docs/user-guide/features/hooks) · [Build a Kase Plugin](/docs/guides/build-a-kase-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Kase Plugin](/docs/guides/build-a-kase-plugin) · [Extending the CLI](/docs/developer-guide/extending-the-cli) |
| A **subcommand** for `kase <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/docs/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Kase auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/docs/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `kase skills tap add <repo>` | [Skills Hub](/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.kase/hooks/<name>/` | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `kase plugins install` needed. See the **[Nix Setup guide](/docs/getting-started/nix-setup#plugins)** for full details.
...
```bash
kase plugins                                       # unified interactive UI
kase plugins list                                  # table: enabled / disabled / not enabled
kase plugins install user/repo                     # install from Git, then prompt Enable? [y/N]
kase plugins install user/repo --enable            # install AND enable (no prompt)
kase plugins install user/repo --no-enable         # install but leave disabled (no prompt)
kase plugins update my-plugin                      # pull latest
kase plugins remove my-plugin                      # uninstall
kase plugins enable my-plugin                      # add to allow-list (flat plugin)
kase plugins enable observability/langfuse         # add to allow-list (sub-category plugin)
kase plugins disable my-plugin                     # remove from allow-list + add to disabled
```

For plugins under a sub-category directory (e.g. `plugins/observability/langfuse/`, `plugins/image_gen/openai/`), use the full `<category>/<plugin>` key — that's exactly what `kase plugins list` shows in the **Name** column.

### Interactive UI

Running `kase plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `kase plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/docs/guides/build-a-kase-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
