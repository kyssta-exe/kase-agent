...
Type `@` followed by a reference to inject content directly into your message. Kase expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Kase env: `$KASE_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$KASE_HOME/skills/.hub/`
