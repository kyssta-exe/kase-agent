...
When Kase runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.kase/config.yaml` under `agent.custom_instructions`.
...
If you use Kase' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.kase/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Kase's deliverable
