...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Kase — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Kase supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Kase](/docs/guides/use-voice-mode-with-kase).

## Prerequisites

Before using voice features, make sure you have:

1. **Kase installed** — `pip install kase` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `kase model` or set your preferred provider credentials in `~/.kase/.env`
3. **A working base setup** — run `kase` to verify the agent responds to text before enabling voice

:::tip
The `~/.kase/` directory and default `config.yaml` are created automatically the first time you run `kase`. You only need to create `~/.kase/.env` manually for API keys.
:::
...
pip install "kase[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "kase[messaging]"

# Premium TTS (ElevenLabs)
pip install "kase[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "kase[all]"
```
...
Voice mode is available in both the **classic CLI** (`kase chat`) and the **TUI** (`kase --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.kase/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
kase                # Start the interactive CLI
```
...
1. Start the CLI with `kase` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.kase/config.yaml` (default: `ctrl+b`).
:::
...
```bash
kase gateway        # Start the gateway (connects to configured platforms)
kase gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.kase/.env`:
```bash
...
```bash
kase gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.kase/.env`
...
- Review gateway logs: `tail -f ~/.kase/logs/gateway.log`
