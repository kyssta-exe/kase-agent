...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Kase supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.kase/config.yaml`:
...
- **CLI mode** — configured in `~/.kase/config.yaml`, loaded at startup
