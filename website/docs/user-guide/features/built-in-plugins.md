...
description: "Plugins shipped with Kase that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Kase ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.kase/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/docs/user-guide/features/plugins) page for the general plugin system, and [Build a Kase Plugin](/docs/guides/build-a-kase-plugin) to write your own.
...
2. **User** — `~/.kase/plugins/<name>/`
3. **Project** — `./.kase/plugins/<name>/` (requires `HERMES_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `kase.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `kase memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `kase plugins list` and the interactive `kase plugins` UI), but none load until you explicitly enable them:

```bash
kase plugins enable disk-cleanup
```

Or via `~/.kase/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Kase. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
kase plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `kase plugins enable <name>`.
...
| `kase-achievements` | dashboard tab | Steam-style collectible badges generated from your real Kase session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `kase memory` and `kase plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `KASE_HOME` or `/tmp/kase-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under KASE_HOME | always | Never |
...
**State** — everything lives at `$KASE_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `KASE_HOME` or `/tmp/kase-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `kase plugins enable disk-cleanup` (or check the box in `kase plugins`).

**Disabling again:** `kase plugins disable disk-cleanup`.

### observability/langfuse

Traces Kase turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Kase' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `kase logs`.
...
kase plugins enable observability/langfuse
```

Or check the box in the interactive `kase plugins` UI. Then put the credentials in `~/.kase/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Kase turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
Session grouping keys off the Kase session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `kase chat` session lives under one Langfuse session.

**Verify:**

```bash
kase plugins list                 # observability/langfuse should show "enabled"
kase chat -q "hello"              # check the Langfuse UI for a "Kase turn" trace
```
...
Kase-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Kase-prefixed wins when both are set.
...
**Disabling:** `kase plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.kase/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
kase plugins enable google_meet
...
**Disabling:** `kase plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.kase/cache/google_meet/` until you remove them.

### kase-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Kase session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Kase feature changes.

**How it works:**

- Scans your entire `~/.kase/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$KASE_HOME/plugins/kase-achievements/state.json`
...
| Secret | Hidden until Kase detects the first related signal in your history |

**API** — routes mount under `/api/plugins/kase-achievements/`:
...
**State files** — live under `$KASE_HOME/plugins/kase-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Kase updates. |
...
**Enabling:** Nothing to enable — `kase-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `kase dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/kase-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.kase/plugins/kase-achievements/` that ships no dashboard. The plugin's state files under `$KASE_HOME/plugins/kase-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Kase plugin — see [Build a Kase Plugin](/docs/guides/build-a-kase-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.kase/plugins/<name>/`
- Manifest source is reported as `bundled` in `kase plugins list`
