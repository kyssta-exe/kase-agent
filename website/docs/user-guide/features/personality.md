...
description: "Customize Kase's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Kase's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `KASE_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Kase is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Kase now seeds a default `SOUL.md` automatically in:

```text
~/.kase/SOUL.md
```

More precisely, it uses the current instance's `KASE_HOME`, so if you run Kase with a custom home directory, it will use:
...
- Kase creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Kase loads `SOUL.md` only from `KASE_HOME`
- Kase does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Kase falls back to a built-in default identity
...
If Kase loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `KASE_HOME`, the personality belongs to the Kase instance itself.

That also makes it easier to teach users:
- "Edit `~/.kase/SOUL.md` to change Kase' default personality."
...
- how Kase should handle uncertainty, disagreement, or ambiguity
...
## What Kase injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Kase falls back to a built-in default identity ("You are Kase, an intelligent AI assistant created by Kyssta..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Kase ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Kase, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Kase its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.kase/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.kase/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-kase)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Kase speaks
- `display.skin` and `/skin` affect how Kase looks in the terminal
