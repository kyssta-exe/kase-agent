...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Kase as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Kase integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Kase opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Kase responds to every message. |
| Group chats | Kase responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Kase will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `kase gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Kase runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Kase behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Kase starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Kase responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Kase responds to @mentions from any user in any group. |
| `allowlist` | Kase only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Kase ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Kase read all group traffic without requiring an @mention:
...
Kase auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Kase ignores messages sent by other bots. Enable bot-to-bot messaging when you want Kase to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Kase. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `kase update --gateway` needs confirmation, the adapter records the selected answer in Kase's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Kase reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.kase/feishu_comment_rules.json` (pairing grants in `~/.kase/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
|---------|---------|---------|
| Quiet period | `KASE_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `KASE_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `KASE_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `KASE_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.kase/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `KASE_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `KASE_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `KASE_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `KASE_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `KASE_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `KASE_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `kase gateway setup` |
| `Another local Kase gateway is already using this Feishu app_id` | Only one Kase instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Kase can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
...
Feishu / Lark uses the `kase-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
