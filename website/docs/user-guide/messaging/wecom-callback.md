...
Connect Kase to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Kase supports two WeCom integration modes:
...
3. Kase decrypts the message, queues it for the agent
...
(Use `kase gateway start` only after `kase gateway install` has registered the systemd/launchd service.)
