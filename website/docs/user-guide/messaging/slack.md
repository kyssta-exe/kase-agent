...
sidebar_position: 4
title: "Slack"
description: "Set up Kase as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Kase to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Kase instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Kase uses the modern
...
The fastest path is to paste a manifest Kase generates for you. It
...
### Option A: From a Kase-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   kase slack manifest --write
   ```
   This writes `~/.kase/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Kase") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Kase can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `kase-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Kase configuration issue.
:::
...
Kase uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
kase gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
kase gateway              # Foreground
kase gateway install      # Install as a user service
sudo kase gateway install --system   # Linux only: boot-time system service
```
...
Every Kase command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Kase command with its description.

Under the hood: Kase ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/Kyssta/kase/blob/main/kase_cli/commands.py)
...
When Kase adds new commands (e.g. after `kase update`), regenerate
...
3. Paste the new contents of `~/.kase/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/kase <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/kase btw run the tests` — Kase routes it the same way as `/btw
run the tests`. Free-form questions also work: `/kase what's the
...
Slack never delivers them to Kase.

As a workaround, Kase recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Kase treats it identically to the slash form and replies in the same
thread.
...
```bash
kase slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Kase behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Kase what time is it?`). In channels, Kase replies in a thread attached to that message. |
| **Threads** | If you @mention Kase inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.kase/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Kase in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Kase will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Kase will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Kase`).

---

## Multi-Workspace Support

Kase can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Kase also loads tokens from an **OAuth token file** at:

```
~/.kase/slack_tokens.json
```
...
- When a message arrives, Kase uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Kase` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Kase` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Kase now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Kase`)
...
- Tokens should be stored in `~/.kase/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Kase config directory
