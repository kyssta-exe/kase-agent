...
description: "Set up Kase as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Kase to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Kase strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create kase-bot --allow-anonymous
devtunnel port create kase-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host kase-bot
...
Alternatively, configure via `~/.kase/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.kase/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create kase-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs kase` for tracebacks |
...
- Store credentials in `~/.kase/.env` with permissions `600` (`chmod 600 ~/.kase/.env`)
