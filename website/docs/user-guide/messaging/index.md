...
description: "Chat with Kase from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Kase from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with Kase](/docs/guides/use-voice-mode-with-kase).
...
```bash
kase gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
kase gateway              # Run in foreground
kase gateway setup        # Configure messaging platforms interactively
kase gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo kase gateway install --system   # Linux only: install a boot-time system service
kase gateway start        # Start the default service
kase gateway stop         # Stop the default service
kase gateway status       # Check default service status
kase gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Kase to the latest version |
...
Configure per-platform overrides in `~/.kase/gateway.json`:
...
kase pairing approve telegram XKGH5N7P

# Other pairing commands:
kase pairing list          # View pending + approved users
kase pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Kase appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.kase/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.kase/config.yaml`:
...
```bash
KASE_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
kase gateway install               # Install as user service
kase gateway start                 # Start the service
kase gateway stop                  # Stop the service
kase gateway status                # Check status
journalctl --user -u kase-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo kase gateway install --system
sudo kase gateway start --system
sudo kase gateway status --system
journalctl -u kase-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Kase will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Kase installations on the same machine (with different `KASE_HOME` directories), each gets its own systemd service name. The default `~/.kase` uses `kase-gateway`; other installations use `kase-gateway-<hash>`. The `kase gateway` commands automatically target the correct service for your current `KASE_HOME`.
:::

### macOS (launchd)

```bash
kase gateway install               # Install as launchd agent
kase gateway start                 # Start the service
kase gateway stop                  # Stop the service
kase gateway status                # Check status
tail -f ~/.kase/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.kase.gateway.plist`. It includes three environment variables:
...
- **KASE_HOME** — scopes the gateway to your Kase installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `kase gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `KASE_HOME` directory gets its own launchd label. The default `~/.kase` uses `ai.kase.gateway`; other installations use `ai.kase.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `kase-cli` | Full access |
| Telegram | `kase-telegram` | Full tools including terminal |
| Discord | `kase-discord` | Full tools including terminal |
| WhatsApp | `kase-whatsapp` | Full tools including terminal |
| Slack | `kase-slack` | Full tools including terminal |
| Google Chat | `kase-google_chat` | Full tools including terminal |
| Signal | `kase-signal` | Full tools including terminal |
| SMS | `kase-sms` | Full tools including terminal |
| Email | `kase-email` | Full tools including terminal |
| Home Assistant | `kase-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `kase-mattermost` | Full tools including terminal |
| Matrix | `kase-matrix` | Full tools including terminal |
| DingTalk | `kase-dingtalk` | Full tools including terminal |
| Feishu/Lark | `kase-feishu` | Full tools including terminal |
| WeCom | `kase-wecom` | Full tools including terminal |
| WeCom Callback | `kase-wecom-callback` | Full tools including terminal |
| Weixin | `kase-weixin` | Full tools including terminal |
| BlueBubbles | `kase-bluebubbles` | Full tools including terminal |
| QQBot | `kase-qqbot` | Full tools including terminal |
| Yuanbao | `kase-yuanbao` | Full tools including terminal |
| Microsoft Teams | `kase-teams` | Full tools including terminal |
| API Server | `kase-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `kase-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.kase/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
