...
description: "Run Kase directly on an Android phone with Termux"
---

# Kase on Android with Termux

This is the tested path for running Kase directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Kase from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Kase now ships a Termux-aware installer path:
...
- links `kase` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `kase` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/kase" "$PREFIX/bin/kase"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `kase` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.kase/.env`.
...
### `kase doctor` says ripgrep or Node is missing
