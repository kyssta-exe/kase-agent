...
description: "How Kase stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `kase_cli/cron.py` | CLI `kase cron` subcommands |
...
Jobs are stored in `~/.kase/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `kase cron` commands are run or during active CLI sessions.
...
```python
# ~/.kase/scripts/check_competitors.py
...
2. **Environment variable** — `KASE_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.kase/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `kase cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `kase cron` CLI provides direct job management:

```bash
kase cron list                    # Show all jobs
kase cron create                  # Interactive job creation (alias: add)
kase cron edit <job_id>           # Edit job configuration
kase cron pause <job_id>          # Pause a running job
kase cron resume <job_id>         # Resume a paused job
kase cron run <job_id>            # Trigger immediate execution
kase cron remove <job_id>         # Delete a job
```
