...
[PR #12550](https://github.com/Kyssta/kase/pull/12550) proposed a
...
(`kase-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Kase `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `kase-acp`, `kase-api-server`, core toolsets (gated on CDP reachability)
...
- `kase_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
