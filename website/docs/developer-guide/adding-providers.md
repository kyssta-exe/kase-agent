...
description: "How to add a new inference provider to Kase — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Kase can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `kase model` menu entries
...
1. `kase_cli/auth.py` decides how credentials are found.
2. `kase_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `kase_cli/models.py` and `kase_cli/main.py` make the provider show up in the CLI. (`kase_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `kase_cli/auth.py`
2. `kase_cli/models.py`
3. `kase_cli/runtime_provider.py`
4. `kase_cli/main.py`
...
:::tip
`kase_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `kase setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$KASE_HOME/plugins/model-providers/` both get picked up.
...
7. `kase model` menu includes the provider
8. `kase setup` wizard delegates to `main.py` automatically
...
User plugins at `$KASE_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `kase model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `kase_cli/auth.py`
- `_PROVIDER_LABELS` in `kase_cli/models.py`
- `_PROVIDER_ALIASES` in both `kase_cli/auth.py` and `kase_cli/models.py`
- CLI `--provider` choices in `kase_cli/main.py`
...
## Step 2: Add auth metadata in `kase_cli/auth.py`
...
- What env vars should Kase check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `kase_cli/models.py`
...
## Step 4: Resolve runtime data in `kase_cli/runtime_provider.py`
...
Be careful with API-key precedence. Kase already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `kase_cli/main.py`

A provider is not discoverable until it shows up in the interactive `kase model` flow.

Update these in `kase_cli/main.py`:
...
:::tip
`kase_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `kase model` and `kase setup` automatically.
:::
...
When you add a native provider, double-check that Kase is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m kase_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `kase_cli/auth.py`
- [ ] aliases added in `kase_cli/auth.py` and `kase_cli/models.py`
- [ ] model catalog added in `kase_cli/models.py`
- [ ] runtime branch added in `kase_cli/runtime_provider.py`
- [ ] CLI wiring added in `kase_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `kase model` but not `kase setup`
