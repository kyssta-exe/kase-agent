...
description: "How Kase builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Kase deliberately separates:
...
1. agent identity — `SOUL.md` from `KASE_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.kase/SOUL.md)
You are Kase, an AI assistant created by Kyssta.
...
`SOUL.md` lives at `~/.kase/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .kase.md / HERMES.md (walks to git root)
    ...
    # SOUL.md from KASE_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.kase.md`, `HERMES.md` | CWD up to git root | Kase-native project config |
...
- **YAML frontmatter stripped** — `.kase.md` frontmatter is removed (reserved for future config overrides)
...
1. `.kase.md` / `HERMES.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Kase already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.kase/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.kase/MEMORY.md` and `~/.kase/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.kase.md`, `HERMES.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Kase.
...
- if you want to change how Kase assembles prompts for everyone, change Python and treat it as a code contribution
