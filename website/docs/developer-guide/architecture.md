...
sidebar_position: 1
title: "Architecture"
description: "Kase internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Kase internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ kase_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # KaseCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── kase_state.py           # SQLite session/state database with FTS5
├── kase_constants.py       # KASE_HOME, profile-aware paths
...
│
├── kase_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `kase` subcommands (large file)
...
│   ├── skills_config.py      # kase skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # kase tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # kase gateway start/stop
│
...
```text
User input → KaseCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .kase.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.kase/plugins/` (user), `.kase/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `kase plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-kase-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Kase as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`kase -p <name>`) gets its own KASE_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
