...
Kase uses a SQLite database (`~/.kase/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `kase_state.py`


## Architecture Overview

```
~/.kase/state.db (SQLite, WAL mode)
...
Multiple kase processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.kase/state.db
...
Default path: `~/.kase/state.db`

This is derived from `kase_constants.get_kase_home()` which resolves to
`~/.kase/` by default, or the value of `KASE_HOME` environment variable.
