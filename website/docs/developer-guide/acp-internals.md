...
The ACP adapter wraps Kase' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
kase acp / kase-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.kase/.env
  -> configure stderr logging
  -> construct KaseACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'kase[acp]==<version>' kase-acp`, pointed at the `kase` PyPI release.
...
- `allow_once` -> Kase `once`
- `allow_always` -> Kase `always`
- reject options -> Kase `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Kase tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["kase-acp"])
...
Instead it reuses Kase' runtime resolver:

- `acp_adapter/auth.py`
- `kase_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Kase provider/credentials. It also always advertises a terminal setup auth method (`kase-setup`, args `--setup`) so first-run registry clients can open Kase' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.kase/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `kase-acp` toolset definition
- `kase_cli/main.py` — `kase acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `kase-acp` script
