...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Kase — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Kase tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Kase core, use the plugin route instead:

- [Plugins](/docs/user-guide/features/plugins)
- [Build a Kase Plugin](/docs/guides/build-a-kase-plugin)
...
If your tool requires an API key, add it to `kase_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `kase_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `kase chat -q "Use the weather tool for London"`
