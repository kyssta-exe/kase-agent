...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Kyssta/kase/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    hermes_meta = (fm.get("metadata") or {}).get("kase") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `kase skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Kase"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Kase ships with a large built-in skill library copied into `~/.kase/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Kase also syncs bundled skills on `kase update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.kase/skills/` tree, it is still shipped with Kase; restore it with `kase skills reset <name> --restore`.",
        "",
