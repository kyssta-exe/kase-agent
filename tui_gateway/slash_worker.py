"""Persistent slash-command worker — one KaseCLI per TUI session.
...
def _run(cli: KaseCLI, command: str) -> str:
...
def main():
    ...
    os.environ["KASE_SESSION_KEY"] = args.session_key
    os.environ["KASE_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = KaseCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
