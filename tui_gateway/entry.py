...
# kase modules.  kase_cli sets HERMES_PYTHON_SRC_ROOT before spawning this
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `KASE_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("KASE_TUI_SIDECAR_URL")
...
# tested; override via ``KASE_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("KASE_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``KASE_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``kase --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
