"""
Kase MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Kase-specific extra)

Usage:
    kase mcp serve
    kase mcp serve --verbose
...
logger = logging.getLogger("kase.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using KASE_HOME."""
    try:
        from kase_constants import get_kase_home
        return get_kase_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("KASE_HOME", Path.home() / ".kase")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from kase_constants import get_kase_home
        directory_file = get_kase_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("KASE_HOME", Path.home() / ".kase")
...
class EventBridge:
    ...
    This is the Kase equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from kase_constants import get_kase_home
            db_file = get_kase_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("KASE_HOME", Path.home() / ".kase")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Kase MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "kase",
        instructions=(
            "Kase messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Kase MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
