...
React + Ink terminal UI for Kase. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
- Input history is stored in `~/.kase/.hermes_history` or under `KASE_HOME`.
...
```text
ui-tui/
  packages/kase-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      kase-ink.d.ts    type declarations for @kase/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent KaseCLI subprocess for slash commands
```
