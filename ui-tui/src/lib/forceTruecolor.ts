...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.KASE_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Kase on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via KASE_TUI_TRUECOLOR=1.
