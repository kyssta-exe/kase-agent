import type { RunExternalProcess } from '@kase/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`kase ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching kase: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`kase ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
