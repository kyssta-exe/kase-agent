#!/usr/bin/env python3
"""
Kase CLI - Interactive Terminal Interface

A beautiful command-line interface for the Kase, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills kase-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: kase_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See kase_bootstrap.py for full rationale.
try:
    import kase_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when kase_bootstrap isn't registered in the venv
    # yet — happens during partial ``kase update`` where git-reset landed
...
os.environ["KASE_QUIET"] = "1"  # Our own modules
...
try:
    from kase_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from kase_cli.banner import _format_context_length, format_banner_version_label
...
from kase_cli.browser_connect import (
...
_kase_home = get_kase_home()
_project_env = Path(__file__).parent / '.env'
load_kase_dotenv(hermes_home=_kase_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.kase/.
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from kase_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.kase/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If KASE_IGNORE_USER_CONFIG=1 is set (via ``kase chat --ignore-user-config``),
    the user config at ``~/.kase/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({KASE_HOME}/config.yaml)
    user_config_path = _kase_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("KASE_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Kase, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Kase - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and KaseCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `kase model`).
    ...
    from kase_cli.config import _expand_env_vars
    defaults = _expand_env_vars(defaults)

    # Apply terminal config to environment variables (so terminal_tool picks them up)
    terminal_config = defaults.get("terminal", {})
    
    # Normalize config key: the new config system (kase_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && kase` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _KASE_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_KASE_GATEWAY") == "1"
...
# Initialize centralized logging early — agent.log + errors.log in ~/.kase/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from kase_logging import setup_logging
...
try:
    from kase_cli.config import print_config_warnings
...
try:
    from kase_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``kase --help`` paths inside the chat command flow).  See
...
from kase_cli.banner import build_welcome_banner
from kase_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from kase_cli.callbacks import prompt_for_secret
...
def _run_cleanup():
    ...
    try:
        from kase_cli.plugins import invoke_hook as _invoke_hook
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run kase -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"kase-{short_id}"
    branch_name = f"kase/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`kase_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from kase_cli.config import load_config as _load_full_config
        from kase_constants import get_kase_home as _get_kase_home
        _kase_home_maint = _get_kase_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_kase_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`kase_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``kase/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("kase-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``kase/kase-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``kase -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("kase/kase-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. HERMES_LIGHT / KASE_TUI_LIGHT env (true/false) — explicit override
#   2. KASE_TUI_THEME=light|dark — explicit theme
#   3. KASE_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Kase assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("HERMES_LIGHT", "KASE_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("KASE_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("KASE_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from kase_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Kase output it's common to display cron schedules verbatim.
    #
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``KaseCLI._busy_command()``.
        """
        yield self

# ASCII Art - HERMES-AGENT logo (full width, single line - requires ~95 char terminal)
KASE_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - Kase Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "Kase") if _skin else "Kase"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Kyssta[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from kase_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.kase/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.kase/config.yaml on first use)
...
class KaseCLI:
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from kase_cli.runtime_provider import _auto_detect_local_model
        ...
        # by `kase chat --ignore-rules` in kase_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("KASE_IGNORE_RULES") == "1"
        ...
        # it's shared across all Kase processes for this KASE_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.kase/checkpoints/.  Opt-in via
        ...
        self._history_file = _kase_home / ".kase_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from kase_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            yolo_active = bool(os.getenv("KASE_YOLO_MODE"))
            if width < 52:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Kase'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from kase_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from kase_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from kase_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from kase_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Kase")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - KaseCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from kase_cli.runtime_provider import (
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: kase setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: kase setup")
            return False
        ...
        # `kase chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `kase auth add openai-codex`
        # without `kase model`), fall back to the provider's first catalog
        # model so the API call doesn't fail with "model must be non-empty".
        if not self.model and resolved_provider:
            try:
                from kase_cli.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from kase_cli.models import resolve_fast_mode_overrides
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (kase sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``kase doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``kase doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   Kase needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        # Warn if the configured model is a Nous Kase LLM (not agentic)
        from kase_cli.model_switch import is_kyssta_kase_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_kyssta_kase_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Kyssta Kase 3 & 4 models are NOT agentic and are not "
                "designed for use with Kase.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Kase: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Kase: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.kase/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from kase_cli.clipboard import save_clipboard_image

        img_dir = get_kase_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: kase --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Kase config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_kase_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from kase_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run kase chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'kase setup' to configure[/]")
        except Exception:
    ...
    def _show_session_status(self):
        ...
        # 2.1.114's /recap.
        try:
            from kase_cli.session_recap import build_recap
            recap = build_recap(
    ...
    def _fast_command_available(self) -> bool:
        try:
            from kase_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from kase_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from kase_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Kase!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in kase_cli/colors.py
        ...
        from kase_cli.tools_config import _get_platform_tools
    ...
    def _handle_profile_command(self):
        ...
        from kase_constants import display_kase_home
        from kase_cli.profiles import get_active_profile_name

        display = display_kase_home()
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from kase_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("KASE_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        """
        from kase_state import format_session_db_unavailable
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `kase gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `kase sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from kase_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `kase sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.kase/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``kase --resume <id>``
        ...
        saved_dir = get_kase_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"kase_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: kase --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from kase_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from kase_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `kase model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from kase_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from kase_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from kase_cli.model_switch import switch_model, parse_model_flags
        from kase_cli.providers import get_label
        ...
        # clobber disk config.
        from kase_cli.inventory import build_models_payload, load_picker_context
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from kase_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to kase_cli.curator so the CLI and the `kase curator`
        ...
        try:
            from kase_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from kase_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to kase_cli.skills_hub."""
        from kase_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_kase_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_kase_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in kase_cli/commands.py instead of touching every dispatch site.
        from kase_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from kase_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from kase_cli.skin_engine import get_active_skin
            ...
            from kase_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from kase_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_kase_home()}/plugins/ to get started.")
                else:
            ...
            payload = parts[1].strip() if len(parts) > 1 else ""
            if not payload:
                _cprint("  Usage: /btw <message>  (inject aside during active task)")
            elif self._agent_running and self.agent is not None:
                try:
                    from agent.caveman_mode import get_instruction
                    enriched = f"[BTW note from user] {payload}"
                    self.agent.steer(enriched)
                    _cprint(f"  \u23e9 BTW noted — arrives after next tool call")
                except Exception as exc:
                    _cprint(f"  BTW failed: {exc}")
            else:
                self._pending_input.put(payload)
                _cprint(f"  \u23e9 No active task; queued as next turn")
        elif canonical == "caveman":
            parts = cmd_original.split(None, 1)
            level = parts[1].strip().lower() if len(parts) > 1 else ""
            from agent.caveman_mode import toggle, set_level, get_level, LEVELS
            if level in LEVELS:
                result = set_level(level)
            elif level == "" or level == "toggle":
                result = toggle()
            elif level in ("0", "false", "no", "off"):
                result = set_level("off")
            else:
                _cprint(f"  Usage: /caveman [{'|'.join(LEVELS)}]")
                _cprint(f"  Current: {get_level()}")
                return True
            if result in LEVELS:
                _cprint(f"  \u26a1 Caveman mode: {result}")
                if result == "off":
                    _cprint(f"  Back to normal verbosity.")
                else:
                    _cprint(f"  Replies will be shorter and more direct.")
            else:
                _cprint(f"  {result}")
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from kase_cli.commands import COMMANDS
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``kase bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: kase bundles create "
        ...
        _cprint(
            ...
            f"Manage with `kase bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Kase-managed isolated debug "
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from kase_cli.goals import GoalManager
            from kase_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Kase keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from kase_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_kase_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from kase_cli.config import load_config
        from kase_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("KASE_YOLO_MODE"))
        if current:
            os.environ.pop("KASE_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Kase is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Kase is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Kase is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from kase_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from kase_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Kase to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``kase update`` so the user sees update output directly and gets
        ...
        when cancelled.
        """
        from kase_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update Kase')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Kase"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Kase",
            detail="This will exit the current session and run `kase update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by kase_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from kase_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.kase/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_kase_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "kase_voice"), exist_ok=True)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _kase_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  kase --resume {self.session_id}")
            if session_title:
                print(f"  kase -c \"{session_title}\"")
            print()
            ...
            try:
                from kase_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from kase_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from kase_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Kase! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Kase! Type your message or /help for commands."
        ...
        # after an OpenClaw→Kase migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from kase_cli.config import get_config_path as _get_cfg_path_resid
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_kase_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Kase binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from kase_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Kase")
        ...
        # Dynamic prompt: shows Kase symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Kase needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Kase needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Kase needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = KaseCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the kase process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: kase setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                if getattr(self, '_delete_session_on_exit', False):
                    try:
                        from kase_constants import get_kase_home as _ghh
...
def main(
    ...
    """
    Kase CLI - Interactive AI Assistant
    ...
    os.environ["KASE_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Kase Gateway (messaging platforms)...")
    ...
    # Default to kase-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside KaseCLI.run(), but `-q` runs
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Kase" box, no tool-gen
            ...
            # Single-query mode (`kase chat -q "…"`): skip the welcome
