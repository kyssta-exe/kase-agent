...
KASE_HOME_DIR="${KASE_HOME:-$HOME/.kase}"
KASE_CFG="${KASE_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Kase config has twozero_td MCP entry ──
if [[ ! -f "$KASE_CFG" ]]; then
    echo -e " ${FAIL} Kase config not found at ${KASE_CFG}"
    manual_steps+=("Create ${KASE_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$KASE_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Kase config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Kase config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${KASE_CFG} manually"); }
    manual_steps+=("Restart Kase session to pick up config change")
fi
