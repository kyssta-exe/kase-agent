...
## 5. GODMODE FAST — Kase 4 405B (Zero Refusal)

**Model:** `kyssta/kase-4-405b`
**Strategy:** Kase models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
Pick a template above and set it in `~/.kase/config.yaml`:
