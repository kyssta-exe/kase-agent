"""CLI entry point for the kase ACP adapter.

Loads environment variables from ``~/.kase/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from KASE_HOME (default ``~/.kase``)."""
    from kase_cli.env_loader import load_kase_dotenv

    hermes_home = get_kase_home()
    loaded = load_kase_dotenv(hermes_home=hermes_home)
    if loaded:
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="kase-acp",
        description="Run Kase as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Kase version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import KaseACPAgent  # noqa: F401

    print("Kase ACP check OK")


def _run_setup() -> None:
    from kase_cli.main import main as hermes_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "kase", "model"]
        hermes_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``kase postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting kase ACP adapter")
    ...
    import acp
    from .server import KaseACPAgent
