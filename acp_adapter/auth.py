"""ACP auth helpers — detect and advertise Kase authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "kase-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Kase runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Kase can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Kase.
    ...
    not have Kase provider credentials configured yet, so Kase always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Kase using the currently configured "
