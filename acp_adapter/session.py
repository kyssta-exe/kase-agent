"""ACP session manager — maps ACP sessions to Kase AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.kase/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Kase itself is running in WSL.

    Windows ACP clients can launch ``kase acp`` inside WSL while still sending
    ...
    """
    from kase_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Kase from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["kase-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Kase agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Kase AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Kase runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.kase/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``KASE_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_kase_home``).
        """
