"""ACP agent server — exposes Kase via the Agent Client Protocol."""
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Kase running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Kase cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Kase/OpenAI-compatible user content payload."""
...
class KaseACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Kase AIAgent."""
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Kase maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from kase_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from kase_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Kase estimates ``used`` from the same
    ...
    async def _send_session_info_update(self, session_id: str) -> None:
        """Send ACP native session metadata after Kase changes it."""
        if not self._conn:
            return
        try:
        ...
        # kase_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["kase-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="kase", version=HERMES_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Kase' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Kase setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Kase
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Kase on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Kase' local kawaii waiting/status
        ...
        # not the executor's. Also set KASE_INTERACTIVE so approval.py
        ...
        def _run_agent() -> dict:
            ...
            # Bind KASE_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("KASE_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("KASE_SESSION_ID")
            os.environ["KASE_SESSION_ID"] = session_id
            try:
                ...
                # Restore KASE_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("KASE_INTERACTIVE", None)
                else:
                    os.environ["KASE_INTERACTIVE"] = previous_interactive
                # Restore KASE_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("KASE_SESSION_ID", None)
                else:
                    os.environ["KASE_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (KASE_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Kase v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Kase has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
