#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Kase Skills Hub.
...
Used by kase_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Kyssta) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hermes_meta = meta_block.get("kase", {})
                if isinstance(hermes_meta, dict):
                    tags = hermes_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Kase centralized index source
# ---------------------------------------------------------------------------

HERMES_INDEX_URL = "https://kase.kyssta.com/docs/api/skills-index.json"
HERMES_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "kase-index.json"
...
def _load_hermes_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("Kase index fetch returned %d", resp.status_code)
...
class KaseIndexSource(SkillSource):
    """Skill source backed by the centralized Kase Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "kase-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "kase-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        KaseIndexSource(auth=auth), # Centralized index (search + resolved install paths)
