...
Copies bundled skills from the repo's skills/ directory into ~/.kase/skills/
...
The manifest lives at ~/.kase/skills/.bundled_manifest.
"""
...
from kase_constants import get_bundled_skills_dir, get_kase_home
...
def _get_bundled_dir() -> Path:
    ...
    Checks KASE_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.kase/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.kase/skills/ using the manifest.
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `kase skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `kase skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `kase update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.kase/skills/ ...")
