"""Central registry for all kase tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``kase tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``kase tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``kase tools enable foo``)
