...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.kase/config.yaml``. Kase
...
Configuration is loaded from ~/.kase/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``kase_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from kase_cli.config import get_env_value as _get_env_value
    except ImportError:
        return os.getenv(name, default)
    value = _get_env_value(name)
    return default if value is None else value
from tools.managed_tool_gateway import resolve_managed_tool_gateway
from tools.tool_backend_helpers import managed_kyssta_tools_enabled, prefers_gateway, resolve_openai_audio_api_key
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.kase/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.kase/config.yaml.
    ...
    try:
        ...
        logger.debug("kase_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into Kase without any Python code
...
#           output_format: wav
#
# Kase writes the input text to a temp UTF-8 file, runs the command with
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `kase model` or set XAI_API_KEY.")
    ...
    # output_format when Kase actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Kase caches Piper voice models.
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.kase/config.yaml (tts: section).
    ...
    platform = get_session_env("KASE_SESSION_PLATFORM", "").lower()
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run kase setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'kase setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'kase tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_kase_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
