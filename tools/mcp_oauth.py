...
This module provides the glue:
    - ``KaseTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Kase"
"""
...
from kase_constants import secure_parent_dir
...
def _get_token_dir() -> Path:
    ...
    Uses KASE_HOME so each profile gets its own OAuth tokens.
    Layout: ``KASE_HOME/mcp-tokens/``
    """
    try:
        from kase_constants import get_kase_home
        base = Path(get_kase_home())
    except ImportError:
        base = Path(os.environ.get("KASE_HOME", str(Path.home() / ".kase")))
...
# ---------------------------------------------------------------------------
# KaseTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class KaseTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        KASE_HOME/mcp-tokens/<server_name>.json         -- tokens
        KASE_HOME/mcp-tokens/<server_name>.client.json   -- client info
        KASE_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Kase records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Kase.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  Then open the URL above. See: https://kase.kyssta.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = KaseTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Kase")
    scope = cfg.get("scope")
