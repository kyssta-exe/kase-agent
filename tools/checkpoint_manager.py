...
-----------------------------------------------------------------------------
    ...
    ~/.kase/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/kase/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_kase_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/kase"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Kase convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _git_env(
    ...
    The shared store is internal Kase infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``kase checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `kase checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "kase@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Kase Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``KASE_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at KASE_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "KASE_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `kase checkpoints` CLI
# ---------------------------------------------------------------------------
