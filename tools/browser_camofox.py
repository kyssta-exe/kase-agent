...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.kase/.env``.
"""
...
from kase_cli.config import cfg_get, load_config
...
def _managed_persistence_enabled() -> bool:
    """Return whether Kase-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    so Kase operates in the same browser profile instead of creating a
...
def _adopt_existing_tab_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether Kase should recover an existing Camofox tab ID."""
...
def _adopt_existing_tab(session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Some integrations own the visible Camofox tab outside Kase. Gateway
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Kase profile so the Camofox server can map it
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_kase_home() / "browser_screenshots"
