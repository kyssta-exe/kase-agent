...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``kase tools`` repaint, tool-registration scans,
    ...
    2. ``~/.kase/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_kase_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.kase/.env`` first, then ``os.environ``.

    Wraps :func:`kase_cli.config.get_env_value` so tests can patch
...
def kase_xai_user_agent() -> str:
    """Return a stable Kase-specific User-Agent for xAI HTTP calls."""
    try:
        from kase_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Kase-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Kase-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``kase_cli.config.get_env_value`` so keys
    stored in ``~/.kase/.env`` (the standard Kase location) are honored —
