...
selectable via ``kase tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `kase tools` → Image Generation."
            ) from exc
        raise
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_kyssta_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`kase setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `kase tools` "
        "→ Image Generation (run `kase plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping kase with a FAL key configured
