...
Lets the LLM write a Python script that calls Kase tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `kase_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `kase_tools.py` with file-based RPC stubs
...
# available on every platform Kase itself runs on.
...
def generate_hermes_tools_module(enabled_tools: List[str],
    ...
    """
    Build the source code for the kase_tools.py stub module.
...
"""Auto-generated Kase tools RPC stubs."""
...
"""Auto-generated Kase tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("HERMES_RPC_DIR") or os.path.join(tempfile.gettempdir(), "kase_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated kase_tools.py module are shipped to
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/kase_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with kase_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="kase_sandbox_")
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"kase_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        ...
        with open(os.path.join(tmpdir, "kase_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        # Ensure the kase root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from kase_tools import ...`` resolves even
        ...
        # HERMES_TIMEZONE is an internal Kase setting and must not leak
        ...
        # {KASE_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.kase/.env')).
...
def _load_config() -> dict:
    ...
    ``kase --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (kase's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``kase tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and kase's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.kase/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Kase tools programmatically. "
        ...
        f"Available via `from kase_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from kase_tools import {import_str}` "
