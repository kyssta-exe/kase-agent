...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("KASE_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("KASE_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("KASE_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set KASE_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind KASE_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds KASE_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("KASE_CRON_SESSION"):
        return False
    if env_var_enabled("KASE_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $KASE_HOME.
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart kase gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "kase update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart kase-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart kase-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(kase|gateway|cli\.py)\b', "kill kase/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill kase` but not
    # `kill -9 $(pgrep -f kase)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("KASE_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("KASE_INTERACTIVE")
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("KASE_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}
