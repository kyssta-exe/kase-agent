...
# plugins drop in under ``~/.kase/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.kase/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Kase silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Kase CLI where sys.stdout and sys.stderr both report
...
def _emergency_cleanup_all_sessions():
    ...
    crashed kase processes — this way every clean kase exit sweeps
    ...
    # Sweep orphans from other crashed kase processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live kase processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current kase PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live kase process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning kase process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced kase PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent kase instances won't reap each
         other's daemons.
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live kase process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Kase-managed
    ...
    # Build an extended search PATH including Kase-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                hermes_nm = str(get_kase_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=hermes_nm)
            if not recheck:
                hermes_node_bin = str(get_kase_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=hermes_node_bin)
            if not recheck:
                hermes_node_root = str(get_kase_home() / "node")
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this kase PID as the session owner (cross-process safe
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/kyssta/kase:latest")
                else:
