...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Kase exit
...
# display_kase_home imported lazily at call site (stale-module safety during kase update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of kase directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "kase-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("KASE_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("KASE_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (KASE_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (KASE_INTERACTIVE=1):
    ...
    # interactive Kase password prompt or the sudo -S password-pipe path.
    ...
    # silently without Kase prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and env_var_enabled("KASE_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Kase can track lifecycle and output.
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.kase/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Modal credentials/config were found. Log in with `kase model` or "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Kase can track the process, then run "
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("KASE_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("KASE_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("KASE_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("KASE_SESSION_USER_ID", "")
                        _gw_user_name = _gse("KASE_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("KASE_SESSION_MESSAGE_ID", "")
            ...
            try:
                from kase_cli.plugins import invoke_hook
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        ...
                        "a paid Nous subscription is required for the Tool Gateway and no direct "
                        "Modal credentials/config were found. Log in with `kase model` "
