...
def _load_kase_env_vars() -> dict[str, str]:
    """Load ~/.kase/.env values without failing Docker command execution."""
    try:
        from kase_cli.config import load_env
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'kase'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.kase/sandboxes/). Non-persistent
        ...
        container_name = f"kase-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Kase secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _KASE_PROVIDER_ENV_BLOCKLIST)
        hermes_env = _load_kase_env_vars() if forward_keys else {}
