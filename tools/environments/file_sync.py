...
def iter_sync_files(container_base: str = "/root/.kase") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.kase to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.kase/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="kase-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.kase/skills/a.md`` →
        ``~/.kase/skills/a.md``, a new remote file at
        ``/root/.kase/skills/b.md`` maps to ``~/.kase/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
