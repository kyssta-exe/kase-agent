...
from tools.tool_backend_helpers import managed_kyssta_tools_enabled, resolve_openai_audio_api_key
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="kase-local-stt-") as output_dir:
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `kase model` or set XAI_API_KEY",
        }
