...
# All skills live in ~/.kase/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
KASE_HOME = get_kase_home()
SKILLS_DIR = KASE_HOME / "skills"
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from KASE_HOME/.env."""
    env_path = get_kase_home() / ".env"
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("KASE_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_kase_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.kase/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``KASE_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    3. ``KASE_SESSION_PLATFORM`` from gateway session context
    """
    try:
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.kase/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``kase skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_kase_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from kase_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: kase plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.kase/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `kase skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.kase.* first (agentskills.io convention), fall back to top-level
        hermes_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("kase", {}) or {}
