...
it is automatically downloaded from GitHub releases to $KASE_HOME/bin/tirith.
...
def _get_kase_home() -> str:
    """Return the Kase home directory, respecting KASE_HOME env var."""
    return str(get_kase_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_kase_home(), ".tirith-install-failed")
...
def _hermes_bin_dir() -> str:
    """Return $KASE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_kase_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $KASE_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $KASE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $KASE_HOME/bin/tirith
