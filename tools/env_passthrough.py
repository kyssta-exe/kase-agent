...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Kase-managed provider credential (API key,
    token, or similar) per ``_KASE_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Kase API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    in the blocklist and remain legitimately registerable — skills that
    wrap third-party APIs still work.
    """
    try:
        from tools.environments.local import _KASE_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _KASE_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Kase-managed provider credentials (from
    ``_KASE_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Kase-managed
    ...
    Non-Kase third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Kase provider "
                "credential %r (blocked by _KASE_PROVIDER_ENV_BLOCKLIST). "
