...
`mcp_tool.py`, and `kase_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# KaseMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------
...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class KaseMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`KaseTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, KaseTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import KaseTokenStorage
                        if isinstance(storage, KaseTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import KaseTokenStorage
            if not isinstance(storage, KaseTokenStorage):
                return
    ...
    return KaseMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_KASE_PROVIDER_CLS: Optional[type] = _make_hermes_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`KaseMCPOAuthProvider` directly using the helpers
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``kase mcp remove <name>`` and (indirectly) by
        ``kase mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
