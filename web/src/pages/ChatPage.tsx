/**
 * ChatPage — embeds `kase --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (kase_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__KASE_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `kase dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const token = window.__KASE_SESSION_TOKEN__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Kase TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[kase-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into Kase. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="kase-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
