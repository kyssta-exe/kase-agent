...
#   2. ~/.kase/node/ from a prior Kase-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.kase/node/ (always works, zero shell rc edits)
#
# Usage:
...
_nb_install_bundled_node() {
    ...
    _nb_log "Extracting to $KASE_HOME/node/..."
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $KASE_HOME/node/"
    return 0
}
...
ensure_node() {
    ...
    if [ -x "$KASE_HOME/node/bin/node" ]; then
        export PATH="$KASE_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Kase-managed)"
            HERMES_NODE_AVAILABLE=true
            return 0
        fi
    fi
