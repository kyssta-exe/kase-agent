#!/usr/bin/env bash
# Canonical test runner for kase. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.kase/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.kase/kase/venv"; do
...
if [ -f "$HOME/.kase/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.kase"
