#!/usr/bin/env python3
"""Build the Kase Model Catalog — a centralized JSON manifest of curated models.
...
Kase CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Kase release.
...
# Ensure KASE_HOME is set for imports that touch it at module level.
os.environ.setdefault("KASE_HOME", os.path.join(os.path.expanduser("~"), ".kase"))

from kase_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS  # noqa: E402
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "kase repo",
            "docs": "https://kase.kyssta.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
            ...
            "nous": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        "(partition_kyssta_models_by_tier), not this manifest."
                    ),
                },
                "models": [
                    {"id": mid}
