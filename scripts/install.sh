...
# Python-driven tool session (e.g. Kase terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:Kyssta/kase.git"
REPO_URL_HTTPS="https://github.com/Kyssta/kase.git"
KASE_HOME="${KASE_HOME:-$HOME/.kase}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${KASE_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$KASE_INSTALL_DIR"
...
#   code at /usr/local/lib/kase, command at /usr/local/bin/kase,
#   data still at /root/.kase (KASE_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.kase/kase"
            echo "                   default (root, Linux): /usr/local/lib/kase"
            echo "  --kase-home PATH  Data directory (default: ~/.kase, or \$KASE_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Kase installs the code under"
            echo "  /usr/local/lib/kase and links the command into"
            echo "  /usr/local/bin/kase (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$KASE_HOME"
            echo "  (default /root/.kase).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$KASE_HOME/kase are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs kase setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Kase Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Kyssta.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where the `kase` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $KASE_HOME/kase
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $KASE_HOME/kase
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/kase
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $KASE_HOME/kase — then preserve it)
#
# Always no-op when the user set --dir or $KASE_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in KASE_HOME.
    if is_termux; then
        INSTALL_DIR="$KASE_HOME/kase"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$KASE_HOME/kase/.git" ]; then
            INSTALL_DIR="$KASE_HOME/kase"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/kase)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/kase"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/kase"
        log_info "  Data:    $KASE_HOME (unchanged)"
        return 0
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/Kyssta/kase/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/kase-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/kase-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$KASE_HOME/node/bin/node" ]; then
        export PATH="$KASE_HOME/node/bin:$PATH"
        local found_ver=$("$KASE_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Kase-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.kase/node/..."
    ...
    # Place into ~/.kase/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$KASE_HOME/node"
    mkdir -p "$KASE_HOME"
    mv "$extracted_dir" "$KASE_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$KASE_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$KASE_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$KASE_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$KASE_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$KASE_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.kase/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Kase install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Kase itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="kase-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Kase behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up kase command..."

    if [ "$USE_VENV" = true ]; then
        KASE_BIN="$INSTALL_DIR/venv/bin/kase"
    else
        KASE_BIN="$(which kase 2>/dev/null || echo "")"
        if [ -z "$KASE_BIN" ]; then
            log_warn "kase not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$KASE_BIN" ]; then
        log_warn "kase entry point not found at $KASE_BIN"
    ...
    # Create a user-facing shim for the kase command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $KASE_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$KASE_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/kase"
    cat > "$command_link_dir/kase" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$KASE_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/kase"
    log_success "Installed kase launcher → $command_link_display_dir/kase"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "kase command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v kase' \
                >/dev/null 2>&1; then
        ...
        log_info "kase not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Kase — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Kase — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Kase — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so kase works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.kase directory structure (config at top level, code in subdir)
    mkdir -p "$KASE_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.kase/.env (top level, easy to find)
    if [ ! -f "$KASE_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$KASE_HOME/.env"
            log_success "Created ~/.kase/.env from template"
        else
            touch "$KASE_HOME/.env"
            log_success "Created ~/.kase/.env"
        fi
    else
        log_info "~/.kase/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$KASE_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.kase/config.yaml (top level, easy to find)
    if [ ! -f "$KASE_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$KASE_HOME/config.yaml"
            log_success "Created ~/.kase/config.yaml from template"
        fi
    else
        log_info "~/.kase/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$KASE_HOME/SOUL.md" ]; then
        cat > "$KASE_HOME/SOUL.md" << 'SOUL_EOF'
# Kase Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Kase communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.kase/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.kase/"

    # Seed bundled skills into ~/.kase/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.kase/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.kase/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$KASE_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$KASE_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.kase/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Kase browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Kase will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (kase --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'kase setup' after install."
        return 0
    fi
    ...
    # Run kase setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m kase_cli.main setup < /dev/tty
    else
        python -m kase_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$KASE_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Kase to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$KASE_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'kase whatsapp' to pair via QR code..."
            echo ""
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'kase whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'kase gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: kase gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: kase gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $HERMES_CMD gateway > "$KASE_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.kase/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: kase gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: kase gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $KASE_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $KASE_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $KASE_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}kase${NC}              Start chatting"
    echo -e "   ${GREEN}kase setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}kase config${NC}       View/edit configuration"
    echo -e "   ${GREEN}kase config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}kase gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}kase update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'kase' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'kase' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'kase' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$KASE_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$KASE_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$KASE_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$KASE_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$KASE_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Kase for pip install"
    ...
    HERMES_CMD="$(command -v kase 2>/dev/null || echo "")"
    if [ -n "$HERMES_CMD" ]; then
        log_info "Running kase setup..."
        "$HERMES_CMD" setup
    else
        log_warn "kase command not found on PATH"
        log_info "Try: python -m kase_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$KASE_HOME/.install_method"
}
