...
# Bootstrap Open WebUI against Kase's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.kase/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-kase.sh
...
KASE_ENV_FILE="${KASE_ENV_FILE:-$HOME/.kase/.env}"
...
HERMES_API_MODEL_NAME="${HERMES_API_MODEL_NAME:-Kase}"
KASE_API_BASE_URL="http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-kase.sh"
LOG_DIR="$HOME/.kase/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$KASE_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$KASE_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.kase.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.kase</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.kase"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.kase"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-kase.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Kase
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-kase.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.kase/logs/openwebui.log
StandardError=append:%h/.kase/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-kase.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$KASE_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Kase API server is configured...'
  upsert_env API_SERVER_ENABLED true "$KASE_ENV_FILE"
  upsert_env API_SERVER_HOST "$HERMES_API_HOST" "$KASE_ENV_FILE"
  upsert_env API_SERVER_PORT "$HERMES_API_PORT" "$KASE_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$HERMES_API_MODEL_NAME" "$KASE_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$KASE_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Kase gateway so API server settings take effect...'
  kase gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    log 'Kase API server did not answer on the first check. Trying to start gateway in the background...'
    nohup kase gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  ...
  log "Kase API endpoint: ${KASE_API_BASE_URL}"
