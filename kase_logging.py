"""Centralized logging setup for Kase.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.kase/logs/`` (profile-aware via ``get_kase_home()``).
...
from kase_constants import get_config_path, get_kase_home
...
# Used by _ComponentFilter and exposed for ``kase logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the Kase logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the Kase home directory.  Falls back to
        ``get_kase_home()`` (profile-aware).
    log_level
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the kase group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from kase_cli.config import is_managed
