...
Used by ``kase profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``kase_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Kase kanban board.
...
Rules:
  ...
  - Never write "Kase profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_kase_home())
        else:
