...
Add, remove, or reorder entries here — both `kase setup` and
`kase` provider-selection will pick up the change automatically.
"""
...
from kase_cli import __version__ as _HERMES_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_HERMES_USER_AGENT = f"kase-cli/{_HERMES_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `kase model`
    flow without maintaining a separate static list.
    """
    from kase_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $KASE_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $KASE_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Kase refreshes the cache from
...
def fetch_kyssta_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    ...
    """
    base = (portal_base_url or "https://portal.kyssta.com").rstrip("/")
...
def is_kyssta_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_kyssta_models_by_tier(
    model_ids: list[str],
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Kase that doesn't ship that model in its hardcoded curated
    ...
    * ``pricing`` gets a synthetic ``{"prompt": "0", "completion": "0"}``
      ...
      map, so :func:`partition_kyssta_models_by_tier` keeps it.
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Kase that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_kyssta_models_by_tier` keeps free models selectable;
...
def check_kyssta_free_tier() -> bool:
    ...
    try:
        from kase_cli.auth import get_provider_auth_state, resolve_kyssta_runtime_credentials

        # Ensure we have a fresh token (triggers refresh if needed)
        resolve_kyssta_runtime_credentials(min_key_ttl_seconds=60)
        ...
        account_info = fetch_kyssta_account_tier(access_token, portal_url)
        result = is_kyssta_free_tier(account_info)
...
def _resolve_kyssta_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.kyssta.com"
...
def get_nous_recommended_aux_model(
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_kyssta_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_kyssta_portal_url()
...
# this list:  kase model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `kase model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `kase model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nous",           "Nous Portal",              "Nous Portal (Kyssta subscription)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``kase model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``kase auth add openai-codex`` without
    ``kase model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    kase is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from kase_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — kase
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from kase_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return kase-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; kase's picker expects
...
_DEFAULT_KYSSTA_INFERENCE_BASE = "https://inference-api.kyssta.com"
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from kase_cli.auth import resolve_kyssta_runtime_credentials
        creds = resolve_kyssta_runtime_credentials()
        if creds:
            return (creds.get("api_key", ""), creds.get("base_url", ""))
    except Exception:
        pass
    return ("", _DEFAULT_KYSSTA_INFERENCE_BASE)
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.kyssta.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``kase model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from kase_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from kase_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Kase' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``kase_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Kase should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``kase auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.kase/.env``.
    ...
    """
    try:
        from kase_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from kase_cli.auth import read_credential_pool
        from kase_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Kase release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Kase release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from kase_cli.auth import fetch_nous_models, resolve_kyssta_runtime_credentials
            creds = resolve_kyssta_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from kase_cli.copilot_auth import copilot_request_headers
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Kase' default Claude IDs elsewhere use
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_kase_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Kase will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Kase cannot verify the model name."
