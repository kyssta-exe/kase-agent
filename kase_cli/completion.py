"""Shell completion script generation for kase CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Kase bash completion
# Add to ~/.bashrc:
#   eval "$(kase completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.kase/profiles"
...
complete -F _hermes_completion kase
"""
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.kase/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.kase/profiles 2>/dev/null)}}")
    fi
...
{top_cmds_str}
            ...
            _describe 'kase command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Kase fish completion",
        "# Add to your config:",
        "#   kase completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.kase/profiles",
        "        ls $HOME/.kase/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c kase -f -s p -l profile"
