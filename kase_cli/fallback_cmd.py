"""
kase fallback — manage the fallback provider chain.
...
Subcommands:
  kase fallback [list]   Show the current fallback chain (default when no subcommand)
  kase fallback add      Pick provider + model via the same picker as `kase model`,
                           then append the selection to the chain
  kase fallback remove   Pick an entry to delete from the chain
  kase fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.kase/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from kase_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `kase model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  kase fallback add")
        print()
        return
    ...
    print("  Docs: https://kase.kyssta.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `kase model`, then append the selection to the chain."""
    from kase_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `kase model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `kase fallback list` to view, or `kase fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``kase fallback [subcommand]``."""
