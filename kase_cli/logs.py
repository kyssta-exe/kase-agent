"""``kase logs`` — view and filter Kase log files.
...
Usage examples::
    ...
    kase logs                    # last 50 lines of agent.log
    kase logs -f                 # follow agent.log in real time
    kase logs errors             # last 50 lines of errors.log
    kase logs gateway -n 100    # last 100 lines of gateway.log
    kase logs --level WARNING    # only WARNING+ lines
    kase logs --session abc123   # filter by session ID substring
    kase logs --component tools  # only tool-related lines
    kase logs --since 1h         # lines from the last hour
    kase logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_kase_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Kase runs — try 'kase chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_kase_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_kase_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_kase_home()}/logs/")
        return

    print(f"Log files in {display_kase_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'kase chat' to generate logs)")
