"""
MCP Server Management CLI — ``kase mcp`` subcommand.

Implements ``kase mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.kase/config.yaml under the ``mcp_servers`` key.
"""
...
from kase_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_kase_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from kase_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # kase_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  kase mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  kase mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  kase mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_kase_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: kase mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_kase_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `kase mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  kase mcp add <name> --url <endpoint>')
        _info('  kase mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `kase mcp remove` + `kase mcp add` to reconfigure auth.")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'kase mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``kase mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("kase mcp serve                              Run as MCP server")
        _info("kase mcp add <name> --url <endpoint>        Add an MCP server")
        _info("kase mcp add <name> --command <cmd>         Add a stdio server")
        _info("kase mcp add <name> --preset <preset>       Add from a known preset")
        _info("kase mcp remove <name>                      Remove a server")
        _info("kase mcp list                               List servers")
        _info("kase mcp test <name>                        Test connection")
        _info("kase mcp configure <name>                   Toggle tools")
        _info("kase mcp login <name>                       Re-authenticate OAuth")
        print()
