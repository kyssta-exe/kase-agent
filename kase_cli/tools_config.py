"""
Unified tool configuration for Kase.

`kase tools` and `kase setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.kase/config.yaml under
...
from kase_cli.kyssta_subscription import (
    apply_nous_managed_defaults,
    get_kyssta_subscription_features,
)
from tools.tool_backend_helpers import fal_key_is_configured, managed_kyssta_tools_enabled
...
from kase_cli.cli_output import (  # noqa: E402 — late import block
...
# who want it opt in via `kase tools` → Video Generation, which walks
...
# `kase tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from kase_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `kase tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``kase tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from kase_cli.plugins import discover_plugins, get_plugin_toolsets
...
from kase_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Kase routes X searches through xAI's built-in x_search "
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``kase update`` and
      by ``kase computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `kase update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Kase process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_kase_home()}/kase && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in kase directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/kyssta/kase:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `kase update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.kase/config.yaml")
        ...
        # Run the full `kase auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.kase/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from kase_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: kase auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `kase auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: kase auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: kase auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `kase auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: kase auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: kase auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "kase-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[kase-cli, spotify]`` after enabling Spotify via ``kase
        ...
        # (e.g. "kase-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``kase tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `kase tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"kase-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("kase-")}
    ...
    # shipped as a bundled plugin but user must opt in via `kase tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `kase tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by kase tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (kase-cli, kase-telegram, etc.)
    ...
    # Opening `kase tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(ts_key: str, config: dict = None) -> bool:
    ...
    if ts_key in {"web", "image_gen", "tts", "browser"}:
        features = get_kyssta_subscription_features(config)
...
def _visible_providers(cat: dict, config: dict) -> list[dict]:
    ...
    for provider in cat.get("providers", []):
        if provider.get("managed_nous_feature") and not managed_kyssta_tools_enabled():
            continue
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `kase tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `kase tools` and `kase setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Kase Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://kase.kyssta.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            if managed_kyssta_tools_enabled():
    ...
    print()
    from kase_constants import display_kase_home
    print(color(f"  Tool configuration saved to {display_kase_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'kase' or gateway restart.", Colors.DIM))
    print()
