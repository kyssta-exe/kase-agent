"""Slash command definitions and autocomplete for the Kase CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Kase config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Kase works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Kase is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.kase/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Kase to the latest version", "Info"),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info"),

    # Communication
    CommandDef("btw", "Inject an aside/question during an active task without interrupting", "Session",
               args_hint="<message>", aliases=("bytheway", "ps", "note")),
    CommandDef("caveman", "Toggle token-efficient caveman mode (drop filler, shorter replies)", "Configuration",
               args_hint="[lite|normal|max|ultra|off]", aliases=("cave", "grunt")),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`kase_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/kase`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Kase
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``kase skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``kase skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``kase skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/kase <verb>`` subcommand.
    ...
    can still reach them via ``/kase <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/kase`` is always reserved as the first entry so the
    legacy ``/kase <subcommand>`` form keeps working for anything that
    ...
    # Reserve /kase as the catch-all top-level command.
    entries.append(("kase", "Talk to Kase or run a subcommand", "[subcommand] [args]"))
    seen.add("kase")
...
def slack_app_manifest(request_url: str = "https://kase.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /kase handler.

    Maps both canonical names and aliases so /kase bg do stuff works
    the same as /kase background do stuff.

    Plugin-registered slash commands are included so ``/kase <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from kase_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
