"""
Single source of truth for provider identity in Kase.
...
2. **Kase overlays** — transport type, auth patterns, aggregator flags,
...
# -- Kase overlay ----------------------------------------------------------
# Kase-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class KaseOverlay:
    """Kase-specific provider metadata layered on top of models.dev."""
...
KASE_OVERLAYS: Dict[str, KaseOverlay] = {
    "openrouter": KaseOverlay(
    ...
    "nous": KaseOverlay(
        ...
        base_url_override="https://inference-api.kyssta.com/v1",
    ),
    "openai-codex": KaseOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": KaseOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": KaseOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="HERMES_QWEN_BASE_URL",
    ),
    "google-gemini-cli": KaseOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": KaseOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": KaseOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": KaseOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": KaseOverlay(
    ...
    "minimax-oauth": KaseOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": KaseOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": KaseOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": KaseOverlay(
        transport="openai_chat",
    ...
    "opencode": KaseOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": KaseOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "kase", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Kase overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Kase overlay
    ...
    overlay = KASE_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + kase extra
    ...
    if overlay is not None:
        # Kase-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in KASE_OVERLAYS
    if provider == "bedrock":
