...
Usage:
    python -m kase_cli.main web          # Start on http://127.0.0.1:9119
    python -m kase_cli.main web --port 8080
"""
...
try:
    ...
    # running `kase dashboard` needs fastapi+uvicorn; lazy install keeps
...
app = FastAPI(title="Kase", version=__version__)
...
_SESSION_HEADER_NAME = "X-Kase-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``kase dashboard --tui``
...
# streamed to a per-action log file under ``~/.kase/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_kase_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "kase-update": "kase-update.log",
}
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``kase <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``kase_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "kase_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "KASE_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``kase gateway restart`` in the background."""
    try:
...
@app.post("/api/kase/update")
async def update_hermes():
    """Kick off ``kase update`` in the background."""
    try:
        proc = _spawn_hermes_action(["update"], "kase-update")
    except Exception as exc:
        _log.exception("Failed to spawn kase update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Kase supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in kase_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.kase/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``kase auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Kase resolves Anthropic creds in this order at runtime:
    1. ``~/.kase/.anthropic_oauth.json`` — Kase-managed PKCE flow
    ...
    try:
        ...
        _KASE_OAUTH_FILE = None  # type: ignore
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "kase_pkce",
            "source_label": f"Kase PKCE ({_KASE_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Kase even
    when they also have a separate Kase-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "kase auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "kase auth add nous",
        "docs_url": "https://portal.kyssta.com",
        "status_fn": None,  # dispatched via auth.get_kyssta_auth_status
    },
    {
        "id": "openai-codex",
        "name": "OpenAI Codex (ChatGPT)",
        "flow": "device_code",
        "cli_command": "kase auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "kase auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "kase auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from kase_cli import auth as hauth
        if provider_id == "nous":
            raw = hauth.get_kyssta_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "kyssta_portal",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("kase_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Kase-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _KASE_OAUTH_FILE
            if _KASE_OAUTH_FILE.exists():
                _KASE_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from kase_cli.auth import clear_provider_auth
...
#          → persists to ~/.kase/.anthropic_oauth.json AND credential pool
...
# Guarded so kase web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Kase file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``kase auth add anthropic``.
    """
    from agent.anthropic_adapter import _KASE_OAUTH_FILE
    payload = {
    ...
    _KASE_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _KASE_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from kase_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from kase_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_kase_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from kase_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from kase_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, KASE_HOME)."""
    from kase_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process KASE_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_kase_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "kase setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from kase_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from kase_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from kase_cli.tools_config import (
...
#
# The endpoint spawns the same ``kase --tui`` binary the CLI uses, behind
...
try:
    from kase_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``kase --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``KASE_TUI_RESUME`` env var —
    matching what ``kase_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``KASE_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from kase_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("KASE_TUI_DISABLE_MOUSE", "1")
    env.setdefault("KASE_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["KASE_TUI_RESUME"] = resume

    if sidecar_url:
        env["KASE_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Kase inside WSL2 to use the dashboard's /chat "
...
# KASE_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/kase"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/kase/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /kase`` on every request. We
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/kase``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__KASE_SESSION_TOKEN__="{_SESSION_TOKEN}";'
    ...
    # under ``/kase`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Kase backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Kase Teal",         "description": "Classic dark teal — the canonical Kase look"},
    {"name": "default-large", "label": "Kase Teal (Large)", "description": "Kase Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.kase/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.kase/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_kase_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.kase/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as kase_cli.plugins):
    1. User plugins:    ~/.kase/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.kase/plugins/  (only if HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from kase_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_kase_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("HERMES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".kase" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from kase_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_kase_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from kase_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from kase_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from kase_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from kase_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"kase_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Kase Web UI → http://{host}:{port}")
