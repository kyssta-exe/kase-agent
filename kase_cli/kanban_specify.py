...
Used by ``kase kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``kase_cli/goals.py`` — same aux
...
KASE_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    int(os.getenv("KASE_KANBAN_SPECIFY_MAX_TOKENS", "6000")),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Kase board.
...
def _profile_author() -> str:
    """Mirror of ``kase_cli.kanban._profile_author``. Kept local to
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=KASE_KANBAN_SPECIFY_MAX_TOKENS,
