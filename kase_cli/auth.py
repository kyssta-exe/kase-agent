"""
Multi-provider authentication system for Kase.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.kase/auth.json with cross-process file locking.
...
- Legacy session key (fallback): mint an opaque 24h key when JWT auth is
  unavailable, or when KASE_USE_LEGACY_SESSION_KEYS is set for
...
from kase_cli.config import get_kase_home, get_config_path, read_raw_config
from kase_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_NOUS_PORTAL_URL = "https://portal.kyssta.com"
DEFAULT_KYSSTA_INFERENCE_URL = "https://inference-api.kyssta.com/v1"
DEFAULT_NOUS_CLIENT_ID = "kase-cli"
NOUS_LEGACY_AGENT_KEY_SCOPE = "inference:mint_agent_key"
KYSSTA_INFERENCE_INVOKE_SCOPE = "inference:invoke"
DEFAULT_NOUS_SCOPE = f"{KYSSTA_INFERENCE_INVOKE_SCOPE} {NOUS_LEGACY_AGENT_KEY_SCOPE}"
NOUS_LEGACY_SESSION_KEYS_ENV = "KASE_USE_LEGACY_SESSION_KEYS"
NOUS_DEVICE_CODE_SOURCE = "device_code"
KYSSTA_INFERENCE_AUTH_MODE_AUTO = "auto"
KYSSTA_INFERENCE_AUTH_MODE_FRESH = "fresh"
KYSSTA_INFERENCE_AUTH_MODE_LEGACY = "legacy"
KYSSTA_INFERENCE_AUTH_MODES = frozenset({
    KYSSTA_INFERENCE_AUTH_MODE_AUTO,
    KYSSTA_INFERENCE_AUTH_MODE_FRESH,
    KYSSTA_INFERENCE_AUTH_MODE_LEGACY,
})
...
SPOTIFY_DOCS_URL = "https://kase.kyssta.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://kase.kyssta.com/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://kase.kyssta.com/docs/guides/oauth-over-ssh"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "nous": ProviderConfig(
        ...
        inference_base_url=DEFAULT_KYSSTA_INFERENCE_URL,
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from kase_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.kase/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `kase model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.kase/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_kase_home() / "auth.json"
    # Seat belt: if pytest is running and KASE_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch KASE_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".kase" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set KASE_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom KASE_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.kase/auth.json`` even when KASE_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_hermes_root()`` for a profile-shaped KASE_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".kase" / "auth.json"
            try:
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "kyssta_portal" in systems:
            providers["nous"] = systems["kyssta_portal"]
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``kase auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Kase.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `kase logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from kase_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'kase doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'kase model' for available providers, or run 'kase doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'kase model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.kase/.env.",
...
def _nous_scope_has_invoke(raw_scope: Any) -> bool:
    return KYSSTA_INFERENCE_INVOKE_SCOPE in _scope_values(raw_scope)


def _normalize_nous_inference_auth_mode(inference_auth_mode: Optional[str]) -> str:
    mode = str(inference_auth_mode or KYSSTA_INFERENCE_AUTH_MODE_AUTO).strip().lower()
    if mode not in KYSSTA_INFERENCE_AUTH_MODES:
        allowed = ", ".join(sorted(KYSSTA_INFERENCE_AUTH_MODES))
        raise ValueError(
...
def _nous_invoke_jwt_status(
    ...
    *,
    ...
    if KYSSTA_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _nous_legacy_session_key_reason(
    ...
    *,
    scope: Any = None,
    expires_at: Any = None,
    inference_auth_mode: str = KYSSTA_INFERENCE_AUTH_MODE_AUTO,
) -> str:
    if inference_auth_mode == KYSSTA_INFERENCE_AUTH_MODE_LEGACY:
...
def _choose_nous_inference_auth_path(
    ...
    *,
    ...
    if (
        ...
        and inference_auth_mode != KYSSTA_INFERENCE_AUTH_MODE_LEGACY
    ...
    if (
        inference_auth_mode == KYSSTA_INFERENCE_AUTH_MODE_AUTO
...
#
# Tokens live in ~/.kase/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `kase auth list` / `kase status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.kase/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("KASE_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set KASE_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("KASE_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("KASE_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("KASE_SPOTIFY_ACCOUNTS_BASE_URL"),
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>kase auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `kase auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `kase auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `kase auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `kase auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.kase/.env, and return it.
    ...
    print("       App name:     anything (e.g. kase)")
    ...
    # Persist so subsequent `kase auth spotify` runs skip the wizard.
    save_env_value("KASE_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("KASE_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved KASE_SPOTIFY_CLIENT_ID to ~/.kase/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "KASE_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Kase:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Kase is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.kase/auth.json (not ~/.codex/)
#
# Kase maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Kase auth store (~/.kase/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `kase auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `kase auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `kase auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `kase auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Kase auth store (~/.kase/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Kase auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `kase auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Kase auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Kase's own Codex token store."""
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other Kase processes
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.kase/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok Subscription) in `kase model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `kase model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `kase model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `kase model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.kase/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `kase model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `kase model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Kase (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `kase model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``kase model``
...
def _is_nous_invoke_scope_refusal(exc: Exception) -> bool:
    ...
    return (
        ...
        or "scope" in text and KYSSTA_INFERENCE_INVOKE_SCOPE in text
    )
...
# so a new `kase --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${HERMES_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<kase-root>/shared/nous_auth.json`` where ``<kase-root>`` is what
# ``get_default_hermes_root()`` returns — ``~/.kase`` on Linux/macOS,
# ``%LOCALAPPDATA%\kase`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's KASE_HOME so named profiles (which
# typically live under ``<kase-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    ``<kase-root>/shared/``, where ``<kase-root>`` is what
    :func:`kase_constants.get_default_hermes_root` returns — so
    Linux/macOS classic installs land at ``~/.kase/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\kase\\shared\\``, and
    Docker / custom ``KASE_HOME`` deployments at
    ``<KASE_HOME>/shared/``. Sits outside any named profile so all
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's Kase root, refuse rather than silently corrupt cross-profile
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No KASE_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    ...
    shared = {
        ...
        "inference_base_url": state.get("inference_base_url") or DEFAULT_KYSSTA_INFERENCE_URL,
...
def _quarantine_kyssta_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    invalidate_kyssta_auth_status_cache()


def _quarantine_kyssta_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    try:
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            state: Dict[str, Any] = {
                ...
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_KYSSTA_INFERENCE_URL,
            ...
            refreshed = refresh_nous_oauth_from_state(
                ...
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                inference_auth_mode=KYSSTA_INFERENCE_AUTH_MODE_FRESH,
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Kase's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Kase's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another Kase install sharing "
            "~/.kase/auth.json) called POST /api/oauth/token with Kase's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Kase may call the "
            "refresh endpoint. For health checks, use `kase auth status` "
            "instead.\n"
            "Re-authenticate with: kase auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Kase models — they're not reliable for agentic tool-calling
            if "kase" in mid.lower():
                continue
...
def resolve_kyssta_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Kase is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_nous_refresh_error(exc):
                        _quarantine_kyssta_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_kyssta_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
...
def refresh_nous_oauth_pure(
    ...
    *,
    ...
    state: Dict[str, Any] = {
        ...
        "inference_base_url": (inference_base_url or DEFAULT_KYSSTA_INFERENCE_URL).rstrip("/"),
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "kase-cli"),
        state.get("portal_base_url", DEFAULT_NOUS_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_KYSSTA_INFERENCE_URL),
...
def persist_nous_credentials(
    ...
    *,
    ...
    - ``providers.nous``: singleton state read by
      ``resolve_kyssta_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.nous``: used by the runtime ``pool.select()`` path.

    Historically ``kase auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``kase auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `kase auth add nous --type oauth`. Best-
...
def resolve_kyssta_runtime_credentials(
    *,
    min_key_ttl_seconds: int = DEFAULT_AGENT_KEY_MIN_TTL_SECONDS,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Kase is not logged into Nous Portal.",
        ...
        inference_base_url = (
            ...
            or os.getenv("KYSSTA_INFERENCE_BASE_URL")
            or DEFAULT_KYSSTA_INFERENCE_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_NOUS_CLIENT_ID)
                ...
                try:
                    ...
                    if (
                        ...
                        retry_inference_auth_mode = (
                            KYSSTA_INFERENCE_AUTH_MODE_LEGACY
                            if inference_auth_mode == KYSSTA_INFERENCE_AUTH_MODE_LEGACY
                            else KYSSTA_INFERENCE_AUTH_MODE_FRESH
                        )
        ...
        _persist_state("resolve_kyssta_runtime_credentials_final")
...
def _snapshot_nous_pool_status() -> Dict[str, Any]:
    ...
    of truth because it is what ``resolve_kyssta_runtime_credentials()`` refreshes
    and mints against.
    """
    try:
        from agent.credential_pool import load_pool
...
# ── Process-level memo for get_kyssta_auth_status() ──
# get_kyssta_auth_status() validates state by calling resolve_kyssta_runtime_credentials(),
# which does a synchronous OAuth refresh POST to portal.kyssta.com. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`kase tools`, status panels,
# subscription-feature checks) call it many times per render — `kase tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `kase auth login/logout/add/remove` invalidate naturally on the next call.
_NOUS_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_kyssta_auth_status_cache: Optional[Tuple[float, Optional[float], Dict[str, Any]]] = None
...
def invalidate_kyssta_auth_status_cache() -> None:
    """Clear the get_kyssta_auth_status() process-level memo.

    Call this from any code path that mutates Nous auth state without going
    through resolve_kyssta_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _kyssta_auth_status_cache
    _kyssta_auth_status_cache = None


def get_kyssta_auth_status() -> Dict[str, Any]:
    ...
    ``invalidate_kyssta_auth_status_cache()`` explicitly.
    """
    global _kyssta_auth_status_cache
    now = time.monotonic()
    mtime = _auth_file_mtime()
    cached = _kyssta_auth_status_cache
    ...
    _kyssta_auth_status_cache = (now, mtime, dict(status))
    return status


def _compute_nous_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_kyssta_auth_status(). See that function."""
    ...
    if state:
        ...
        try:
            creds = resolve_kyssta_runtime_credentials(min_key_ttl_seconds=60)
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `kase auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `kase auth` and
    # `kase model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``kase doctor`` runs the live
    ...
    try:
        from kase_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Kase' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `kase doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `kase logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from kase_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from kase_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'kase model' or 'kase setup' instead."""
    print("The 'kase login' command has been removed.")
    print("Use 'kase auth' to manage credentials,")
    print("'kase model' to select a provider, or 'kase setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.kase/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Kase-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Kase auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Kase will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Kase will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Kase gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Kase creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Kase auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Kase auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Kase creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=kase` lets xAI attribute Kase-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Kase — please report at "
            "https://github.com/Kyssta/kase/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize Kase with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Kase auth store (~/.kase/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Kase login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `kase model` and select "
            "MiniMax (OAuth).",
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Kase login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `kase model` again to finish setup.")
            raise SystemExit(1)
        raise
    ...
    try:
        ...
        try:
            ...
            from kase_cli.models import (
                ...
                check_kyssta_free_tier, partition_kyssta_models_by_tier,
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Kase builds still see
        ...
        if not selected_model:
            ...
            print("  Run `kase model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Kase will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `kase model` or configure an API key to use Kase.")
        else:
