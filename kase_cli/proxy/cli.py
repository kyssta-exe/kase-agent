"""CLI handlers for the ``kase proxy`` subcommand."""
...
from kase_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "kase proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"kase login {adapter.name}")
        print(
    ...
    print(
        f"Starting Kase proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Kase proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: kase proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``kase proxy <subcommand>``."""
    ...
    print(
        "kase proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  kase proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
