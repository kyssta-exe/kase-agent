...
# xAI's public API is OpenAI-compatible for the endpoints Kase commonly
# uses. The Responses endpoint is included because Kase' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Kase-managed OAuth credentials."""

    auth_hint = "kase auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`kase auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`kase auth reset xai-oauth` or re-authenticate with "
                    "`kase auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `kase auth add xai-oauth --type oauth`."
            )
