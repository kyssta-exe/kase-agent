...
Reads the user's Nous OAuth state from ``~/.kase/auth.json`` through the
...
:func:`kase_cli.auth.resolve_kyssta_runtime_credentials`.
"""
...
# Endpoints inference-api.kyssta.com actually serves. Anything else
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def __init__(self) -> None:
        ...
        # and persistence are handled by resolve_kyssta_runtime_credentials().
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `kase login nous` first."
                )

            try:
                refreshed = resolve_kyssta_runtime_credentials(
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `kase login nous` to re-authenticate."
                )

            base_url = refreshed.get("base_url") or DEFAULT_KYSSTA_INFERENCE_URL
    ...
    # to kase_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
