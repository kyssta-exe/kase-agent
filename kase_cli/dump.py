"""
Dump command for kase CLI.

Outputs a compact, plain-text summary of the user's Kase setup
...
from kase_cli.config import get_kase_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``kase dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from kase_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    lines.append(f"hermes_home:      {display_kase_home()}")
    ...
    toolsets = config.get("toolsets", ["kase-cli"])
