"""
Interactive setup wizard for Kase.
...
from kase_cli.kyssta_subscription import get_kyssta_subscription_features
from tools.tool_backend_helpers import managed_kyssta_tools_enabled
...
_DOCS_BASE = "https://kase.kyssta.com/docs"
...
# display_kase_home imported lazily at call sites (stale-module safety during kase update)
...
from kase_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Kase Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Kase using environment variables or config commands:")
    print_info("  kase config set model.provider custom")
    print_info("  kase config set model.base_url http://localhost:8080/v1")
    print_info("  kase config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'kase setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from kase_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from kase_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'kase setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    tool_status = []
    subscription_features = get_kyssta_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'kase setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from kase_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `kase tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from kase_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'kase setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'kase setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'kase setup terminal'"))
    elif managed_kyssta_tools_enabled() and subscription_features.nous_auth_present:
    ...
    # Spotify (OAuth via kase auth spotify — check auth.json, not env vars)
    try:
        from kase_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'kase setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('kase setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('kase setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('kase setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('kase setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('kase setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('kase config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('kase config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('kase config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('kase', Colors.GREEN)}              Start chatting")
    print(f"   {color('kase gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('kase doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `kase tools` and `kase setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``kase model``)
    ...
    provider added to ``kase model`` is automatically available here.
    ...
    # Delegate to the shared kase model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from kase_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: kase model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from kase_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Kase can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Kase can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'kase setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``kase model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
        from kase_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_kyssta_tools_enabled() and subscription_features.nous_auth_present:
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.kase/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Kase auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via kase setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'kase setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Kase runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        managed_modal_available = bool(
            ...
            and
            get_kyssta_subscription_features(config).nous_auth_present
        ...
        print_info("Requires the optional SDK: pip install 'kase[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'kase[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `kase setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Kase delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `kase update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://kase.kyssta.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under KASE_HOME, and print
    ...
    """
    try:
        from kase_cli.slack_cli import _build_full_manifest
        from kase_constants import get_kase_home

        manifest = _build_full_manifest(
            bot_name="Kase",
            bot_description="Your Kase agent on Slack",
        )
        target = Path(get_kase_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `kase slack manifest --write` anytime to refresh after "
            "Kase adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Kase delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Kase delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  kase config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Kase to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from kase_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://kase.kyssta.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://kase.kyssta.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  kase config edit")
    print_info("   Open config in your editor:  kase config edit")
...
def setup_gateway(config: dict):
    ...
    from kase_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Kase from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'kase setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     kase config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  kase gateway")
        else:
            from kase_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   kase gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   kase gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `kase setup tools` and `kase tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``kase_cli.auth`` — lists every supported
    ...
    """
    try:
        from kase_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from kase_cli.config
    ...
    if section_key == "model":
        ...
        from kase_cli.gateway import _all_platforms, _platform_status
...
# Config values may have different semantics between OpenClaw and Kase.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Kase to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Kase at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Kase at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Kase at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Kase at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Kase equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Kase config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Kase.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Kase's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Kase can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: kase claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: kase claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Kase configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Kase config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Kase (use kase claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      kase setup           — full or quick (auto-detected)
      kase setup model     — just model/provider
      kase setup tts       — just text-to-speech
      kase setup terminal  — just terminal backend
      kase setup gateway   — just messaging platforms
      kase setup tools     — just tool configuration
      kase setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Kase Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Kase Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Kase installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Kase configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'kase setup model|terminal|")
        ...
        setup_mode = prompt_choice("How would you like to set up Kase?", [
    ...
    print()
    print_info("You can edit these files directly or use 'kase config edit'")
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    the user can customize later via ``kase setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'kase setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    kase setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  kase setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'kase setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Kase to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'kase setup gateway'.")
