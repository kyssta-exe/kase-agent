...
- Keep data: Remove code but keep ~/.kase/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Kase PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing kase or kase PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Kase" comment and following line
                if '# Kase' in line or '# kase' in line:
                    skip_next = True
                    continue
                if skip_next and ('kase' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing kase
                if 'kase' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the kase wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "kase",
        Path("/usr/local/bin/kase"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains kase_cli reference)
                content = wrapper.read_text()
                if 'kase_cli' in content or 'kase' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``kase gateway run`` processes
    ...
    try:
        from kase_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from kase_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``KASE_HOME`` and ``HERMES_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\kase\git\cmd``, ``%LOCALAPPDATA%\kase\git\bin``,
#      ``%LOCALAPPDATA%\kase\git\usr\bin``, ``%LOCALAPPDATA%\kase\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\kase\git\`` and Node to
#      ``%LOCALAPPDATA%\kase\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``kase dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\kase\gateway-service\`` and sometimes
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify Kase-owned User-PATH entries."""
    root = str(hermes_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare kase install dir.
    markers = [root + "\\kase", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if KASE_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "kase".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-kase as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Kase-owned entries from User-scope PATH in the registry.
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete KASE_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("KASE_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(hermes_home: Path) -> list[Path]:
    ...
    ``%LOCALAPPDATA%\\kase\\``.  Only called on full uninstall; they're
...
def _is_default_kase_home(hermes_home: Path) -> bool:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its KASE_HOME directory.

    We shell out to ``kase -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current KASE_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m kase_cli.main` so we don't depend on a `kase`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "kase_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's KASE_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.kase/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.kase/ for future reinstall
    """
    project_root = get_project_root()
    hermes_home = get_kase_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie KASE_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_kase_home(hermes_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Kase Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their KASE_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Kase data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Kase code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # like C:\Users\<u>\AppData\Local\kase\git\cmd, not %LOCALAPPDATA%.
        ...
        if removed_path_entries:
            ...
            log_info("No Kase-owned PATH entries in User environment")

        log_info("Removing KASE_HOME / HERMES_GIT_BASH_PATH User env vars...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Kase-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing kase command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.kase/, just remove the kase subdir
    ...
    #     under KASE_HOME but they're install tooling, not config — safe to
    ...
    # 5. Optionally remove ~/.kase/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile KASE_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/Kyssta/kase/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/Kyssta/kase/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Kase! ⚕")
    print()
