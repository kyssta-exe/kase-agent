...
from kase_cli.config import get_env_value, load_config
...
class NousSubscriptionFeatures:
    ...
    features: Dict[str, KysstaFeatureState]

    @property
    def web(self) -> KysstaFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> KysstaFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> KysstaFeatureState:
        return self.features["tts"]

    @property
    def browser(self) -> KysstaFeatureState:
        return self.features["browser"]

    @property
    def modal(self) -> KysstaFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[KysstaFeatureState]:
...
def get_kyssta_subscription_features(
    ...
    managed_tools_flag = managed_kyssta_tools_enabled()
    ...
    # Tool Gateway via `kase model`, so direct credentials should NOT
...
def get_gateway_eligible_tools(
    ...
    # Quick provider check without the heavy get_kyssta_subscription_features call
    model_cfg = config.get("model")
...
def prompt_enable_tool_gateway(config: Dict[str, object]) -> set[str]:
    ...
    try:
        from kase_cli.setup import prompt_choice
    except Exception:
        return set()
