"""`kase checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.kase/checkpoints/``.  Actions:

    kase checkpoints               # same as `status`
    kase checkpoints status        # total size, project count, breakdown
    kase checkpoints list          # per-project checkpoint counts + workdir
    kase checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    kase checkpoints clear [-f]    # nuke the entire base (asks first)
    kase checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    kase checkpoints
    kase checkpoints prune --retention-days 3 --max-size-mb 200
    kase checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: kase checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``kase checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `kase checkpoints` → status
