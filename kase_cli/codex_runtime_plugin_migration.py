"""Migrate Kase' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Kase' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Kase mcp_servers.<n>.command/args/env  → codex stdio transport
  Kase mcp_servers.<n>.url/headers       → codex streamable_http transport
  Kase mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Kase mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Kase-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Kase' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Kase config.")
...
# Kase keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Kase' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one Kase MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # Kase' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown Kase key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (KASE_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Kase)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Kase' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Kase' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Kase
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="kase-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a KASE_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed kase-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.kase`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Kase' own
    ...
    so a kase installed under /opt/, /usr/local/, or a venv all work.
    KASE_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # KASE_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_kase_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever KASE_HOME its launcher sets
    ...
    # — that would override the launcher's KASE_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("KASE_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("KASE_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["KASE_HOME"] = hermes_home
    # PYTHONPATH passes through so a worktree-launched kase finds the
...
def migrate(
    ...
    *,
    ...
    """Translate Kase mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.kase/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register Kase' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Kase for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Kase config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject Kase' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Kase for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/kase_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_hermes_tools:
        translated["kase-tools"] = _build_hermes_tools_mcp_entry()
        if "kase-tools" not in report.migrated:
            report.migrated.append("kase-tools")
