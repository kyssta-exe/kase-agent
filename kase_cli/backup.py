"""
Backup and import commands for kase CLI.

`kase backup` creates a zip archive of the entire ~/.kase/ directory
(excluding the kase repo and transient files).

`kase import` restores from a backup zip, overlaying onto the current
KASE_HOME root.
"""
...
from kase_constants import get_default_hermes_root, get_kase_home, display_kase_home
...
_EXCLUDED_DIRS = {
    "kase",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to kase root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Kase home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Kase home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"kase-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"kase-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_kase_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        for fname in filenames:
            ...
            # Skip the output zip itself if it happens to be inside kase root
            try:
    ...
    print(f"\nRestore with: kase import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Kase backup.
    ...
    # Look for telltale files that a kase home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Kase backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.kase/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a kase dir name
        if prefix in {".kase", "kase"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_kase_home()}")
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Kase configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # kase_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: kase profile list  (after installing kase)")

        # Guidance
        print()
        if not (hermes_root / "kase").is_dir():
            print("Note: The kase codebase was not included in the backup.")
            print("  If this is a fresh install, run: kase update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  kase -p {pname} gateway install")

        print("Done. Your Kase configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and kase backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to KASE_HOME).
...
# — `kase update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(hermes_home: Optional[Path] = None) -> Path:
    home = hermes_home or get_kase_home()
...
def run_quick_backup(args) -> None:
    """CLI entry point for kase backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_kase_home()}/state-snapshots/")
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `kase claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of KASE_HOME under ``backups/`` before a
    ``kase claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``kase import <archive>``.  Writes to
    ``<KASE_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `kase import` and the
