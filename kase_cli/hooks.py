"""kase hooks — inspect and manage shell-script hooks.

Usage::

    kase hooks list
    kase hooks test <event> [--for-tool X] [--payload-file F]
    kase hooks revoke <command>
    kase hooks doctor

Consent records live under ``~/.kase/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.kase/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``kase hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: kase hooks {list|test|revoke|doctor}")
        print("Run 'kase hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.kase/config.yaml.")
        print("See `kase hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `kase hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `kase hooks test` and `kase hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Kase wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `kase hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `kase hooks doctor` would execute
