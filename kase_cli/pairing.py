...
Usage:
    kase pairing list              # Show all pending + approved users
    kase pairing approve <platform> <code>  # Approve a pairing code
    kase pairing revoke <platform> <user_id> # Revoke user access
    kase pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle kase pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: kase pairing {list|approve|revoke|clear-pending}")
        print("Run 'kase pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.kase/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'kase pairing list' to see pending codes.\n")
