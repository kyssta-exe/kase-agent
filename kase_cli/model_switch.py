...
- ``kase_cli.providers``        -- canonical provider identity + overlays
- ``kase_cli.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    "Kyssta Kase 3 & 4 models are NOT agentic and are not designed "
    "for use with Kase. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Kyssta Kase 3 / Kase 4 chat families.
# The previous substring check (`"kase" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``kase-brain:qwen3-14b-ctx16k`` that just
# happen to carry "kase" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   Kyssta/Kase-3-Llama-3.1-70B, kase-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   kase-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def is_kyssta_kase_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Kase 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Kase 3/4 chat model."""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``kase config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from kase_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'kase model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'kase doctor' — config issues detected:"
    ...
    # Mirrors the same logic in kase_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from kase_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from kase_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by kase provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://kase.kyssta.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Kase release. Falls back to the in-repo
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (KASE_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from kase_cli.auth import _load_auth_store
        ...
        # show up in the picker without requiring a Kase release.
    ...
    # --- 2. Check Kase-only providers (nous, openai-codex, copilot, opencode-go) ---
    from kase_cli.providers import KASE_OVERLAYS
    from kase_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Kase provider ID.
    # KASE_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Kase IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in KASE_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Kase slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # Use curated list — look up by Kase slug, fall back to overlay key
            model_ids = curated.get(hermes_slug, []) or curated.get(pid, [])
    ...
    # in PROVIDER_TO_MODELS_DEV or KASE_OVERLAYS (keeps /model in sync
    # with `kase model`).
    try:
        from kase_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Kase's canonical write key (matches
            ...
            # Kase writes ``models:`` as a dict keyed by model id
            # (see kase_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Kase's own writer uses; a hyphen variant is accepted
            ...
            # active model. Kase's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, kase_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`kase_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from kase_cli.models import fetch_openrouter_models
