"""
Gateway subcommand for kase CLI.

Handles: kase gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_kase_home is imported lazily at call sites to avoid ImportError
# when kase_constants is cached from a pre-update version during `kase update`.
from kase_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from kase_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "kase-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "kase CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``kase gateway status`` from falsely counting the ``kase`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``kase gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "kase_cli.main gateway",
        "kase_cli.main --profile",
        "kase_cli.main -p",
        "kase_cli/main.py gateway",
        "kase_cli/main.py --profile",
        "kase_cli/main.py -p",
        "kase gateway",
        "gateway/run.py",
    ]
    current_home = str(get_kase_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"KASE_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. KASE_HOME
        ...
        # KASE_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "KASE_HOME=" in command and f"KASE_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``kase update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Kase profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Kase profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from kase_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "kase_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``kase update`` users
    # with no running gateway until they re-invoke ``kase gateway``
...
def _sync_kase_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``KASE_HOME``.

    Under ``sudo``, ``KASE_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_kase_home` falls back to ``/root/.kase`` — the wrong
    profile. The unit file pins ``KASE_HOME`` for the actual gateway
    ...
    unit_home = env.get("KASE_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("KASE_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["KASE_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}kase gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}kase gateway restart{scope_flag}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `kase gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``kase gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from kase_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (KASE_HOME-scoped).
...
from kase_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``kase gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``KASE_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("KASE_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "kase-gateway"
SERVICE_DESCRIPTION = "Kase Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current KASE_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (KASE_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from kase_constants import get_default_hermes_root
    home = get_kase_home().resolve()
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary KASE_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when KASE_HOME is a named profile.

    For ``~/.kase/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit KASE_HOME path. Defaults to the current
            ``get_kase_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from kase_constants import get_default_hermes_root
    home = Path(hermes_home or str(get_kase_home())).resolve()
    default = get_default_hermes_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this KASE_HOME.

    Default ``~/.kase`` returns ``kase-gateway`` (backward compatible).
    Profile ``~/.kase/profiles/coder`` returns ``kase-gateway-coder``.
    Any other KASE_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Kase installs that predate the
# kase-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (kase-gateway-*.service) and unrelated third-party
# "kase" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("kase.service",)
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Kase gateway units.

    Detects unit files installed by older Kase versions that used a
    different service name (e.g. ``kase.service`` before the rename to
    ``kase-gateway.service``). When both a legacy unit and the current
    ``kase-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``kase-gateway-coder.service`` and unrelated third-party
      ``kase-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``kase.service`` running an unrelated
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy Kase gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Kase gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Kase gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current kase-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    kase gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy Kase gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Kase gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: kase gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo kase gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    kase gateway uninstall")
    print_info("    sudo kase gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Kase can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo kase gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo kase gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo kase gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Kase often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.kase`` → ``ai.kase.gateway.plist`` (backward compatible).
    Profile ``~/.kase/profiles/coder`` → ``ai.kase.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.kase.gateway-{suffix}" if suffix else "ai.kase.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.kase/kase  -> /home/alice/.kase/kase
      /opt/kase                 -> /opt/kase  (kept as-is)
...
def _kase_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current KASE_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_kase_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.kase                    → /home/alice/.kase
      /root/.kase/profiles/coder     → /home/alice/.kase/profiles/coder
      /opt/custom-kase               → /opt/custom-kase  (kept as-is)
    """
    current_hermes = get_kase_home().resolve()
    current_default = (Path.home() / ".kase").resolve()
    target_default = Path(target_home_dir) / ".kase"

    # Default ~/.kase → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.kase → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.kase) — keep as-is
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        hermes_home = _kase_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m kase_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="KASE_HOME={hermes_home}"
Restart=always
RestartSec=5
...
"""
    ...
    hermes_home = str(get_kase_home().resolve())
    profile_arg = _profile_arg(hermes_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only KASE_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp KASE_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../kase_test`` path into
    # ``Environment="KASE_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Kase install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo kase gateway uninstall --system")
    print_info("         kase gateway install")
    print_info("         kase gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (kase.service from pre-rename installs)
    # before installing the new kase-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}kase gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}kase gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}kase gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_kase_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `kase gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `kase gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}kase gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}kase gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}kase gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}kase gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.kase.gateway-{suffix}" if suffix else "ai.kase.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Kase install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  kase gateway status             # Check status")
    from kase_constants import display_kase_home as _dhh
...
def launchd_stop():
    ...
    # `kase gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Kase install")
    else:
        print("⚠ Service definition is stale relative to the current Kase install")
        print("  Run: kase gateway start")
    ...
    if deep:
        log_file = get_kase_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("KASE_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Kase gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'kase' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/kase/docker/entrypoint.sh before the Kase command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$KASE_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set KASE_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `kase gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set KASE_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `kase gateway restart` which already
    ...
    # `kase gateway start/restart` — leaving the gateway vulnerable to
    ...
    # KASE_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("KASE_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Kase will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @kase:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. kase) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Kase delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Kase agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Kase will use (e.g., kase@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Kase connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: kase pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Kase will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``kase setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run kase under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.kase/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_kase_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'kase pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: kase pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'kase gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from kase_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from kase_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from kase_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Kase will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Kase will store the returned account_id/token in ~/.kase/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `kase gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_kase_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `kase pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `kase pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"Kase\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``kase_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from kase_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.kase/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.kase/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: kase gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: kase gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: kase gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo kase gateway install --system")
                    print_info("  Or run in foreground:  kase gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: kase gateway run")
                print_info("  For persistence:   tmux new -s kase 'kase gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from kase_constants import display_kase_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: kase gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup kase gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: kase gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'kase gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``kase gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: kase gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: kase gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s kase 'kase gateway run'")
                print()
            ...
            print()
            print("  kase gateway run                              # direct foreground")
            print("  tmux new -s kase 'kase gateway run'         # persistent via tmux")
            print("  nohup kase gateway run > ~/.kase/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: kase gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: kase gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: kase gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: kase gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: kase gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  kase gateway install")
                else:
                    print("To install as a service:")
                    print("  kase gateway install")
                    print("  sudo kase gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  kase gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup kase gateway run > ~/.kase/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s kase 'kase gateway run'         # persistent via tmux")
                    print("  nohup kase gateway run > ~/.kase/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  kase gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  kase gateway install  # Install as user service")
                    print("  sudo kase gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Kase gateway unit files from
        # pre-rename installs (e.g. kase.service). Profile units and
        # unrelated third-party services are never touched.
