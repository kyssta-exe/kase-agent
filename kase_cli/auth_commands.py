...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:kase_pkce",
    ...
    if provider == "nous":
        ...
        # <kase-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `kase --profile <name>
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `kase auth
    ...
    raise SystemExit(f"`kase auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Kase reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `kase auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `kase auth` is called bare."""
