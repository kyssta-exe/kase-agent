"""
Configuration management for Kase.

Config files are stored in ~/.kase/ for easy access:
- ~/.kase/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.kase/.env         - API keys and secrets

This module provides:
- kase config          - Show current configuration
- kase config edit     - Open config in editor
- kase config set      - Set a specific value
- kase config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.kase/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``kase logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  kase config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `kase plugins enable
...
from kase_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_kase_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Kase is running in package-manager-managed mode.

    Two signals: the HERMES_MANAGED env var (set by the systemd service),
    or a .managed marker file in KASE_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Kase was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.kase/.install_method`` file (written by installers)
    ...
    5. Fallback -> 'pip'
    """
    stamp = get_kase_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.kase/.install_method."""
    stamp = get_kase_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade kase"
    if method == "docker":
        return "docker pull kyssta/kase:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade kase"
        return "pip install --upgrade kase"
    return "kase update"
...
def format_managed_message(action: str = "modify this Kase installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this Kase installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.kase.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Kase installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade kase"
        )
    ...
    return (
        f"Cannot {action}: this Kase installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Kase."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from KASE_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or KASE_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("KASE_DEV") == "1":
        return None
    ...
    container_mode_file = get_kase_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "kase")
    exec_user = info.get("exec_user", "kase")
    hermes_bin = info.get("hermes_bin", "/data/current-package/bin/kase")
...
# Re-export from kase_constants — canonical definition lives there.
from kase_constants import get_kase_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_kase_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_kase_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the kase group can
    share state with the gateway service.

    The mode can be overridden via the KASE_HOME_MODE environment variable
    (e.g. KASE_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse KASE_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("KASE_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Kase runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("HERMES_CONTAINER") or os.environ.get("KASE_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set KASE_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into KASE_HOME if the user doesn't have one yet."""
...
def ensure_kase_home():
    """Ensure ~/.kase directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_kase_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_kase_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"KASE_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Kase-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Kase auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Kase sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Kase
        ...
        # bundled Kase image, which drops to the `kase` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Kase sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Kase should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``kase chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.kase/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: kase sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Kase truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``kase kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``kase kanban decompose`` and the kanban
        ...
        # ``kase profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `kase model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `kase --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `kase -c` muscle memory.  Default off so existing
        # users aren't surprised.  KASE_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/kase`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.kase/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.kase/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Kase feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Kase auto-pauses the goal and
    ...
    # always goes to ~/.kase/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${HERMES_SKILL_DIR} and ${KASE_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `kase curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.kase/skills/ into
        # ~/.kase/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `kase curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for kase-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Kase*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (KASE_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.kase/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or KASE_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `kase doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``kase_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Kase to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via KASE_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `kase -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `kase` launches with
        ...
        # decomposition is manual via `kase kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     kase's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.kase/logs/.
    ...
    # update model picker lists without shipping a kase release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://kase.kyssta.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `kase model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.kase/state.db.
    ...
    "sessions": {
        ...
        # default of ``kase sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.kase/sessions/session_{sid}.json`` on every turn
    ...
    # ``kase update`` behaviour.
    "updates": {
        # Run a full ``kase backup``-style zip of KASE_HOME before every
        # ``kase update``.  Backups land in ``<KASE_HOME>/backups/`` and
        # can be restored with ``kase import <path>``.  Off by default —
        # on large KASE_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<KASE_HOME>/lsp/bin/`` on first use.
    ...
    # `kase tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.kase/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.kase/.env (or your shell) and never in config.yaml.
            ...
            # ~/.kase/bin/ on first use.  When False you must install
            # bws yourself and have it on PATH.
            "auto_install": True,
        },
    },

    # Multi-model routing: assign different models/providers per task category.
    # Each category supports its own provider, model, base_url, and api_key,
    # giving you full flexibility (different providers per category if desired).
    # When enabled=false or a category's values are empty, falls back to the
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'kase model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. kyssta.com -> firecrawl-gateway.kyssta.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Kase auth store)",
    ...
    "COMPOSIO_API_KEY": {
        "description": "Composio.dev API key — 250+ app integrations (GitHub, Gmail, Slack, Notion, Jira, etc.)",
        "prompt": "Composio API key",
        "url": "https://app.composio.dev/",
        "tools": ["composio_execute", "composio_list_actions", "composio_list_connections", "composio_connect"],
        "password": True,
        "category": "tool",
    },
    "SEARXNG_URL": {
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @kase:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #kase)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: kase-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'kase' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Kase API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Kase API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Kase API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.kase.config`` entries, then checks
    ...
    try:
        ...
        # should never break `kase update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Kase versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``kase_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'kase setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Kase won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'kase doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("KASE_HOME", "~/.kase")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$KASE_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_kase_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `kase plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `kase curator status` has no
    ...
    #   3. Creates `~/.kase/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_kase_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_kase_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: kase config set <key> <value>")
    ...
    # metadata.kase.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _kase_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.kase/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.kase/config.yaml.
    ...
    (which change ``KASE_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — kase auth) — OpenAI Codex
#   nous         (OAuth — kase auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.kase/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.kase/.env.
    ...
    interactive menu render (`kase tools`, `kase setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `kase tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Kase env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.kase/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.kase/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.kase/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.kase/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Kase — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Kase vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.kase/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Kase Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  kase config edit     # Edit config file", Colors.DIM))
    print(color("  kase config set <key> <value>", Colors.DIM))
    print(color("  kase setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: kase config set <key> <value>")
            print()
            print("Examples:")
            print("  kase config set model anthropic/claude-sonnet-4")
            print("  kase config set terminal.backend docker")
            print("  kase config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'kase config migrate' to add them")
        ...
        print("  kase config           Show current configuration")
        print("  kase config edit      Open config in editor")
        print("  kase config set <key> <value>   Set a config value")
        print("  kase config check     Check for missing/outdated config")
        print("  kase config migrate   Update config with new options")
        print("  kase config path      Show config file path")
        print("  kase config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``kase config`` UI so users
