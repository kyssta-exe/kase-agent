"""
Status command for kase CLI.

Shows the status of all Kase components.
"""
...
from kase_cli.auth import AuthError, resolve_provider
from kase_cli.colors import Colors, color
from kase_cli.config import get_env_path, get_env_value, get_kase_home, load_config
from kase_cli.models import provider_label
from kase_cli.kyssta_subscription import get_kyssta_subscription_features
from kase_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``kase config``'s
...
def show_status(args):
    """Show status of all Kase components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Kase Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: kase auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: kase model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: kase auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: kase auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: kase model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from kase_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'kase[vercel]')"
    ...
    try:
        from kase_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_kase_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_kase_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'kase doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'kase setup' to configure", Colors.DIM))
    print()
