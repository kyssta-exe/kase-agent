#!/usr/bin/env python3
"""
Kase CLI - Main entry point.

Usage:
    kase                     # Interactive chat (default)
    kase chat                # Interactive chat
    kase gateway             # Run gateway in foreground
    kase gateway start       # Start gateway as service
    kase gateway stop        # Stop gateway service
    kase gateway status      # Show gateway status
    kase gateway install     # Install gateway service
    kase gateway uninstall   # Uninstall gateway service
    kase setup               # Interactive setup wizard
    kase logout              # Clear stored authentication
    kase status              # Show status of all components
    kase cron                # Manage cron jobs
    kase cron list           # List cron jobs
    kase cron status         # Check if cron scheduler is running
    kase doctor              # Check configuration and dependencies
    kase honcho setup                    # Configure Honcho AI memory integration
    kase honcho status                   # Show Honcho config and connection status
    kase honcho sessions                 # List directory → session name mappings
    kase honcho map <name>               # Map current directory to a session name
    kase honcho peer                     # Show peer names and dialectic settings
    kase honcho peer --user NAME         # Set user peer name
    kase honcho peer --ai NAME           # Set AI peer name
    kase honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    kase honcho mode                     # Show current memory mode
    kase honcho mode [hybrid|honcho|local]  # Set memory mode
    kase honcho tokens                   # Show token budget settings
    kase honcho tokens --context N       # Set session.context() token cap
    kase honcho tokens --dialectic N     # Set dialectic result char cap
    kase honcho identity                 # Show AI peer identity representation
    kase honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    kase honcho migrate                  # Step-by-step migration guide: OpenClaw native → Kase + Honcho
    kase version             Show version
    kase update              Update to latest version
    kase uninstall           Uninstall Kase
    kase acp                 Run as an ACP server for editor integration
    kase sessions browse     Interactive session picker with search

    kase claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: kase_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``kase_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``kase update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``kase_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, kase crashes on import and the user can't run
# ``kase update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import kase_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to KASE_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (kase tools, kase setup, kase model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'kase {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any kase module import.
#
# Many modules cache KASE_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("KASE_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.kase/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set KASE_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors kase_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If KASE_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.kase/profiles/coder or
    # /opt/data/profiles/coder).  If KASE_HOME points to the kase root
    # instead (e.g. systemd hardcodes KASE_HOME=/root/.kase), we must
    # still read active_profile — the user may have switched profiles via
    # `kase profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("KASE_HOME", "")
    if profile_name is None and hermes_home_env:
        if Path(hermes_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the kase root
    if profile_name is None:
        try:
            from kase_constants import get_default_hermes_root
    ...
    # 3. If we found a profile, resolve and set KASE_HOME
    if profile_name is not None:
        try:
            from kase_cli.profiles import resolve_profile_env
            ...
            # A bug in profiles.py must NEVER prevent kase from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["KASE_HOME"] = hermes_home
...
# Load .env from ~/.kase/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from kase_cli.config import get_kase_home
from kase_cli.env_loader import load_kase_dotenv

load_kase_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERMES_REDACT_SECRETS env
# var BEFORE kase_logging imports agent.redact (which snapshots the flag at
...
try:
    if "HERMES_REDACT_SECRETS" not in os.environ:
        ...
        _cfg_path = get_kase_home() / "config.yaml"
        if _cfg_path.exists():
...
# Initialize centralized file logging early — all `kase` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from kase_logging import setup_logging as _setup_logging
...
try:
    from kase_cli.config import load_config as _load_config_early
    from kase_constants import apply_ipv4_preference as _apply_ipv4
...
from kase_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_kase_home() / "skills" / ".termux_bundled_sync_stamp"
...
def _has_any_provider_configured() -> bool:
    ...
    from kase_cli.config import get_env_path, get_kase_home, load_config
    from kase_cli.auth import get_auth_status

    # Determine whether Kase itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from kase_cli.config import DEFAULT_CONFIG
    ...
    from kase_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_kase_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Kase has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Kase to use their tokens.
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'kase')
    """
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo kase {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo kase {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  kase --tui --resume {target}")
    if title:
        print(f'  kase --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @kase/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@kase" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/kase-ink/package.json",
    "packages/kase-ink/package-lock.json",
    "packages/kase-ink/index.js",
    "packages/kase-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``KASE_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("KASE_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``KASE_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("KASE_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    hermes_home = os.environ.get("KASE_HOME") or str(Path.home() / ".kase")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "KASE_HOME": hermes_home},
            capture_output=True,
            text=True,
            check=False,
        )
...
def _find_bundled_tui(kase_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if kase_cli_dir is None:
        kase_cli_dir = Path(__file__).parent
    bundled = kase_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (KASE_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from kase_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("KASE_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with KASE_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset KASE_TUI_DIR (e.g. `unset KASE_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("KASE_QUIET"):
    ...
    if tui_dev:
        # Keep the local @kase/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @kase/ink resolves through
        # packages/kase-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "kase-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from kase_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="kase-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["KASE_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["HERMES_PYTHON_SRC_ROOT"] = os.environ.get(
        "HERMES_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("HERMES_PYTHON", sys.executable)
    env.setdefault("KASE_CWD", os.getcwd())
    ...
    if provider:
        env["KASE_TUI_PROVIDER"] = provider
        env["HERMES_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["KASE_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["KASE_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["KASE_TUI_SKILLS"] = value
    if query:
        env["KASE_TUI_QUERY"] = query
    if image:
        env["KASE_TUI_IMAGE"] = image
    if checkpoints:
        env["KASE_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["KASE_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["KASE_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["KASE_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["KASE_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["KASE_ACCEPT_HOOKS"] = "1"
    ...
    # KASE_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `kase --tui` try to
    ...
    # still set KASE_TUI_RESUME themselves.
    env.pop("KASE_TUI_RESUME", None)
    if resume_session_id:
        env["KASE_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `kase update` so
    ...
    if code == 42:
        from kase_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``KASE_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``kase kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``kase kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("KASE_KANBAN_BOARD"):
        return
    try:
        from kase_cli.kanban_db import get_current_board

        os.environ["KASE_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("KASE_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'kase sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from kase_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from kase_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'kase doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Kase isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  kase setup")
        print()

        from kase_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'kase setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from kase_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["KASE_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.kase/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["KASE_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["KASE_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["KASE_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from kase_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from kase_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from kase_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Kase?")
        print()
    ...
    # has no creds.json.  Every subsequent `kase gateway` then paid a 30s
    ...
    session_dir = get_kase_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: kase gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `kase gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  kase gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Kase'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  kase gateway")
        ...
        print()
        print("  Or install as a service: kase gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'kase whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from kase_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from kase_cli.config import stamp_install_method
    from kase_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Kase post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``kase model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from kase_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from kase_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from kase_cli.providers import resolve_provider_full
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from kase_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'kase model' for "
                "available providers, or run 'kase doctor' to diagnose config "
                "issues."
            )
    ...
    from kase_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.kase/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.kase/.env if the active provider is not 'custom'.
    ...
    """
    from kase_cli.config import get_env_value, save_env_value, load_config
...
#
# Kase uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `kase model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `kase model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from kase_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Kase only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``kase model`` flow, then route aux tasks to them here.
    """
    from kase_cli.config import load_config
    from kase_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from kase_cli.auth import _prompt_model_selection
    from kase_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from kase_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from kase_constants import OPENROUTER_BASE_URL
    from kase_cli.auth import (
        ProviderConfig,
    ...
    # in-flow (K/R/C) instead of having to edit ~/.kase/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `kase setup` from scratch. OpenRouter
    ...
    from kase_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from kase_constants import AI_GATEWAY_BASE_URL
    from kase_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.kase/.env by hand.
    ...
    from kase_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from kase_cli.auth import (
        ...
        resolve_kyssta_runtime_credentials,
        AuthError,
        format_auth_error,
        _login_nous,
        PROVIDER_REGISTRY,
    ...
    from kase_cli.kyssta_subscription import prompt_enable_tool_gateway
    ...
    from kase_cli.models import (
        get_curated_nous_model_ids,
        get_pricing_for_provider,
        check_kyssta_free_tier,
        partition_kyssta_models_by_tier,
    ...
    try:
        creds = resolve_kyssta_runtime_credentials(min_key_ttl_seconds=5 * 60)
    except Exception as exc:
    ...
    free_tier = check_kyssta_free_tier()
    ...
    try:
        ...
        if _nous_state:
            _kyssta_portal_url = _nous_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if free_tier:
        model_ids, pricing = union_with_portal_free_recommendations(
            model_ids, pricing, _kyssta_portal_url,
        )
        model_ids, unavailable_models = partition_kyssta_models_by_tier(
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from kase_cli.auth import DEFAULT_NOUS_PORTAL_URL

            _url = (_kyssta_portal_url or DEFAULT_NOUS_PORTAL_URL).rstrip("/")
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from kase_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `kase auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from kase_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from kase_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``kase model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``kase model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``kase auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from kase_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from kase_cli.auth import _save_model_choice, deactivate_provider
    from kase_cli.config import get_env_value, load_config, save_config
    ...
    from kase_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `kase model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from kase_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Kase URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from kase_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from kase_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from kase_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Kase will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Kase will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from kase_cli.curses_ui import flush_stdin
...
# it to the model-selection handlers so plain `kase --tui` does not pay for
...
def _model_flow_copilot(config, current_model=""):
    ...
    from kase_cli.config import save_env_value, load_config, save_config
    from kase_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from kase_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from kase_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from kase_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Kase turns to `copilot --acp`.")
    print("  Kase currently starts its own ACP subprocess for each request.")
    print("  Kase uses your selected model as a hint for the Copilot ACP session.")
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``kase setup`` / ``kase model``.
    ...
    recover from a malformed paste without editing ``~/.kase/.env`` by hand.
    ...
    from kase_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from kase_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `kase setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from kase_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so kase knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Kase typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Kase, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from kase_cli.auth import AuthError
        from kase_cli.models import fetch_lmstudio_models
        ...
        from kase_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from kase_constants import display_kase_home as _dhh_fn

            print(
                f"    Kase will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from kase_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'kase model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Kase CLI with a provider."""
    from kase_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from kase_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from kase_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from kase_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from kase_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from kase_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``kase slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: kase slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `kase slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from kase_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from kase_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from kase_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from kase_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from kase_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from kase_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from kase_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Kase home directory to a zip file."""
    if getattr(args, "quick", False):
        from kase_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from kase_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Kase backup from a zip file."""
    from kase_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    print(f"Kase v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from kase_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from kase_cli.uninstall import run_uninstall
...
# Critical files that every ``kase`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``kase update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "kase_cli/main.py",
    "kase_cli/config.py",
    "kase_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``kase`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="kase-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``kase update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``kase_cli/web_dist/`` (per vite.config.ts
    outDir: "../kase_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "kase_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``kase update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``kase web``).
    ...
    # build path usable on a stock `py -m kase_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (kase web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``kase dashboard`` processes other than ourselves.

    ``kase dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``kase update`` replaces files on disk, the running
    ...
    patterns = [
        "kase dashboard",
        "kase_cli.main dashboard",
        "kase_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "kase.*dashboard"`) keeps us consistent
            # with `kase_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `kase update`.
    ...
    print("  Preview now:  kase curator run --dry-run")
    print("  Pause it:     kase curator pause")
    print(
        "  Docs:         https://kase.kyssta.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``kase update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``kase update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: kase curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``kase dashboard`` processes.

    Called at the end of ``kase update`` (default ``reason``) and also
    from ``kase dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    kase dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Kase by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/Kyssta/kase/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="kase-update-")
        zip_path = os.path.join(tmp_dir, f"kase-{branch}.zip")
        ...
        # GitHub ZIPs extract to kase-<branch>/
        extracted = os.path.join(tmp_dir, f"kase-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "kase-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Kase behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes kase completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Kase couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Kase couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Kase behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `kase update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/Kyssta/kase.git",
    "git@github.com:Kyssta/kase.git",
    "https://github.com/Kyssta/kase",
    "git@github.com:Kyssta/kase",
}
OFFICIAL_REPO_URL = "https://github.com/Kyssta/kase.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_kase_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_kase_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Kase repository.")
        print("  This means you may miss updates from Kyssta/kase.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/Kyssta/kase.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/Kyssta/kase.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``kase update``, every profile is now current.
    """
    homes = []
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``kase update`` is still progressing even if pip/uv itself is silent.
    """
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "kase.exe",
        scripts_dir / "kase-gateway.exe",
    ]
...
def _detect_concurrent_hermes_instances(
    ...
    The Kase Desktop Electron app spawns ``kase.EXE`` as a backend child,
    so during ``kase update`` the user-invoked process and the desktop's
    ...
    shims (``kase.exe`` / ``kase-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID is excluded so the running
    ``kase update`` invocation never reports itself.
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "kase.exe"
    lines = ["✗ Another kase.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Kase Desktop, exit any open `kase` REPLs, and")
    lines.append("  stop the gateway (`kase gateway stop`) before retrying.")
    lines.append("  Override with `kase update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    """Pre-empt Windows file lock on the running ``kase.exe``.
    ...
    when ``kase update`` runs, ``kase.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``kase.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next kase invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Kase Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Kase Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close Kase Desktop, exit other `kase` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `kase update`."
        )
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``kase.exe.old.*`` left by prior updates.

    Called early on every kase invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``kase update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `kase update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``kase.exe`` / ``kase-gateway.exe`` shims
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.kase/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``kase update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.kase/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``kase update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``kase update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.kase/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``kase update --gateway``) the update is already
    ...
    try:
        ...
        # kase_cli.config.get_kase_home to simulate setup failure.
        from kase_cli.config import get_kase_home as _get_kase_home

        logs_dir = _get_kase_home() / "logs"
...
def _cmd_update_check():
    """Implement ``kase update --check``: fetch and report without installing."""
    from kase_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from kase_cli.config import recommended_update_command
        from kase_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``kase update`` without requiring a reinstall.  The
    ...
    ``kase`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/kase.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v kase'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/kase, code at /usr/local/lib/kase).
    fhs_link = Path("/usr/local/bin/kase")
    ...
    path_comment = (
        "# Kase — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of KASE_HOME before running the update.
    ...
    KASE_HOME directories.  The ``--backup`` flag on ``kase update``
    ...
    try:
        from kase_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_kase_home so the user sees ~/.kase/...
    try:
        from kase_constants import get_kase_home, display_kase_home

        home = get_kase_home()
        try:
            display_path = f"{display_kase_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  kase import {out_path}")
...
def cmd_update(args):
    """Update Kase to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from kase_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Kase")
        return
...
def _cmd_update_pip(args):
    """Update Kase via pip (for PyPI installs)."""
    from kase_cli import __version__
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "kase"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "kase"]
    ...
    print("✓ Update complete! Restart kase to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another kase.exe is holding the venv shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/Kyssta/kase/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # orphan merge-conflict markers in kase_cli/config.py bricked
        # every user who ran ``kase update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``kase
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``kase update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_kase_home added to kase_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload kase_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_kase_home() added since the last release.
        try:
            import importlib
            import kase_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit KASE_HOME so
        # it is not affected by sync_skills()'s module-level KASE_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's KASE_HOME env var points at the default or a named profile.
        try:
            from kase_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: kase config migrate")
            else:
                print()
                print("Skipped. Run 'kase config migrate' later to configure.")
        else:
        ...
        # check `kase curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``kase update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from kase_cli.tools_config import install_cua_driver
        ...
        # When running as ``kase update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_kase_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all kase-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. kase-gateway.service or kase-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: kase gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: kase -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Kase gateway unit files are still installed.
        # When both kase.service (from a pre-rename install) and the
        # current kase-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `kase update` surfaces the issue until the user migrates.
        try:
            from kase_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy Kase gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_hermes_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (kase.service) fight the current")
                print("  kase-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    kase gateway migrate-legacy")
                print()
        ...
        print("  kase model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``kase -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from kase_constants import display_kase_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `kase profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_kase_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → kase -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.kase)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  kase profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     kase -p {name} chat")
                else:
        ...
        from kase_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from kase_constants import get_kase_home as _hh
        ...
        from kase_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `kase profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from kase_cli.profiles import profile_exists
        ...
        from kase_cli.profiles import rename_profile
        ...
        from kase_cli.profiles import export_profile
        ...
        from kase_cli.profiles import import_profile
        ...
        import tempfile
        from kase_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  kase -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      kase -p {plan.manifest.name} chat")
        ...
        from kase_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `kase profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"kase -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from kase_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     Kase {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from kase_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.hermes_requires:
        print(f"  Requires: Kase {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``kase dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``kase dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No kase dashboard processes running.")
        return 0

    print(f"{len(pids)} kase dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No kase dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `kase update`.
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "kase_cli" / "web_dist"
        )
    ...
    from kase_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from kase_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Kase log files."""
    from kase_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from kase_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``kase -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``kase_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``kase -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``kase`` or only flags → defaults to ``chat``.
        return False
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        from kase_cli.plugins import discover_plugins
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if "-h" in argv or "--help" in argv:
        return False
    if os.environ.get("KASE_TUI") == "1" or "--tui" in argv:
        return False
    ...
    if getattr(args, "oneshot", None):
        ...
        from kase_cli.oneshot import run_oneshot
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `kase --tui` is the hot path on phones. The full parser setup imports
    ...
    wants_tui = os.environ.get("KASE_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("KASE_TUI") == "1"):
        return False
...
def main():
    """Main entry point for kase CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from kase_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``kase.exe.old.*`` quarantine files left by previous
    # ``kase update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: kase-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from kase_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://kase.kyssta.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `kase model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.kase/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://kase.kyssta.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from kase_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from kase_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy kase.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Kase gateway unit files "
            "(e.g. kase.service) left over from older installs. Profile "
            "units (kase-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `kase proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Kase with an interactive wizard. "
        "Run a specific section: kase setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'kase setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Kase.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $KASE_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Kase")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from kase_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Kase CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: kase-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Kase with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set KASE_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Kase components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.kase/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Kase profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.kase/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Kase profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from kase_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.kase/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.kase/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Kase setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `kase "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Kase setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Kase. Use 'kase debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    kase debug share              Upload debug report and print URL
    kase debug share --lines 500  Include more log lines
    kase debug share --expire 30  Keep paste for 30 days
    kase debug share --local      Print report locally (no upload)
    kase debug share --no-redact  Disable upload-time secret redaction
    kase debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'kase debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Kase home directory to a zip file",
        description="Create a zip archive of your entire Kase configuration, "
        "skills, sessions, and data (excludes the kase codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/kase-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.kase/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo kase uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from kase_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Kase backup from a zip file",
        description="Extract a previously created Kase backup into your "
        "Kase home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Kase configuration",
    )
    ...
    def cmd_pairing(args):
        from kase_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.kase/skills/.bundled_manifest) "
            "so future 'kase update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from kase_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from kase_cli.skills_hub import skills_command
    ...
    from kase_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/kase-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `kase plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from kase_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``kase --help``, ``kase version``, ``kase logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from kase_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from kase_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_kase_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_kase_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_kase_home()}/memories/\n")
        else:
            from kase_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'kase tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # kase tools disable <name...> [--platform cli]
    ...
    # kase tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from kase_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from kase_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `kase computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `kase tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: kase computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: kase computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Kase as an MCP server",
        description=(
            "Manage MCP server connections and run Kase as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'kase mcp add' to connect to a new server, or\n"
            "'kase mcp serve' to expose Kase conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Kase as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `kase mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from kase_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_kase_home() / "sessions"
            ...
            # Launch kase --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Kase",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Kase",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.kase/ (by default a "
        "single restore-point archive is written to ~/.kase/backups/ "
        "before apply; restorable with 'kase import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from kase_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Kase to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'kase config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--force",
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another kase.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Kase",
        description="Remove Kase from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Kase as an ACP (Agent Client Protocol) server",
        description="Start Kase in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Kase ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Kase provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.kase/node/ "
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Kase instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `kase update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `kase profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Kase profile distribution. SOURCE can be a git URL "
    ...
    # for `kase dashboard` cmdlines and SIGTERM them directly — the same
    # path `kase update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running kase dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running kase dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Kase log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    kase logs                    Show last 50 lines of agent.log
    kase logs -f                 Follow agent.log in real time
    kase logs errors             Show last 50 lines of errors.log
    kase logs gateway -n 100     Show last 100 lines of gateway.log
    kase logs --level WARNING    Only show WARNING and above
    kase logs --session abc123   Filter by session ID
    kase logs --component tools  Only show tool-related lines
    kase logs --since 1h         Lines from the last hour
    kase logs --since 30m -f     Follow, starting from 30 min ago
    kase logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``kase -c Pokemon Agent Dev`` → ``kase -c 'Pokemon Agent Dev'``
    ...
    from kase_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'kase -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (kase hooks list, cron
