...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or kase -c.",
    ...
    "Ctrl+Z suspends Kase to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "kase -c resumes your most recent CLI session. kase -c \"project name\" resumes by title.",
    "kase -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "kase -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "kase chat -t web,terminal enables only specific toolsets for a focused session.",
    "kase chat -s github-pr-workflow preloads a skill at launch.",
    "kase chat -q \"query\" runs a single non-interactive query and exits.",
    "kase chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "kase chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "kase --yolo bypasses all dangerous command approval prompts for the entire session.",
    "kase chat --source telegram tags the session for filtering in kase sessions list.",
    "kase -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "kase doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "kase dump outputs a compact setup summary — great for bug reports.",
    "kase config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "kase config edit opens config.yaml in your default editor.",
    "kase config check scans for missing or stale configuration options.",
    "kase sessions browse opens an interactive session picker with search.",
    "kase sessions stats shows session counts by platform and database size.",
    "kase sessions prune --older-than 30 cleans up old sessions.",
    "kase skills search react --source skills-sh searches the skills.sh public directory.",
    "kase skills check scans installed hub skills for upstream updates.",
    "kase skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "kase skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "kase mcp add github --command npx adds MCP servers from the command line.",
    "kase mcp serve runs Kase itself as an MCP server for other agents.",
    "kase auth add lets you add multiple API keys for credential pool rotation.",
    "kase completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "kase logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "kase backup creates a zip backup of your entire Kase home directory.",
    "kase profile create coder creates an isolated profile that becomes its own command.",
    "kase profile create work --clone copies your current config and keys to a new profile.",
    "kase update syncs new bundled skills to ALL profiles automatically.",
    "kase gateway install sets up Kase as a system service (systemd/launchd).",
    "kase memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "kase webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: kase tools disables unused tools, kase skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "kase models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as kase -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Kase tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'kase profile create coder' creates the 'coder' command.",
    "kase profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Kase prints a resume command with session ID and stats.",
    "kase sessions export backup.jsonl exports all sessions for backup or analysis.",
    "kase -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "kase skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .kase/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: kase cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Kase runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "kase gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Kase via a local macOS server.",
    ...
    "Context files (.kase.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.kase/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Kase loads project context from .kase.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — kase tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.kase/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "kase chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "kase plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "kase acp runs Kase as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "kase login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "kase status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.kase/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Kase your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.kase/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Kase auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.kase/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at KASE_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "KASE_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in KASE_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.kase/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'kase profile create ls' would be rejected.",
    "kase profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "kase claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "KASE_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via kase tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.kase/.env.",
    "Skills from trusted repos (Kyssta) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.kase/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Kase auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Kase config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Kase is working.',
    ...
    '/reload reloads ~/.kase/.env into the running session — pick up new API keys without restarting.',
    ...
    "KASE_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.kase/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.kase/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'kase curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "kase curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'kase curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'kase auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'KASE_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "KASE_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'kase dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'kase dashboard --tui embeds the full Kase TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.kase/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.kase/dashboard-plugins/ — no npm build required.',
    ...
    'KASE_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'HERMES_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'KASE_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'KASE_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.kase/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.kase/skills/ so drop-in skills appear without restarting the session.',
    ...
    'kase -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'kase chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'kase chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'kase chat --ignore-user-config skips ~/.kase/config.yaml — reproducible bug reports and CI runs.',
    "kase chat --source tool tags programmatic chats so they don't clutter kase sessions list.",
    'kase dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'kase sessions rename <ID> "new title" renames any past session; kase sessions delete <ID> removes one.',
    'kase import restores a session export or profile archive produced by sessions export or profile export.',
    'kase fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'kase pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'kase setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'kase status --deep runs the full health sweep across every component; plain kase status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'KASE_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.kase/plugins/ — trust-gated by design.',
    "KASE_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'KASE_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'KASE_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'KASE_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    ...
    'KASE_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'KASE_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Kase block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from kase sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.kase/dashboard-plugins/.',
]
