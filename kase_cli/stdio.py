...
on Japanese locales, etc.).  Kase's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``kase_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Kase is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``KASE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("KASE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Kase-managed Git install directories so
    ...
    # already-running shells don't see the change — which means kase
    ...
    # install without Kase-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Kase's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Kase's
    ``kase config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Kase (User env var in Windows Settings, or
    export in a PowerShell profile) and Kase picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Kase-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\kase\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``kase`` from right after install) retain their old PATH.

    Any subprocess Kase spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Kase-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "kase", "git", "cmd"),
        os.path.join(local_appdata, "kase", "git", "bin"),
        os.path.join(local_appdata, "kase", "git", "usr", "bin"),
        # Kase venv Scripts directory — host of the kase.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes kase, but harmless to include.
        os.path.join(local_appdata, "kase", "kase", "venv", "Scripts"),
