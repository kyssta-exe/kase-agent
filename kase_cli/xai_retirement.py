...
Pure logic: walks a Kase config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `kase doctor` and a future `kase migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Kase config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Kase config and return retirement issues.
