...
Pure display functions with no KaseCLI state dependency.
"""
...
from kase_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

KASE_LOGO = """[bold #38B2AC]██╗  ██╗ █████╗ ███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #38B2AC]██║ ██╔╝██╔══██╗██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#4FD1C5]█████╔╝ ███████║███████╗█████╗  █████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#4FD1C5]██╔═██╗ ██╔══██║╚════██║██╔══╝  ╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#81E6D9]██║  ██╗██║  ██║███████║███████╗      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# (e.g. nix-built kase — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/Kyssta/kase.git"
...
def _fetch_pypi_latest(package: str = "kase") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Kase update is available.
    ...
    if embedded_rev:
        ...
        # $KASE_HOME/kase/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "kase"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Kase git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$KASE_HOME/kase/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
_RELEASE_URL_BASE = "https://github.com/Kyssta/kase/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Kase checkout. Cached per-process. Release URL always points at the
    canonical Kyssta/kase repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Kase v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Kyssta[/]")

    if os.getenv("KASE_YOLO_MODE"):
    ...
    try:
        from kase_cli.codex_runtime_switch import get_current_runtime
        from kase_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from kase_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built kase; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else KASE_LOGO
        console.print(_logo)
        console.print()
