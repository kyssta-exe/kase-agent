...
``<root>`` is the **shared Kase root** (the parent of any active
...
``kase -p <profile>`` joins the same board as the dispatcher that
...
the profile's Kase home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``KASE_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``KASE_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``kase kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.kase``. In Docker / custom
deployments where ``KASE_HOME`` points outside ``~/.kase`` (e.g.
``/opt/kase``), ``<root>`` is ``KASE_HOME``. Legacy env-var
overrides still work:

* ``KASE_KANBAN_DB`` — pin the database file path directly.
* ``KASE_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``KASE_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``KASE_KANBAN_DB``,
``KASE_KANBAN_WORKSPACES_ROOT``, and ``KASE_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/kase-kanban-v1-spec.pdf`` for the full
...
# ``KASE_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``KASE_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("KASE_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``kase``
...
def kanban_home() -> Path:
    """Return the shared Kase root that anchors the kanban board.

    Resolution order:

    1. ``KASE_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``KASE_HOME`` is ``<root>/profiles/<name>``, and returns
       ``KASE_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``KASE_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("KASE_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``kase kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``KASE_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``kase kanban boards
    ...
    """
    env = os.environ.get("KASE_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``kase kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``KASE_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("KASE_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``KASE_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("KASE_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``kase kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``KASE_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``KASE_SESSION_ID``. NULL
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``KASE_KANBAN_DB`` env → ``KASE_KANBAN_BOARD`` env →
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # See kase_state._WAL_INCOMPAT_MARKERS for detection logic.
            from kase_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``kase kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``KASE_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from kase_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``kase --skills ...`` alongside the built-in
    ...
    # `kase --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``kase kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `kase kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``kase kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``kase kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``kase kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``kase kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Kase
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``kase kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Kase profile.
    ...
    if not rows:
        return False
    try:
        from kase_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Kase profile.
        # `_default_spawn` invokes ``kase -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Kase
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `kase kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Kase CLI invocation."""
    # ``kase_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``kase`` package — there is no
    # ``kase`` package to import.
    return [sys.executable, "-m", "kase_cli.main"]


def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Kase shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Kase executable path.
...
def _resolve_kase_argv() -> list[str]:
    """Resolve the ``kase`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$KASE_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("kase")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\kase.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m kase_cli.main`` — fallback for setups where
       Kase is launched from a venv and the ``kase`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_kase_bin`` for the same reason. Kept
    local (not imported from gateway) because ``kase_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("KASE_BIN", "").strip()
    if env_bin:
    ...
    hermes_bin = _safe_which_no_cwd("kase") if _IS_WINDOWS else shutil.which("kase")
    if hermes_bin:
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``kase -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset KASE_HOME means the worker falls back to the default root
    # home (``~/.kase``), which ships the bundled skill.
    base = _Path(hermes_home) if hermes_home else (_Path.home() / ".kase")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``kase -p <profile> chat -q ...`` subprocess.
    ...
    ``KASE_KANBAN_DB`` / ``KASE_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject KASE_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `kase -p <name>` the
    # _apply_profile_override() runs *before* kase_constants is imported.
    # If KASE_HOME is absent from the child's env, get_kase_home() falls
    # back to Path.home() / ".kase" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from kase_cli.profiles import resolve_profile_env
    try:
        env["KASE_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HERMES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # KASE_HOME never had profiles created.
        pass
    if task.tenant:
        env["HERMES_TENANT"] = task.tenant
    env["KASE_KANBAN_TASK"] = task.id
    env["KASE_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["KASE_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["KASE_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["KASE_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`kase -p <name>`
    # rewrites KASE_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hermes_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["KASE_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["KASE_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["KASE_KANBAN_BOARD"] = resolved_board
    # HERMES_PROFILE is the author the kanban_comment tool defaults to.
    # `kase -p <assignee>` activates the profile, but the env var is
    ...
    cmd = [
        *_resolve_kase_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped KASE_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("KASE_HOME")):
    ...
    # `kase kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`kase` executable not found on PATH. "
            "Install Kase or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``kase kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HERMES_PROFILE values like "kase-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Kase root exists

    Reads profile paths directly so this module has no import dependency on
    ``kase_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from kase_constants import get_default_hermes_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``kase kanban assignees`` for the terminal.
