"""``kase debug`` debug tools for Kase.

Currently supports:
    kase debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.kase/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[kase debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.kase/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``kase debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``kase debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Kase version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `kase debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "kase/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``kase debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.kase/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``kase debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"kase debug delete {url}"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from kase_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_kase_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``kase dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``kase dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "kase debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  kase debug delete <url>")

    print(f"\nShare these links with the Kase team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: kase debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'kase debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``kase debug`` call.
    ...
    # best-effort — any failure is swallowed so ``kase debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: kase debug <command>")
        print()
        print("Commands:")
