...
configured for "cli" in `kase tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (KASE_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `kase chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"kase -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "kase -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"kase -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "kase -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "kase -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "kase -z: --provider requires --model (or HERMES_INFERENCE_MODEL). "
    ...
    os.environ["KASE_ACCEPT_HOOKS"] = "1"
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``kase -z`` / oneshot mode.

    Oneshot bypasses ``KaseCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when kase is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from kase_cli.config import load_config
    from kase_cli.models import detect_provider_for_model
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from kase_cli import model_switch as _ms
    ...
    # workers spawned via `kase -p <profile> chat -q ...`) honour the
    ...
    agent = AIAgent(
        ...
        #                KASE_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via KASE_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via KASE_YOLO_MODE=1
