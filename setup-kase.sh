...
# 5. Symlinks the 'kase' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/kase-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink kase into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up kase command..."

KASE_BIN="$SCRIPT_DIR/venv/bin/kase"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$KASE_BIN" "$COMMAND_LINK_DIR/kase"
echo -e "${GREEN}✓${NC} Symlinked kase → $COMMAND_LINK_DISPLAY_DIR/kase"
...
# ============================================================================
# Seed bundled skills into ~/.kase/skills/
# ============================================================================

KASE_SKILLS_DIR="${KASE_HOME:-$HOME/.kase}/skills"
mkdir -p "$KASE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.kase/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$KASE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  kase status        # Check configuration"
if is_termux; then
    echo "  kase gateway       # Run gateway in foreground"
else
    echo "  kase gateway install # Install gateway service (messaging + cron)"
fi
echo "  kase cron list     # View scheduled jobs"
echo "  kase doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m kase_cli.main setup
fi
