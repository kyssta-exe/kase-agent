...
"""
    ...
    assert "Tyler Williams > Active Projects: Kase" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `kase cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.kase/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Kase``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Kase prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Kase to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Kase config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Kase uses tools well"
    # All-lowercase matches → lowercase ``kase``; this preserves the
    # real filesystem path ``~/.kase`` (Kase home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "kase should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Kase remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "kase prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Kase was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "kase uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Kase said to ask Kase about Kase settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.kase`` (the real Kase home), not the broken
    ``.Kase``.
    ...
    rewritten to ``~/.Kase/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.kase/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .kase directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.kase'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "kase config path: ~/.kase/"
