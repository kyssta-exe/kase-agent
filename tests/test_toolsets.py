...
class TestToolsetConsistency:
    ...
    def test_hermes_platforms_share_core_tools(self):
        """All kase-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on kase-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["kase-cli", "kase-telegram", "kase-discord", "kase-whatsapp", "kase-slack", "kase-signal", "kase-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_hermes_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("kase-whatsapp")

    def test_hermes_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("kase-api-server")
