"""Tests for kase_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Kase entry point
(kase, kase, kase-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Kase entry point imports kase_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported kase_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("kase_bootstrap", None)
    import kase_bootstrap  # noqa: WPS433
    return kase_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Kase CLI
...
class TestEntryPointsImportBootstrap:
    """Every Kase entry point must import kase_bootstrap as its
    ...
    # Entry points that invoke Kase as a process.  Each one must
    # import kase_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "kase_cli/main.py",   # kase CLI (console_script)
        "run_agent.py",          # kase (console_script)
        "acp_adapter/entry.py",  # kase-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import kase_bootstrap' and that
        ...
        half-finished ``kase update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``kase_bootstrap`` as a top-level module) leaves kase
        ...
        """
        # Resolve relative to the kase repo root.  Tests live
        # at tests/test_kase_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # kase start even when kase_bootstrap hasn't been
        ...
        assert first_import_name == "kase_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'kase_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import kase_bootstrap' line to be the first import."
        )
