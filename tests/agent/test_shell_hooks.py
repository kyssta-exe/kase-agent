...
class TestCallbackSubprocess:
    ...
    def test_block_translation_end_to_end(self, tmp_path):
        ...
        must translate it to the canonical Kase block shape so that
    ...
    def test_block_aggregation_through_plugin_manager(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KASE_ACCEPT_HOOKS", "1")
...
class TestAllowlistConcurrency:
    ...
    def test_save_allowlist_failure_logs_actionable_warning(
        ...
        re-prompt consequence so "kase keeps asking" is debuggable."""
        import logging
        monkeypatch.setenv("KASE_HOME", str(tmp_path / "home"))
        monkeypatch.setattr(
    ...
    def test_command_script_path_resolution(self):
        ...
        ``kase hooks doctor``'s executability check and silently
