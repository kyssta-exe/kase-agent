...
def hermes_home(monkeypatch):
    """Isolate KASE_HOME for ``reload_skills`` tests.
    ...
    module-level ``KASE_HOME`` / ``SKILLS_DIR`` constants in place so the
    isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="kase-reload-skills-")
    monkeypatch.setenv("KASE_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "KASE_HOME", home, raising=False)
