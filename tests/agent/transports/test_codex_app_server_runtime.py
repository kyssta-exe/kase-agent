...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Kase passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("KASE_HOME", "/users/alice/.kase/profiles/backend-worker")
        monkeypatch.setenv("KASE_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "KASE_KANBAN_DB",
            "/users/alice/.kase/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.kase/kanban/boards/smoke"]'
            in cmd
        )
