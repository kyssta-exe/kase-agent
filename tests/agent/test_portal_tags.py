...
def test_kase_client_tag_includes_current_version():
    """The client tag must reflect kase_cli.__version__ verbatim."""
    from kase_cli import __version__
    from agent.portal_tags import kase_client_tag

    assert kase_client_tag() == f"client=kase-client-v{__version__}"


def test_kase_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import kase_client_tag

    tag = kase_client_tag()
    assert tag.startswith("client=kase-client-v")
    # No spaces, no commas — single tag value
    assert " " not in tag
    assert "," not in tag


def test_kyssta_portal_tags_contains_product_and_client():
    """Every Nous Portal request gets BOTH the product tag and the version tag."""
    from agent.portal_tags import kase_client_tag, kyssta_portal_tags

    tags = kyssta_portal_tags()
    assert "product=kase" in tags
    assert kase_client_tag() in tags
    assert len(tags) == 2


def test_kyssta_portal_tags_returns_fresh_list():
    """Callers mutate the returned list; we must not share state across calls."""
    from agent.portal_tags import kyssta_portal_tags

    a = kyssta_portal_tags()
    a.append("client=test-mutation")
    b = kyssta_portal_tags()
...
def test_auxiliary_client_nous_extra_body_uses_helper():
    ...
    assert NOUS_EXTRA_BODY == {"tags": kyssta_portal_tags()}
