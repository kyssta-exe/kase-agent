"""Tests for kase_constants module."""
...
class TestGetDefaultHermesRoot:
    ...
    def test_no_kase_home_returns_native(self, tmp_path, monkeypatch):
        """When KASE_HOME is not set, returns ~/.kase."""
        monkeypatch.delenv("KASE_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_hermes_root() == tmp_path / ".kase"

    def test_kase_home_is_native(self, tmp_path, monkeypatch):
        """When KASE_HOME = ~/.kase, returns ~/.kase."""
        native = tmp_path / ".kase"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KASE_HOME", str(native))
        assert get_default_hermes_root() == native

    def test_kase_home_is_profile(self, tmp_path, monkeypatch):
        """When KASE_HOME is a profile under ~/.kase, returns ~/.kase."""
        native = tmp_path / ".kase"
    ...
    def test_kase_home_is_docker(self, tmp_path, monkeypatch):
        """When KASE_HOME points outside ~/.kase (Docker), returns KASE_HOME."""
    ...
    def test_kase_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any KASE_HOME outside ~/.kase is treated as the root."""
        custom = tmp_path / "my-kase-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (KASE_HOME=<root>/profiles/<name>),
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(kase_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(kase_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".kase"
