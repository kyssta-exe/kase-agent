...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves kase_cli.kanban_db on its own
  ...
  - worker logs are captured to KASE_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "KASE_KANBAN_TASK": task.id,
            "KASE_KANBAN_WORKSPACE": workspace,
...
def main():
    ...
    # Point the `kase` CLI child processes will run at the worktree
    # kase_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "kase")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m kase_cli.main "$@"
""")
