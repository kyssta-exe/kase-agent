...
See: https://github.com/Kyssta/kase/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for kase_constants.get_subprocess_home()."""

    def test_returns_none_when_kase_home_unset(self, monkeypatch):
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KASE_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("KASE_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from kase_constants import (
            get_kase_home,
            reset_kase_home_override,
            set_kase_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_kase_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_kase_home_override(profile)
        try:
            assert get_kase_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_kase_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_kase_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_no_injection_when_kase_home_unset(self, monkeypatch):
        monkeypatch.delenv("KASE_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["KASE_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from kase_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from kase_cli.profiles import create_profile
