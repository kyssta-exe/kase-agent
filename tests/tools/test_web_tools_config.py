...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.web_tools.managed_kyssta_tools_enabled", return_value=True),
            patch("tools.managed_tool_gateway.managed_kyssta_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("tools.web_tools.Firecrawl"):
            with patch("tools.web_tools._read_kyssta_access_token", return_value=None):
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "kyssta.com"}):
            with patch("tools.web_tools._read_kyssta_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    from tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.kyssta.com",
                    )
    ...
    def test_tool_gateway_scheme_can_switch_derived_gateway_origin_to_http(self):
        ...
        with patch.dict(os.environ, {
            "TOOL_GATEWAY_DOMAIN": "kyssta.com",
            ...
            with patch("tools.web_tools._read_kyssta_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="http://firecrawl-gateway.kyssta.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_nous_auth_token_respects_kase_home_override(self, tmp_path):
        """Auth lookup should read from KASE_HOME/auth.json, not ~/.kase/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".kase").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "KASE_HOME": str(hermes_home),
        }, clear=False):
            import tools.web_tools
            importlib.reload(tools.web_tools)
            assert tools.web_tools._read_kyssta_access_token() == "nous-token"
...
class TestBackendSelection:
    ...
    ``kase tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
