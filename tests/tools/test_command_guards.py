...
def _clean_state():
    ...
    saved = {}
    for k in ("KASE_INTERACTIVE", "KASE_GATEWAY_SESSION", "HERMES_EXEC_ASK", "KASE_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["KASE_INTERACTIVE"] = "1"
        session_key = os.getenv("KASE_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No KASE_INTERACTIVE or KASE_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
