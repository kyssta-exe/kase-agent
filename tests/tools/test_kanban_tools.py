...
Verifies:
  - Tools are gated on KASE_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `kase chat` sessions (no KASE_KANBAN_TASK) must have
    zero kanban_* tools in their schema."""
    monkeypatch.delenv("KASE_KANBAN_TASK", raising=False)
    home = tmp_path / ".kase"
    home.mkdir()
    ...
    schema = registry.get_definitions(set(resolve_toolset("kase-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("KASE_KANBAN_TASK", "t_fake")
    home = tmp_path / ".kase"
    home.mkdir()
    monkeypatch.setenv("KASE_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the KASE_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: KASE_HOME isolated, KASE_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".kase"
    home.mkdir()
    monkeypatch.setenv("KASE_HOME", str(home))
    monkeypatch.setenv("HERMES_PROFILE", "test-worker")
    monkeypatch.delenv("KASE_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("KASE_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["kase_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("KASE_SESSION_ID", "session-trusted")
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    under an authoritative-looking name like ``kase-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "kase-system",
    })
    ...
    try:
        ...
        # caller-supplied "kase-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("KASE_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from kase_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no KASE_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (KASE_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has KASE_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no KASE_KANBAN_TASK in env) are intentionally
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("KASE_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("KASE_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no KASE_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via KASE_KANBAN_BOARD env var,
...
# restarting Kase. These tests pin that ``board=<slug>`` argument
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning KASE_KANBAN_TASK
