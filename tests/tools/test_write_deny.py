...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_env(self):
        # ``.env`` under the active KASE_HOME (profile-aware, not just
        # ``~/.kase``) must be write-denied. The hermetic test conftest
        # points KASE_HOME at a tempdir — resolve via get_kase_home()
        # to match the denylist.
        from kase_constants import get_kase_home
        path = str(get_kase_home() / ".env")
        assert _is_write_denied(path) is True
    ...
    def test_hermes_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.kase/.env`` (whose credentials are inherited by every profile)
        ...
        # Sanity check: KASE_HOME does point to the profile dir, not the root.
        from kase_constants import get_kase_home, get_default_hermes_root
        assert get_kase_home() == profile_home
...
class TestWriteAllowed:
    ...
    def test_hermes_config_not_env(self):
        path = os.path.join(str(Path.home()), ".kase", "config.yaml")
