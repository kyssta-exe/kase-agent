...
def _restore_tool_modules():
    original_kase_home = os.environ.get("KASE_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_kase_home is None:
            os.environ.pop("KASE_HOME", None)
        else:
            os.environ["KASE_HOME"] = original_kase_home
        _reset_modules(("tools", "kase_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    kase_cli = types.ModuleType("kase_cli")
    kase_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["kase_cli"] = kase_cli
    hermes_home = tmp_path / "kase-home"
    os.environ["KASE_HOME"] = str(hermes_home)
    sys.modules["kase_cli.config"] = types.SimpleNamespace(
        get_kase_home=lambda: hermes_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="kase", create_if_missing=create_if_missing)
