...
def test_no_backend_message_mentions_fal_signup_and_plugins(monkeypatch):
    ...
    monkeypatch.setattr(
        image_generation_tool, "managed_kyssta_tools_enabled", lambda: False
    )
    ...
    assert "kase tools" in msg or "kase plugins" in msg
...
def test_no_backend_message_mentions_managed_gateway_when_enabled(monkeypatch):
    ...
    monkeypatch.setattr(
        image_generation_tool, "managed_kyssta_tools_enabled", lambda: True
    )

    msg = image_generation_tool._build_no_backend_setup_message()

    assert "managed FAL gateway" in msg
    assert "Nous account" in msg or "kase setup" in msg
