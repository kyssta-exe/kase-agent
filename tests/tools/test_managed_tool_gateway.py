...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        ...
        clear=False,
    ), patch.object(managed_tool_gateway, "managed_kyssta_tools_enabled", return_value=True):
    ...
    assert result.gateway_origin == "https://firecrawl-gateway.kyssta.com"
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "kyssta.com"}, clear=False), \
         patch.object(managed_tool_gateway, "managed_kyssta_tools_enabled", return_value=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "nous-token",
        )
...
def test_read_kyssta_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kase_cli.auth.resolve_kyssta_access_token",
        lambda refresh_skew_seconds=120: "fresh-token",
    )

    assert managed_tool_gateway.read_kyssta_access_token() == "fresh-token"
