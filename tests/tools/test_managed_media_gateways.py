...
def _enable_managed_nous_tools(monkeypatch):
    """Patch the source modules so managed_kyssta_tools_enabled() returns True
    even after tool modules are dynamically reloaded."""
    monkeypatch.setattr("kase_cli.auth.get_kyssta_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr("kase_cli.models.check_kyssta_free_tier", lambda: False)
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "kyssta.com")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "nous-token")
    ...
    assert captured["base_url"] == "https://openai-audio-gateway.kyssta.com/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.kyssta.com/v1"
