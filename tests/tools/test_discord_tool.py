...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in kase-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_hermes_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["kase-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["kase-discord"]["tools"]
    ...
    def test_discord_tools_not_in_other_toolsets(self):
        ...
        for name, ts in TOOLSETS.items():
            if name in {"kase-discord", "kase-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("kase_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["kase-discord"], quiet_mode=True)
