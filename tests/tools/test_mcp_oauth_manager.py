...
def test_hermes_provider_subclass_exists():
    """KaseMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _KASE_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _KASE_PROVIDER_CLS is not None
    assert issubclass(_KASE_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_hermes_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns KaseMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _KASE_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("KASE_HOME", str(tmp_path))
    ...
    assert _KASE_PROVIDER_CLS is not None
    assert isinstance(provider, _KASE_PROVIDER_CLS)
