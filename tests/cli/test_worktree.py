...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("kase/kase-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned kase/* and pr-* branches."""

    def test_prunes_orphaned_hermes_branch(self, git_repo):
        """kase/kase-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "kase/kase-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "kase/kase-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "kase/kase-deadbeef" in result.stdout
        ...
        assert "kase/kase-deadbeef" in orphaned
        ...
        assert "kase/kase-deadbeef" not in result.stdout
