...
from kase_cli import main as hermes_main
...
def test_kase_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in KaseCLI.__init__")

    monkeypatch.setattr("kase_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("kase_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.KaseCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("kase_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    shell = cli.KaseCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "kase_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.KaseCLI(compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "kase_cli.auth.get_provider_auth_state",
    ...
    monkeypatch.setattr(
        "kase_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("kase_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("kase_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("kase_cli.auth._update_config_for_provider", lambda provider, url: None)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.KaseCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.KaseCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("kase_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("kase_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("kase_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("kase_cli.auth.resolve_provider", _resolve_provider)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "kase_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("kase_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("kase_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("kase_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("kase_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "kase_cli.models.probe_api_models",
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("kase_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "kase_cli.main._save_custom_provider",
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("kase_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("kase_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("kase_cli.auth.get_provider_auth_state", lambda provider_id: None)
    ...
    monkeypatch.setattr("kase_cli.auth._login_nous", _fake_login)

    hermes_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.kyssta.com",
            inference_url="https://inference.kyssta.com/v1",
            client_id="kase-local",
...
def test_auto_provider_name_localhost():
    from kase_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from kase_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "kase_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("kase_cli.config.save_config", _save)
