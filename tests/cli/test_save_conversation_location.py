...
Regression: the old implementation wrote ``kase_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``kase sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.kase/sessions/saved/`` and prints
...
def hermes_home(tmp_path, monkeypatch):
    ...
    if hasattr(kase_constants, "_kase_home_cache"):
        kase_constants._kase_home_cache = None
    return home
...
def test_save_conversation_writes_under_kase_home(hermes_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.kase/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the KASE_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "kase_constants"]:
    ...
    cli.KaseCLI.save_conversation(stub)

    # File must NOT be in CWD
    cwd_leak = list(work.glob("kase_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.kase/sessions/saved/
    saved_dir = hermes_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("kase_conversation_*.json"))
    ...
    assert "kase --resume 20260101_120000_abc123" in out, out
