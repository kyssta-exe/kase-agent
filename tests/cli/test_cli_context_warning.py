...
def _isolate(tmp_path, monkeypatch):
    """Isolate KASE_HOME so tests don't touch real config."""
    home = tmp_path / ".kase"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal KaseCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = KaseCLI.__new__(KaseCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["kase-core"]
        obj.compact = False
