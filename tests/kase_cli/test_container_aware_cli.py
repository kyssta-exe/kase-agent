...
def container_env(tmp_path, monkeypatch):
    """Set up a fake KASE_HOME with .container-mode file."""
    hermes_home = tmp_path / ".kase"
    hermes_home.mkdir()
    monkeypatch.setenv("KASE_HOME", str(hermes_home))
    monkeypatch.delenv("KASE_DEV", raising=False)
    ...
    container_mode.write_text(
        ...
        "container_name=kase\n"
        "exec_user=kase\n"
        "hermes_bin=/data/current-package/bin/kase\n"
    )
    return hermes_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("kase_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "kase"
    assert info["exec_user"] == "kase"
    assert info["hermes_bin"] == "/data/current-package/bin/kase"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("kase_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_hermes_dev(container_env, monkeypatch):
    """Returns None when KASE_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("KASE_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_hermes_dev_zero(container_env, monkeypatch):
    """KASE_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("KASE_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch("kase_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_kase_home": lambda: hermes_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KASE_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=kase-custom\n"
        "exec_user=myuser\n"
        "hermes_bin=/opt/kase/bin/kase\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "kase-custom"
    assert info["exec_user"] == "myuser"
    assert info["hermes_bin"] == "/opt/kase/bin/kase"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from kase_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "kase"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "kase" in cmd
    assert "/data/current-package/bin/kase" in cmd
    assert "chat" in cmd
