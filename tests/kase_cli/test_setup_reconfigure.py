...
- Bare `kase setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `kase setup --quick` runs the narrower "fill in missing items" flow.
- `kase setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("kase_cli.setup.ensure_kase_home", {}),
        ("kase_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("kase_cli.config.is_managed", {"return_value": False}),
        ("kase_cli.setup.load_config", {"return_value": {}}),
        ("kase_cli.setup.save_config", {}),
        ("kase_cli.setup.get_env_value", {"return_value": None}),
        ("kase_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("kase_cli.setup._print_setup_summary", {}),
        ("kase_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("kase_cli.setup.load_config", {"return_value": {}}),
        ("kase_cli.setup.save_config", {}),
        ("kase_cli.auth.get_active_provider", {"return_value": None}),
        ("kase_cli.setup.get_env_value", {"return_value": None}),
        ("kase_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `kase setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="kase_cli.setup.prompt_choice",
                quick="kase_cli.setup._run_quick_setup",
                model="kase_cli.setup.setup_model_provider",
                terminal="kase_cli.setup.setup_terminal_backend",
                agent="kase_cli.setup.setup_agent_settings",
                gateway="kase_cli.setup.setup_gateway",
                tools="kase_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`kase setup --reconfigure` behaves the same as bare `kase setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("kase_cli.setup.prompt_choice", {"return_value": 0}),
                first="kase_cli.setup._run_first_time_quick_setup",
            )
            from kase_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "kase_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["kase", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["kase", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["kase", "setup"])
        try:
            main()
        except SystemExit:
            pass
