...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "kase" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    entry = next(item for item in entries if item["source"] == "manual:kase_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:kase_pkce"
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kase_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `kase auth add nous` must also populate providers.nous so the
    # 401-recovery path (resolve_kyssta_runtime_credentials) can mint a fresh
    # agent_key when the 24h TTL expires. If this mirror is missing, recovery
    # raises "Kase is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kase_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`kase auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kase_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from kase_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from kase_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`kase logout --provider openai-codex` must still clear model.provider.
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `kase logout` should target configured Codex if auth has no active provider."""
    hermes_home = tmp_path / "kase"
    monkeypatch.setenv("KASE_HOME", str(hermes_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (hermes_home / "config.yaml").write_text(
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from kase_cli.auth import _reset_config_provider
    ...
    with patch("kase_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from kase_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "kase_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("kase_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("kase_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("kase_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "kase_cli.auth_commands.time.time",
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from kase_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from kase_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `kase auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("KASE_HOME", str(tmp_path / "kase"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    # Pre-existing suppression (simulating a prior `kase auth remove`)
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.auth._import_codex_cli_tokens", _fake_import)
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`kase auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.kase/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.kase/.env (not the shell), the
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `kase auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from kase_cli.auth import is_source_suppressed
    from kase_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `kase auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Kase reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import kase_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_kase_pkce_suppression(tmp_path, monkeypatch):
    """anthropic kase_pkce must not re-seed from ~/.kase/.anthropic_oauth.json when suppressed."""
    hermes_home = tmp_path / "kase"
    hermes_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("KASE_HOME", str(hermes_home))
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["kase_pkce"]},
    }))

    # Stub the readers so only kase_pkce is "available"; claude_code returns None
    ...
    # kase_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "kase_pkce" not in active
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `kase auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `kase auth add openai-codex`)
