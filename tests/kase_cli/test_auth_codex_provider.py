"""Tests for Codex auth — tokens stored in Kase auth store (~/.kase/auth.json)."""
...
def _setup_hermes_auth(hermes_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Kase auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Kase auth store, not ~/.codex/."""
    hermes_home = tmp_path / "kase"
    codex_home = tmp_path / "codex-cli"
    hermes_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "kase-at", "refresh_token": "kase-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Kase store
    assert not (codex_home / "auth.json").exists()

    # Kase auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "kase-at"
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "kase_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("kase_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("kase_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
