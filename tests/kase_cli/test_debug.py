"""Tests for ``kase debug`` CLI command and debug utilities."""
...
def hermes_home(tmp_path, monkeypatch):
    """Set up an isolated KASE_HOME with minimal logs."""
    home = tmp_path / ".kase"
    home.mkdir()
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from kase_cli.debug import _upload_paste_rs
        ...
        with patch("kase_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "kase_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from kase_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from kase_cli.debug import upload_to_pastebin

        with patch("kase_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("kase_cli.debug._upload_paste_rs",
             ...
             patch("kase_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, hermes_home):
        from kase_cli.debug import _capture_log_snapshot
...
class TestCaptureLogSnapshotRedaction:
    ...
    def hermes_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated KASE_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".kase"
        home.mkdir()
        monkeypatch.setenv("KASE_HOME", str(home))
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from kase_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, hermes_home):
        from kase_cli.debug import collect_debug_report

        with patch("kase_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- kase dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- kase dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, hermes_home):
        from kase_cli.debug import collect_debug_report

        with patch("kase_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, hermes_home, capsys):
        ...
        with patch("kase_cli.dump.run_dump"), \
             patch("kase_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("kase_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, hermes_home, capsys):
        ...
        with patch("kase_cli.dump.run_dump"), \
             patch(
                 "kase_cli.debug._sweep_expired_pastes",
    ...
    def test_share_uploads_three_pastes(self, hermes_home, capsys):
        ...
        with patch("kase_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- kase dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, hermes_home, capsys):
        ...
        from kase_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("kase_cli.dump.run_dump"), \
             patch("kase_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("kase_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        ...
        with patch("kase_cli.dump.run_dump"), \
             patch("kase_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("kase_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from kase_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from kase_cli.debug import delete_paste
        ...
        with patch("kase_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.kase/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``kase debug``
    invocation.
    """
    ...
    def test_does_not_spawn_subprocess(self, hermes_home):
        ...
        import ast
        import inspect
        from kase_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, hermes_home):
        ...
        from kase_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_merges_with_existing_pending(self, hermes_home):
        ...
        from kase_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, hermes_home):
        ...
        with patch("kase_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, hermes_home):
        from kase_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("kase_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, hermes_home):
        ...
        with patch("kase_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
    ...
    def test_run_debug_survives_sweep_failure(self, hermes_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: kase debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from kase_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("kase_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("kase_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, hermes_home, capsys):
        ...
        with patch("kase_cli.dump.run_dump"), \
             ...
             patch("kase_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, hermes_home, capsys):
        ...
        with patch("kase_cli.dump.run_dump"), \
             patch("kase_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("kase_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
