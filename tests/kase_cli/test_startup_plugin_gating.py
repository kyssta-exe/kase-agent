...
``kase_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``kase --help``, ``kase version``,
``kase logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``kase --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from kase_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["kase", "-w", "--tui"], None),
        (["kase", "version"], "version"),
        (["kase", "--tui", "chat"], "chat"),
        (["kase", "-w", "logs"], "logs"),
        (["kase", "chat", "hello world"], "chat"),
        (["kase", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["kase", "-m", "gpt5", "chat"], "chat"),
        (["kase", "--model", "gpt5", "chat", "hi"], "chat"),
        (["kase", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["kase", "-z", "hello world"], None),
        (["kase", "-z", "hello", "chat"], "chat"),
        (["kase", "--model=gpt5", "chat"], "chat"),     # inline form
        (["kase", "--", "chat"], "chat"),               # -- terminator
        (["kase", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["kase", "some-plugin-cmd"], "some-plugin-cmd"),
        (["kase", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["kase"],                          # bare → chat
        ["kase", "--help"],                # top-level help
        ["kase", "-h"],
        ["kase", "version"],               # known built-in
        ["kase", "logs"],
        ["kase", "gateway", "run"],
        ["kase", "--tui"],
        ["kase", "-w", "--tui"],
        ["kase", "chat", "hi"],
        ["kase", "help"],                  # accepted built-in-ish
        ["kase", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["kase", "meet", "join"],          # potential google_meet plugin
        ["kase", "honcho", "status"],      # potential memory plugin
        ["kase", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``kase help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"kase_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
