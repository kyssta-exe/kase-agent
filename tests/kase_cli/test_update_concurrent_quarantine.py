"""Tests for issue #26670 — concurrent kase.exe detection and improved
quarantine retry / reboot-deferred fallback during `kase update` on Windows.
...
from kase_cli import main as cli_main


# Tests in this module either exercise the REAL _detect_concurrent_hermes_instances
# helper (and need the autouse stub in tests/kase_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "kase.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "kase.exe").write_bytes(b"")
    (scripts_dir / "kase-gateway.exe").write_bytes(b"")
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "kase.exe")]
...
def test_detect_concurrent_finds_other_hermes_process(_winp, tmp_path):
    ...
    procs = [
        _make_proc(other_pid, str(shim), "kase.exe"),
    ...
    assert result == [(other_pid, "kase.exe")]
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    # Simulate the desktop spawning kase.EXE (uppercase ext) from same path
    upper = str(shim).replace("kase.exe", "HERMES.EXE")
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    matches = [(1234, "kase.exe"), (5678, "kase.exe")]
    msg = cli_main._format_concurrent_instances_message(matches, tmp_path)

    assert "1234" in msg
    assert "5678" in msg
    assert "kase.exe" in msg
    assert "Kase Desktop" in msg
    assert "--force" in msg
    # Mentions the file that would have been overwritten
    assert str(tmp_path / "kase.exe") in msg
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    assert orig == shim
    assert quarantine.name.startswith("kase.exe.old.")
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Kase Desktop" in captured or "gateway" in captured.lower()
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another kase.exe is running, the update bails out before
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "kase.exe")])
