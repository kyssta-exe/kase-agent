"""Tests for kase_cli/completion.py — shell completion script generation."""
...
from kase_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real kase structure."""
    p = argparse.ArgumentParser(prog="kase")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "complete -F _hermes_completion kase" in out
...
class TestGenerateZsh:
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _hermes kase' in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[kase]}} == _hermes ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c kase -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from kase_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        ...
        assert 'profiles_dir="$HOME/.kase/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'kase profile use', complete with profile names."""
        out = generate_bash(_make_parser())
    ...
    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_hermes_profiles()" in out
        assert "$HOME/.kase/profiles" in out
