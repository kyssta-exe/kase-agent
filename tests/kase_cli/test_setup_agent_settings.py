...
from kase_cli.setup import setup_agent_settings
...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("kase_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("kase_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("kase_cli.setup.save_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("kase_cli.setup.remove_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("kase_cli.setup.save_config", lambda *args, **kwargs: None)
...
def test_setup_agent_settings_prefers_config_over_stale_env(tmp_path, monkeypatch, capsys):
    ...
    from an old `kase setup` run shadowed `agent.max_turns: 500` in
    ...
    monkeypatch.setattr(
        "kase_cli.setup.remove_env_value",
