...
from kase_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from kase_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from kase_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_expose_hermes_tools_writes_callback_mcp_entry(self, tmp_path):
        ...
        [mcp_servers.kase-tools] entry is written so codex calls back
        into Kase for browser/web/delegate_task/vision/memory tools.

        This is the fix for 'all other tools that codex doesn't provide
        should be useable by kase' — quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_hermes_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.kase-tools]" in text
        assert "kase_tools_mcp_server" in text
        ...
        assert "kase-tools" in report.migrated
    ...
    def test_expose_hermes_tools_disabled_skips_entry(self, tmp_path):
        ...
        migrate({}, codex_home=tmp_path,
                ...
                expose_hermes_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.kase-tools]" not in text
        assert "kase_tools_mcp_server" not in text
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Kase appends
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Kase' managed block, re-running
        ...
        migrate({"mcp_servers": {"kase-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.kase-mcp]" in final
...
class TestStripUnmanagedPluginTables:
    ...
    ``kase codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "kase_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: KASE_HOME tempdir leak into ~/.codex/config.toml ----


class TestKaseHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``KASE_HOME``
    ...
    once codex spawned the kase-tools MCP subprocess.
    """
    ...
    def test_tempdir_detector_accepts_real_kase_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.kase")
        assert not _looks_like_test_tempdir("/home/bob/.kase")
        assert not _looks_like_test_tempdir("/opt/kase")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when KASE_HOME points at a pytest
        ...
        env = entry.get("env", {})
        assert "KASE_HOME" not in env, (
            f"pytest-tempdir KASE_HOME leaked into codex MCP entry: "
            f"{env.get('KASE_HOME')!r}"
        )

    def test_real_kase_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate KASE_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.kase"
        monkeypatch.setenv("KASE_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("KASE_HOME") == real_path

    def test_unset_kase_home_omits_env_key(self, monkeypatch):
        """When KASE_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever KASE_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "KASE_HOME" not in env, (
            f"KASE_HOME should not be set when env var is unset, got: "
            f"{env.get('KASE_HOME')!r}"
        )
