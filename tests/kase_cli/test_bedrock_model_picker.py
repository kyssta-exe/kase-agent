...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (KASE_OVERLAYS) — bedrock
...
class TestProviderModelIdsBedrock:
    ...
    def test_falls_back_to_static_list_when_discovery_empty(self, monkeypatch):
        ...
        from kase_cli.models import _PROVIDER_MODELS, provider_model_ids
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via KASE_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in KASE_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from kase_cli.providers import KASE_OVERLAYS
        assert "bedrock" in KASE_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from kase_cli.providers import KASE_OVERLAYS
        assert KASE_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from kase_cli.providers import KASE_OVERLAYS
        assert KASE_OVERLAYS["bedrock"].auth_type == "aws_sdk"
