"""Tests for the top-level `./kase` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./kase` should use `kase_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "kase"
    called = []

    fake_main_module = types.ModuleType("kase_cli.main")

    def fake_main():
        called.append("kase_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "kase_cli.main", fake_main_module)
