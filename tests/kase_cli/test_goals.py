"""Tests for kase_cli/goals.py — persistent cross-turn goals."""
...
def hermes_home(tmp_path, monkeypatch):
    """Isolated KASE_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves KASE_HOME.
    from kase_cli import goals
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from kase_cli.goals import _parse_judge_response
...
class TestJudgeGoal:
    def test_empty_goal_skipped(self):
        from kase_cli.goals import judge_goal
...
class TestGoalManager:
    ...
    def test_continuation_prompt_shape(self, hermes_home):
        ...
        mgr.set("port goal command to kase")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to kase" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from kase_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, hermes_home):
        ...
        from kase_cli import goals
        from kase_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from kase_cli import goals
        from kase_cli.goals import GoalManager, load_goal
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        import json
        from kase_cli.goals import GoalState
