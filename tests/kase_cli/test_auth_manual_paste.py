...
Regression coverage for [#26923](https://github.com/Kyssta/kase/issues/26923):
...
def test_xai_loopback_login_manual_paste_skips_http_server(monkeypatch):
    ...
    def _fake_prompt(_redirect_uri):
        # Kase generates state internally; we won't know it ahead of
        # time, so capture the state Kase baked into the authorize
