"""Tests for the kase_cli models module."""

from unittest.mock import patch, MagicMock

from kase_cli.models import (
    OPENROUTER_MODELS, fetch_openrouter_models, model_ids, detect_provider_for_model,
    is_kyssta_free_tier, partition_kyssta_models_by_tier,
    check_kyssta_free_tier, _FREE_TIER_CACHE_TTL,
    union_with_portal_free_recommendations,
    union_with_portal_paid_recommendations,
)
import kase_cli.models as _models_mod
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("kase_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    def test_live_fetch_recomputes_free_tags(self, monkeypatch):
        ...
        with patch("kase_cli.models.urllib.request.urlopen", return_value=_Resp()):
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with patch("kase_cli.models.urllib.request.urlopen", side_effect=OSError("boom")):
            models = fetch_openrouter_models(force_refresh=True)
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        kase is tool-calling-first — surfacing a non-tool model leads to
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from kase_cli.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from kase_cli.models import _find_openrouter_slug
        with patch("kase_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestPartitionNousModelsByTier:
    """Tests for partition_kyssta_models_by_tier — free vs paid tier model split."""
    ...
    def test_paid_tier_all_selectable(self):
        ...
        sel, unav = partition_kyssta_models_by_tier(models, pricing, free_tier=False)
        assert sel == models
        assert unav == []
    ...
    def test_free_tier_splits_correctly(self):
        ...
        sel, unav = partition_kyssta_models_by_tier(models, pricing, free_tier=True)
    ...
    def test_no_pricing_returns_all(self):
        ...
        sel, unav = partition_kyssta_models_by_tier(models, {}, free_tier=True)
        assert sel == models
        assert unav == []
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "kase_cli.models.fetch_nous_recommended_models",
    ...
    def test_then_partition_keeps_portal_free_model(self):
        ...
        with patch(
            ...
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        sel, unav = partition_kyssta_models_by_tier(ids, p, free_tier=True)
    ...
    def test_empty_payload_returns_inputs_unchanged(self):
        ...
        with patch("kase_cli.models.fetch_nous_recommended_models", return_value={}):
            ids, p = union_with_portal_free_recommendations(curated, pricing, "")
        assert ids == curated
        assert p == pricing
...
class TestUnionWithPortalPaidRecommendations:
    ...
    def test_does_not_synthesize_pricing_for_paid_models(self):
        ...
        :func:`partition_kyssta_models_by_tier` into treating them as free;
...
class TestCheckNousFreeTierCache:
    """Tests for the TTL cache on check_kyssta_free_tier()."""
    ...
    @patch("kase_cli.models.fetch_kyssta_account_tier")
    @patch("kase_cli.models.is_kyssta_free_tier", return_value=True)
    def test_result_is_cached(self, mock_is_free, mock_fetch):
        ...
        with patch("kase_cli.auth.get_provider_auth_state", return_value={"access_token": "tok"}), \
             patch("kase_cli.auth.resolve_kyssta_runtime_credentials"):
            result1 = check_kyssta_free_tier()
            result2 = check_kyssta_free_tier()
    ...
    @patch("kase_cli.models.fetch_kyssta_account_tier")
    @patch("kase_cli.models.is_kyssta_free_tier", return_value=False)
...
class TestNousRecommendedModels:
    ...
    def test_fetch_caches_per_portal_url(self):
        from kase_cli.models import fetch_nous_recommended_models
    ...
    def test_fetch_cache_is_keyed_per_portal(self):
        ...
        with patch("urllib.request.urlopen", return_value=mock_cm) as mock_urlopen:
            ...
            fetch_nous_recommended_models("https://portal.staging-kyssta.com")
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("kase_cli.models.fetch_nous_recommended_models", return_value=payload):
    ...
    def test_auto_detects_tier_when_not_supplied(self):
        """Default behaviour: call check_kyssta_free_tier() to pick the tier."""
        from kase_cli.models import get_nous_recommended_aux_model
        payload = {
        ...
        with (
            patch("kase_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("kase_cli.models.check_kyssta_free_tier", return_value=True),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "free-model"
        with (
            patch("kase_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("kase_cli.models.check_kyssta_free_tier", return_value=False),
        ):
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("kase_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("kase_cli.models.check_kyssta_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_nous_recommended_aux_model(vision=False) == "paid-model"
