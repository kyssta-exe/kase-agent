...
Complements tests/kase_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/kase_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def test_per_task_max_retries_overrides_dispatcher_limit(kanban_home, all_assignees_spawnable):
    ...
    Three-tier resolution order:
      1. ``task.max_retries`` (set via ``create_task(max_retries=N)`` /
         ``kase kanban create --max-retries N``)
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.kase/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("KASE_HOME", raising=False)
    profiles = tmp_path / ".kase" / "profiles"
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom KASE_HOME root."""
...
def test_parse_duration_accepts_formats():
    from kase_cli.kanban import _parse_duration
...
def test_cli_create_max_runtime_via_duration(kanban_home):
    """`kase kanban create --max-runtime 2h` should persist 7200 seconds."""
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: kase_cli.main doesn't propagate sub-command exit codes
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "kase_cli.main", "kanban",
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    """First CLI action on an empty KASE_HOME must not error with
    ...
    env = {**os.environ, "KASE_HOME": str(home),
           "PYTHONPATH": str(worktree_root)}
    r = _sp.run(
        [_sys.executable, "-m", "kase_cli.main", "kanban",
...
def test_connect_auto_inits_fresh_db(tmp_path, monkeypatch):
    """Calling connect() on a fresh KASE_HOME must create the
...
def test_cli_show_json_carries_runs(kanban_home):
    """kase kanban show --json must include runs[] so scripts that
...
def test_build_worker_context_renders_author_with_safe_framing(kanban_home):
    ...
    (e.g. "kase-system", "operator") can't be misread as a system
    directive above the comment body. Defense-in-depth — see #22452."""
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="t", assignee="worker")
        kb.add_comment(conn, tid, author="kase-system", body="some note")
        ctx = kb.build_worker_context(conn, tid)

        # No bold-author rendering anywhere in the context.
        assert "**kase-system**" not in ctx
        # Explicit provenance prefix is present.
        assert "comment from worker `kase-system` at " in ctx
...
def test_default_spawn_auto_loads_kanban_worker_skill(kanban_home, monkeypatch):
    ...
    kase subprocess (which would hang trying to call an LLM).
    """
    # Pretend the bundled kanban-worker skill resolves for this isolated
    # KASE_HOME — the fixture creates an empty tmpdir without the
    ...
    env = captured["env"]
    assert env.get("KASE_KANBAN_TASK") == tid
...
def test_cli_create_skill_flag_repeatable(kanban_home):
    """`kase kanban create --skill a --skill b` persists the list."""
    out = run_slash(
...
def test_cli_show_renders_skills(kanban_home):
    """`kase kanban show <id>` prints a skills row when present."""
    out = run_slash(
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from kase_cli import kanban as kb_cli
...
def test_check_dispatcher_presence_warns_when_no_gateway(monkeypatch):
    ...
    assert "kase gateway start" in msg
...
def test_cli_create_warns_when_no_gateway(kanban_home, monkeypatch, capsys):
    ...
    assert "kase gateway start" in captured.err
...
def test_cli_create_silent_when_gateway_up(kanban_home, monkeypatch, capsys):
    ...
    assert kb_cli._cmd_create(ns) == 0
    captured = capsys.readouterr()
    assert "kase gateway start" not in captured.err
...
def test_cli_create_no_warn_on_triage(kanban_home, monkeypatch, capsys):
    ...
    assert "kase gateway start" not in err
...
def test_cli_daemon_without_force_prints_deprecation_exits_2(kanban_home, capsys):
    """`kase kanban daemon` (no --force) is a deprecation stub."""
    from kase_cli import kanban as kb_cli
    ns = argparse.Namespace(
    ...
    assert "kase gateway start" in err
...
def test_cli_daemon_help_marks_deprecated():
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `kase kanban --help` output"
    )
...
def test_gateway_dispatcher_watcher_respects_config_flag_off(monkeypatch):
    ...
    import kase_cli.config as _cfg_mod
...
def test_gateway_dispatcher_watcher_respects_env_override(monkeypatch):
    """KASE_KANBAN_DISPATCH_IN_GATEWAY=0 disables without touching config."""
    import asyncio
    from gateway.run import GatewayRunner
    monkeypatch.setenv("KASE_KANBAN_DISPATCH_IN_GATEWAY", "0")
...
def test_gateway_dispatcher_watcher_env_truthy_uses_config(monkeypatch):
    ...
    monkeypatch.setenv("KASE_KANBAN_DISPATCH_IN_GATEWAY", "yes")
    monkeypatch.setattr(
        _cfg_mod, "load_config",
        lambda: {"kanban": {"dispatch_in_gateway": False}},
    )
