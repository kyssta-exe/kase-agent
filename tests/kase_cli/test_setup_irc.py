"""Tests for IRC gateway configuration via `kase setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Kase install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Kase install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#kase")
            monkeypatch.setenv("IRC_NICKNAME", "kase-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `kase setup gateway` experience with IRC present."""
