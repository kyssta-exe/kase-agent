"""Tests for the get_kyssta_auth_status() process-level cache.

The cache avoids re-validating Nous credentials on every menu paint —
`kase tools` → "All Platforms" used to fire ~31 OAuth refresh POSTs
against portal.kyssta.com during one render. The cache is keyed
on auth.json mtime so login/logout flows invalidate naturally; tests
and other writers can also call invalidate_kyssta_auth_status_cache().
"""
...
def _seed_auth_file(tmp_path):
    """Drop a placeholder auth.json into the test KASE_HOME.
...
def test_get_kyssta_auth_status_caches_consecutive_calls(tmp_path, monkeypatch):
    ...
    auth_mod.invalidate_kyssta_auth_status_cache()
    ...
    with patch.object(auth_mod, "_compute_nous_auth_status", side_effect=fake_compute):
        first = auth_mod.get_kyssta_auth_status()
        second = auth_mod.get_kyssta_auth_status()
        third = auth_mod.get_kyssta_auth_status()
    ...
    first["mutated"] = True
    assert "mutated" not in auth_mod.get_kyssta_auth_status()

    auth_mod.invalidate_kyssta_auth_status_cache()


def test_get_kyssta_auth_status_invalidates_on_auth_file_mtime(tmp_path, monkeypatch):
    ...
    with patch.object(auth_mod, "_compute_nous_auth_status", side_effect=fake_compute):
        auth_mod.get_kyssta_auth_status()
...
def test_invalidate_kyssta_auth_status_cache_forces_recompute(tmp_path, monkeypatch):
...
def test_get_kyssta_auth_status_caches_failure_path(tmp_path, monkeypatch):
    """Logged-out snapshots are cached too — that's where the cost was.

    Teknium's case: ~31 cache misses per `kase tools` "All Platforms"
