...
The cache exists to keep `kase tools` → "All Platforms" fast: every
...
def test_load_env_caches_on_repeat_calls():
    ...
    from kase_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("kase_cli.config.get_env_path", return_value=env_path):
            first = load_env()
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from kase_cli import config as config_mod
    from kase_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_kase_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "kase-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("kase_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
