"""Tests for kase backup and import commands."""
...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.kase directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "kase_state.db").write_bytes(b"fake-state")
    ...
    # kase repo (should be EXCLUDED)
    (root / "kase").mkdir(exist_ok=True)
    (root / "kase" / "run_agent.py").write_text("# big file\n")
    (root / "kase" / ".git").mkdir()
    (root / "kase" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_kase(self):
        from kase_cli.backup import _should_exclude
        assert _should_exclude(Path("kase/run_agent.py"))
        assert _should_exclude(Path("kase/.git/HEAD"))
...
class TestBackup:
    ...
    def test_excludes_kase(self, tmp_path, monkeypatch):
        """Backup does NOT include kase/ directory."""
        hermes_home = tmp_path / ".kase"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "kase" in n]
            assert agent_files == [], f"kase files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/kase-backup-*.zip."""
        hermes_home = tmp_path / ".kase"
        hermes_home.mkdir()
        ...
        zips = list(tmp_path.glob("kase-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Kase backup."""
        from kase_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only kase_state.db (old wrong name) is rejected."""
        from kase_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["kase_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into kase home."""
        hermes_home = tmp_path / ".kase"
        hermes_home.mkdir()
        monkeypatch.setenv("KASE_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_strips_hermes_prefix(self, tmp_path, monkeypatch):
        """Import strips .kase/ prefix if all entries share it."""
        hermes_home = tmp_path / ".kase"
        hermes_home.mkdir()
        monkeypatch.setenv("KASE_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".kase/config.yaml": "model: test\n",
            ".kase/skills/a/SKILL.md": "# A\n",
        })
    ...
    def test_rejects_non_hermes_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a kase backup."""
        hermes_home = tmp_path / ".kase"
        hermes_home.mkdir()
        monkeypatch.setenv("KASE_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside kase home
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".kase"
        src_home.mkdir(parents=True)
        _make_hermes_tree(src_home)

        monkeypatch.setenv("KASE_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from kase_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".kase"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("KASE_HOME", str(dst_home))
        ...
        # kase should NOT be present
        assert not (dst_home / "kase").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from kase_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without kase markers fails validation."""
        import io
        from kase_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_hermes(self):
        """Detects .kase/ prefix wrapping all entries."""
        import io
        from kase_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".kase/config.yaml", "test")
            zf.writestr(".kase/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".kase/"
...
class TestBackupEdgeCases:
    def test_nonexistent_kase_home(self, tmp_path, monkeypatch):
        """Backup exits when kase home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".kase"
        monkeypatch.setenv("KASE_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("kase-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_kase_home(self, tmp_path, monkeypatch):
        """Backup handles empty kase home (no files to back up)."""
        hermes_home = tmp_path / ".kase"
        hermes_home.mkdir()
    ...
    def test_skips_output_zip_inside_hermes(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside kase root."""
        hermes_home = tmp_path / ".kase"
        hermes_home.mkdir()
        (hermes_home / "config.yaml").write_text("model: test\n")
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "kase -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import kase_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "kase_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from kase_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def hermes_home(self, tmp_path):
        """Create a fake KASE_HOME with critical state files."""
        home = tmp_path / ".kase"
        home.mkdir()
    ...
    def test_list_snapshots(self, hermes_home):
        from kase_cli.backup import create_quick_snapshot, list_quick_snapshots
    ...
    def test_restore_config(self, hermes_home):
        from kase_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(hermes_home=hermes_home)
    ...
    def test_restore_nonexistent(self, hermes_home):
        from kase_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", hermes_home=hermes_home) is False

    def test_auto_prune(self, hermes_home):
        from kase_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
    ...
    def test_manual_prune(self, hermes_home):
        from kase_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", hermes_home=hermes_home)
...
# ---------------------------------------------------------------------------
# Pre-update backup (kase update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``kase update``
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        ``kase backup`` would, and should exclude the same directories."""
        from kase_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(hermes_home=hermes_home)
        assert out is not None
        ...
        assert "profiles/coder/config.yaml" in names
        # kase repo excluded
        assert not any(n.startswith("kase/") for n in names)
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_hermes_tree(root)
        # Point KASE_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("KASE_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for kase_cli.config + kase_constants so they pick up KASE_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("kase_cli.config") or mod == "kase_constants":
    ...
    def test_config_enabled_creates_backup(self, hermes_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("kase_cli.config"):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (kase claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``kase claw migrate`` runs before mutating ~/.kase/."""
    ...
    def test_creates_backup_under_backups_dir(self, hermes_home):
        from kase_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `kase
    ...
    def test_backup_uses_shared_exclusion_rules(self, hermes_home):
        ...
        ``kase backup`` / ``create_pre_update_backup`` — no drift."""
        from kase_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Kase
        backup — `kase import` should accept it."""
        from kase_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_missing_kase_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.kase yet — nothing to back up."""
        from kase_cli.backup import create_pre_migration_backup
    ...
    def test_does_not_touch_pre_update_backups(self, hermes_home):
        ...
        from kase_cli.backup import create_pre_update_backup, create_pre_migration_backup
