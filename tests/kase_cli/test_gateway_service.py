...
import kase_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "kase-gateway.service"
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "kase gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`kase gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `kase gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `kase gateway restart`)."""
        unit_path = tmp_path / "kase-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``KASE_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../kase_test`` KASE_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir KASE_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="KASE_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/kase_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "kase gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_kase_home_for_target_user", lambda home: "/home/alice/.kase")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.kase.gateway.plist"
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: kase gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup kase gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.kase.gateway"])
            ),
        )
...
class TestSystemUnitHermesHome:
    """KASE_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'KASE_HOME=/home/alice/.kase' in unit
        assert '/root/.kase' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: KASE_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("KASE_HOME", "/root/.kase/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'KASE_HOME=/home/alice/.kase/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_kase_home(self, monkeypatch):
        # Custom KASE_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("KASE_HOME", "/opt/kase-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'KASE_HOME=/opt/kase-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's KASE_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        hermes_home = str(gateway_cli.get_kase_home().resolve())
        assert f'KASE_HOME={hermes_home}' in unit


class TestKaseHomeForTargetUser:
    """Unit tests for _kase_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("KASE_HOME", raising=False)

        result = gateway_cli._kase_home_for_target_user("/home/alice")
        assert result == "/home/alice/.kase"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._kase_home_for_target_user("/home/alice")
        assert result == "/home/alice/.kase/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("KASE_HOME", "/opt/kase")

        result = gateway_cli._kase_home_for_target_user("/home/alice")
        assert result == "/opt/kase"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "kase gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_kase_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.kase should not produce a --profile flag."""
        hermes_home = tmp_path / ".kase"
        hermes_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KASE_HOME", str(hermes_home))
        result = gateway_cli._profile_arg(str(hermes_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.kase/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".kase" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KASE_HOME", str(tmp_path / ".kase"))
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile KASE_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "kase"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.kase/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".kase" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".kase" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_kase_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".kase" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.kase.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".kase" / "kase"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".kase" / "kase")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/kase", str(tmp_path / "alice"))
        assert result == "/opt/kase"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".kase" / "kase")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("KASE_HOME", str(root_home / ".kase"))
        monkeypatch.setattr(gateway_cli, "get_kase_home", lambda: root_home / ".kase")
        ...
        assert "WorkingDirectory=/home/alice/.kase/kase" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "kase-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "kase-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'kase gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'kase gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'kase gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "kase gateway run" in out
...
class TestLegacyHermesUnitDetection:
    ...
    older install left a ``kase.service`` unit behind when the service was
    renamed to ``kase-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``kase.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``kase-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Kase Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m kase_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_hermes_service_in_system_scope(self, tmp_path, monkeypatch):
        ...
        legacy = system_dir / "kase.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_hermes_gateway_coder(self, tmp_path, monkeypatch):
        ...
        Teknium's concern — ``kase-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "kase-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "kase-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "kase-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
    ...
    def test_ignores_unrelated_hermes_service(self, tmp_path, monkeypatch):
        """Third-party ``kase.service`` that isn't ours stays untouched.

        If a user has some other package named ``kase`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "kase.service").write_text(
            "[Unit]\nDescription=Some Other Kase\n[Service]\n"
            "ExecStart=/opt/other-kase/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "kase.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "kase.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m kase_cli.main gateway run
          - python path/to/kase_cli/main.py gateway run
          - kase gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m kase_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/kase/kase_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/kase gateway run --replace",
            "ExecStart=/venv/bin/python /opt/kase/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"kase.service" if i == 0 else f"kase.service"  # same name
            # Test each variant fresh
            (user_dir / "kase.service").write_text(
                f"[Unit]\nDescription=Old Kase\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_hermes_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "kase.service" in out
        assert "kase gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "kase.service"
...
class TestRemoveLegacyHermesUnits:
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop kase.service" in c for c in cmds_joined)
        assert any("--user disable kase.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo kase gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop kase.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable kase.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "kase.service"
        system_legacy = system_dir / "kase.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (kase-gateway-coder.service)
        ...
        profile_unit = user_dir / "kase-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "kase-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `kase gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import kase_cli.main as cli_main
        ...
        # public, just confirm that `kase gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import kase_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "kase_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "kase_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "kase-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "kase-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "kase-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``kase gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "kase-gateway")
        ...
        assert "sudo systemctl start kase-gateway" in out
        assert "sudo kase gateway uninstall --system" in out
        assert "kase gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart kase-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop kase-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``kase gateway start --system`` etc.) must
