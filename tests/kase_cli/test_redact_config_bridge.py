...
`HERMES_REDACT_SECRETS` at module-import time. `kase_cli/main.py` at
...
in .env), the toggle was silently ignored in both `kase chat` and
`kase gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `HERMES_REDACT_SECRETS`
env var in `kase_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports kase_cli.main and checks
    ...
    probe = textwrap.dedent(
        ...
        sys.path.insert(0, %r)
        import kase_cli.main  # triggers the bridge + setup_logging
        import agent.redact
