...
from kase_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from kase_cli.config import load_config, save_config, save_env_value
from kase_cli.kyssta_subscription import KysstaFeatureState, NousSubscriptionFeatures
from kase_cli.setup import _print_setup_summary, setup_model_provider
...
def _stub_tts(monkeypatch):
    monkeypatch.setattr("kase_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("kase_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside kase setup must survive the wizard's final save."""
    ...
    monkeypatch.setattr("kase_cli.main.select_provider_and_model", fake_select)
...
def test_setup_same_provider_rotation_strategy_saved_for_multi_credential_pool(tmp_path, monkeypatch):
    ...
    import kase_cli.main as _main_mod
    import kase_cli.setup as _setup_mod
...
def test_setup_same_provider_fallback_can_add_another_credential(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.main.select_provider_and_model", fake_select)
    _stub_tts(monkeypatch)
    monkeypatch.setattr("kase_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("kase_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("kase_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("agent.credential_pool.load_pool", fake_load_pool)
    monkeypatch.setattr("kase_cli.auth_commands.auth_add_command", fake_auth_add_command)
...
def test_setup_pool_step_shows_manual_vs_auto_detected_counts(tmp_path, monkeypatch, capsys):
    ...
    _stub_tts(monkeypatch)
    monkeypatch.setattr("kase_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("kase_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("kase_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("agent.credential_pool.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("kase_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("kase_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'kase setup' to configure" not in output
...
def test_setup_summary_shows_camofox_when_browser_feature_is_camofox(tmp_path, monkeypatch, capsys):
    ...
    _clear_provider_env(monkeypatch)
    monkeypatch.setattr(
        "kase_cli.setup.get_kyssta_subscription_features",
        lambda config: NousSubscriptionFeatures(
            ...
            features={
                ...
                "browser": KysstaFeatureState("browser", "Browser automation", True, True, True, False, True, True, "Camofox"),
                "modal": KysstaFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
    ...
    monkeypatch.setattr(
        ...
        lambda config: NousSubscriptionFeatures(
            ...
            features={
                ...
                "image_gen": KysstaFeatureState("image_gen", "Image generation", True, False, False, False, False, True, ""),
                "tts": KysstaFeatureState("tts", "OpenAI TTS", True, False, False, False, False, True, ""),
                "browser": KysstaFeatureState("browser", "Browser automation", True, False, False, False, False, True, "Browserbase"),
                "modal": KysstaFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
