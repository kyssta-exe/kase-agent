"""Tests for the post-pull syntax guard in ``kase update``.

When a bad commit lands on ``main`` with a syntax error in a critical file
(e.g. orphan merge-conflict markers in ``kase_cli/config.py``), the CLI
becomes unbootable — every ``kase`` invocation imports those files at
...
markers in ``kase_cli/config.py``; users who ran ``kase update`` in
the 7-minute window before #28458 landed could not run any ``kase``
command afterward.
"""
...
def test_validate_critical_files_syntax_detects_conflict_markers(tmp_path):
    ...
    _populate_critical_tree(tmp_path, broken_file="kase_cli/config.py")

    ok, failing_path, error = hermes_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("kase_cli/config.py")
...
def test_validate_critical_files_syntax_detects_break_in_main_py(tmp_path):
    _populate_critical_tree(tmp_path, broken_file="kase_cli/main.py")

    ok, failing_path, _ = hermes_main._validate_critical_files_syntax(tmp_path)

    assert ok is False
    assert failing_path is not None and failing_path.endswith("kase_cli/main.py")
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except kase_constants.py
    for relpath in hermes_main._UPDATE_CRITICAL_FILES:
        if relpath == "kase_constants.py":
            continue
...
# release; if a future ``kase update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    assert ok is True, (
        ...
        f"kase update would brick users. Error: {error}"
    )
