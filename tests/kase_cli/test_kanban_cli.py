"""Tests for the kanban CLI surface (kase_cli.kanban)."""
...
from kase_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`kase kanban list --session <id>` filters by the originating
...
def test_kanban_in_autocomplete_table():
    from kase_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_kanban_not_gateway_only():
    ...
    from kase_cli.commands import COMMAND_REGISTRY
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("KASE_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("KASE_KANBAN_BOARD") == "beta"
