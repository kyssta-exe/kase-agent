"""Tests for the `kase proxy` subcommand and its upstream adapters."""
...
from kase_cli.proxy.adapters import ADAPTERS, get_adapter
from kase_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from kase_cli.proxy.adapters.kyssta_portal import NousPortalAdapter
from kase_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_auth_store(hermes_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic KASE_HOME."""
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # KASE_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_authenticated_with_agent_key(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path, {
        ...
        "inference_base_url": "https://inference-api.kyssta.com/v1",
    })
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "kase_cli.proxy.adapters.kyssta_portal.resolve_kyssta_runtime_credentials",
    ...
    assert cred.base_url == "https://inference-api.kyssta.com/v1"
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("KASE_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="kase login nous"):
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic KASE_HOME."""
    auth_path = hermes_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
from kase_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("KASE_HOME", str(tmp_path))
    from kase_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from kase_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from kase_cli.proxy.cli import cmd_proxy_start
