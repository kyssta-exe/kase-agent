...
from kase_cli.auth import AuthError, get_provider_auth_state, resolve_kyssta_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from kase_cli.auth import _resolve_verify
    ...
    def test_missing_hermes_ca_bundle_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.setenv("HERMES_CA_BUNDLE", "/nonexistent/kase-ca.pem")
...
def test_resolve_kyssta_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    import kase_cli.auth as auth_mod
    ...
    creds = auth_mod.resolve_kyssta_runtime_credentials(min_key_ttl_seconds=300)
...
def test_resolve_kyssta_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import kase_cli.auth as auth_mod
...
def test_resolve_kyssta_runtime_credentials_trusts_invoke_jwt_exp_over_stale_metadata(
    tmp_path,
    monkeypatch,
):
    import kase_cli.auth as auth_mod
...
def test_resolve_kyssta_runtime_credentials_does_not_apply_legacy_ttl_to_invoke_jwt(
    tmp_path,
    monkeypatch,
):
    import kase_cli.auth as auth_mod
    ...
    creds = auth_mod.resolve_kyssta_runtime_credentials(min_key_ttl_seconds=1800)
...
def test_legacy_auth_mode_bypasses_usable_invoke_jwt(tmp_path, monkeypatch):
    ...
    creds = auth_mod.resolve_kyssta_runtime_credentials(
        min_key_ttl_seconds=300,
        inference_auth_mode=auth_mod.KYSSTA_INFERENCE_AUTH_MODE_LEGACY,
    )
...
def test_resolve_kyssta_runtime_credentials_falls_back_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import kase_cli.auth as auth_mod
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="kase_cli.auth")
    auth_mod.resolve_kyssta_runtime_credentials(min_key_ttl_seconds=300)
...
def test_get_kyssta_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_kyssta_auth_status() should find Nous credentials in the pool
...
def test_get_kyssta_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    """get_kyssta_auth_status() falls back to auth store when credential
    pool is empty.
    """
    from kase_cli.auth import get_kyssta_auth_status
    ...
    monkeypatch.setenv("KASE_HOME", str(hermes_home))
    monkeypatch.setattr(
        "kase_cli.auth.resolve_kyssta_runtime_credentials",
...
def test_get_kyssta_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    from kase_cli.auth import get_kyssta_auth_status
    from agent.credential_pool import PooledCredential, load_pool
...
def test_get_kyssta_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    from kase_cli.auth import get_kyssta_auth_status
    ...
    monkeypatch.setattr("kase_cli.auth.resolve_kyssta_runtime_credentials", _boom)
...
def test_get_kyssta_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_kyssta_auth_status() returns logged_in=False when both pool
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("kase_cli.auth._mint_agent_key", _fake_mint_agent_key)

    with pytest.raises(AuthError) as exc:
        resolve_kyssta_runtime_credentials(min_key_ttl_seconds=300)
    ...
    creds = resolve_kyssta_runtime_credentials(min_key_ttl_seconds=300)
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    from kase_cli import auth as auth_mod
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    with pytest.raises(AuthError, match="Invalid refresh token"):
        auth_mod.resolve_kyssta_access_token()
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `kase model` → Nous Portal → Skip (keep current) after
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import kase_cli.auth as auth_mod
        import kase_cli.models as models_mod
        import kase_cli.kyssta_subscription as ns

        fake_auth_state = {
            "access_token": "fake-nous-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.kyssta.com",
            "portal_base_url": "https://portal.kyssta.com",
        ...
        monkeypatch.setattr(models_mod, "check_kyssta_free_tier", lambda: None)
        monkeypatch.setattr(
            models_mod, "partition_kyssta_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from kase_cli.auth import PROVIDER_REGISTRY, _login_nous
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `kase auth add nous`
    wrote only the pool. After the Nous agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_kyssta_runtime_credentials
    ...
    """
    from kase_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    """End-to-end: after persisting via the helper, resolve_kyssta_runtime_credentials
    must succeed (not raise "Kase is not logged into Nous Portal").
    ...
    # doesn't raise "Kase is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
    ...
    creds = resolve_kyssta_runtime_credentials(
        min_key_ttl_seconds=300,
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `kase auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Kase must surface an
    ...
    re-authenticate" string led users to report this as a Kase persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from kase_cli.auth import _refresh_access_token
    ...
    assert "kase auth add nous" in message.lower()
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    writing to the user's real ``~/.kase/shared/`` across CI runs.
    """
    from kase_cli.auth import _nous_shared_store_path
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    ...
    from kase_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from kase_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from kase_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from kase_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `kase auth add nous
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from kase_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    def _fake_refresh(state, **kwargs):
        ...
        assert (
            ...
            == auth_mod.KYSSTA_INFERENCE_AUTH_MODE_FRESH
        )
        return {
            **state,
    ...
    assert result["client_id"] == "kase-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different KASE_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("KASE_HOME", str(profile_a))
    ...
    # Profile B: fresh KASE_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("KASE_HOME", str(profile_b))
...
def test_runtime_refresh_uses_newer_shared_token_before_local_stale_token(
    ...
    creds = auth_mod.resolve_kyssta_runtime_credentials(
        min_key_ttl_seconds=300,
        inference_auth_mode=auth_mod.KYSSTA_INFERENCE_AUTH_MODE_FRESH,
    )
...
def test_managed_gateway_access_token_uses_newer_shared_token(
    ...
    assert auth_mod.resolve_kyssta_access_token() == "shared-fresh-access"
