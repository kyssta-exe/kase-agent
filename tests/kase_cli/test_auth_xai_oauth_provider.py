"""Tests for xAI Grok OAuth — tokens stored in Kase auth store (~/.kase/auth.json)."""
...
def _setup_hermes_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Kase auth store at the given root."""
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("kase_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_authorize_url_includes_referrer_kase():
    """Attribution: xAI's OAuth server can identify Kase-originated logins
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "kase auth add xai-oauth" in body
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    ...
    assert creds["source"] == "kase-auth-store"
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    monkeypatch.setattr("kase_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    monkeypatch.setattr("kase_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``kase model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``kase model``
    ...
    assert "kase model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from kase_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Kase that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `kase model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `kase auth list` reflects truth and
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`kase auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `kase auth remove`.
    from kase_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``kase auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `kase auth remove xai-oauth 1`.
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Kase invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from kase_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Kase process refreshed the singleton
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``kase model`` -> xAI Grok OAuth (or another process
    ...
    # Simulate the user re-running `kase model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``kase auth add xai-oauth``) own their own refresh-token lifecycle
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`kase auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``kase auth status``, ``kase setup``, and the
    ``kase`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
