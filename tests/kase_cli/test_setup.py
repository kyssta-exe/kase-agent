...
from kase_cli import setup as setup_mod
from kase_cli.setup import setup_model_provider
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import kase_cli.gateway as gateway_mod
...
def test_setup_gateway_in_container_shows_docker_guidance(monkeypatch, capsys):
    ...
    monkeypatch.setattr(kase_constants, "is_container", lambda: True)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("kase_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("kase_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "kase_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "kase_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_kyssta_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("kase_cli.setup.managed_kyssta_tools_enabled", lambda: True)
    monkeypatch.setenv("KASE_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("kase_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("kase_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
    ...
    from kase_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kase_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("kase_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("kase_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "kase_cli.setup.get_kyssta_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
