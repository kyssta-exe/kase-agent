...
When ``KASE_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.kase/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.kase            (has its own auth.json fixture)
    * Profile     -> tmp_path/.kase/profiles/coder   (active, KASE_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (KASE_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.kase as the "real home" — which is NOT equal
    # to the KASE_HOME we set (tmp_path/classic), so the guard passes.
    ...
    from kase_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: KASE_HOME is set to a custom path that is NOT under
    # ~/.kase/profiles/ — get_default_hermes_root() returns KASE_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from kase_cli.auth import read_credential_pool, write_credential_pool
