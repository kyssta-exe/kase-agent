...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("kase_cli.config.get_managed_system", return_value=None), \
         patch("kase_cli.config.get_kase_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("kase_cli.config.get_managed_system", return_value="NixOS"), \
         patch("kase_cli.config.get_kase_home", return_value=tmp_path):
        from kase_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from kase_cli.config import recommended_update_command_for_method
...
def test_docker_detected_via_dockerenv(tmp_path):
    with patch("kase_cli.config.get_managed_system", return_value=None), \
         patch("kase_cli.config.get_kase_home", return_value=tmp_path), \
         patch("kase_constants.is_container", return_value=True):
        from kase_cli.config import detect_install_method
        assert detect_install_method(project_root=tmp_path) == "docker"
