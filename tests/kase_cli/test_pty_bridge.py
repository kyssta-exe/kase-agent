"""Unit tests for kase_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf kase-ok"])
        try:
            output = _read_until(bridge, b"kase-ok")
            assert b"kase-ok" in output
        finally:
            bridge.close()
