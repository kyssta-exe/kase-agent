"""Tests for the Kanban DB layer (kase_cli.kanban_db)."""
...
from kase_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated KASE_HOME with an empty kanban DB."""
    home = tmp_path / ".kase"
    home.mkdir()
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("KASE_HOME", str(home))
    monkeypatch.delenv("KASE_KANBAN_DB", raising=False)
    monkeypatch.delenv("KASE_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("KASE_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import kase_cli.kanban_db as _kb
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Kase profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="kase-task", assignee="daily")
...
def test_scratch_workspace_created_under_kase_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `kase -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's KASE_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's KASE_HOME."""
    ...
    def test_default_install_anchors_at_home_dot_hermes(
        ...
        # Standard install: KASE_HOME == ~/.kase, no profile active.
        default_home = tmp_path / ".kase"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.kase/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.kase/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.kase/kanban.db.
        default_home = tmp_path / ".kase"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # KASE_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `kase -p coder`).
        monkeypatch.setenv("KASE_HOME", str(profile_home))
    ...
    def test_docker_custom_kase_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: KASE_HOME points outside ~/.kase.
        # `get_default_hermes_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".kase"`.
        custom_root = tmp_path / "opt" / "kase"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: KASE_HOME=/opt/kase/profiles/coder;
        # `get_default_hermes_root()` walks up to /opt/kase because
    ...
    def test_explicit_override_via_hermes_kanban_home(
        ...
        # Explicit override: KASE_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".kase"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KASE_HOME", str(profile_home))
        monkeypatch.setenv("KASE_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KASE_HOME", str(default_home))
        monkeypatch.setenv("KASE_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # KASE_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile KASE_HOME and reads.
        monkeypatch.setenv("KASE_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_hermes_kanban_db_pin_beats_kanban_home(
        ...
        # KASE_KANBAN_DB pins the file path directly and beats both
        # KASE_KANBAN_HOME and the `get_default_hermes_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KASE_HOME", str(default_home))
        monkeypatch.setenv("KASE_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("KASE_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows KASE_KANBAN_HOME -- the pins
        # are independent.
    ...
    def test_hermes_kanban_workspaces_root_pin_beats_kanban_home(
        ...
        # KASE_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".kase"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("KASE_HOME", str(default_home))
        monkeypatch.setenv("KASE_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("KASE_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows KASE_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KASE_HOME", str(default_home))
        monkeypatch.setenv("KASE_KANBAN_DB", "   ")
        monkeypatch.setenv("KASE_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject KASE_KANBAN_DB
        # and KASE_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites KASE_HOME.
        default_home = tmp_path / ".kase"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["KASE_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["KASE_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["KASE_KANBAN_TASK"] == "t_dispatch_env"
        assert env["KASE_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see kase_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("kase_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="kase_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'kase kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_kase_argv()
#
# Workers spawned by the dispatcher must use a `kase` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["kase", ...]`
...
def test_resolve_kase_argv_prefers_path_shim(monkeypatch):
    """When `kase` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import kase_cli.kanban_db as kb

    monkeypatch.delenv("KASE_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/kase")
    argv = kb._resolve_kase_argv()
    assert argv == ["/usr/local/bin/kase"]


def test_resolve_kase_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import kase_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("KASE_BIN", ".\\kase.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_kase_argv() == [os.path.abspath(".\\kase.exe")]


def test_resolve_kase_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "kase.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("KASE_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_kase_argv() == [sys.executable, "-m", "kase_cli.main"]


def test_resolve_kase_argv_honors_hermes_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like KASE_BIN lets service managers pin the executable."""
    import shutil
    import kase_cli.kanban_db as kb

    shim = tmp_path / "bin" / "kase"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("KASE_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_kase_argv() == [str(shim)]


def test_resolve_kase_argv_hermes_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare KASE_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import kase_cli.kanban_db as kb

    cwd_hermes = tmp_path / "kase"
    cwd_hermes.write_text("wrong\n", encoding="utf-8")
    cwd_hermes.chmod(cwd_hermes.stat().st_mode | stat.S_IXUSR)
    path_hermes = tmp_path / "bin" / "kase"
    ...
    monkeypatch.setenv("KASE_BIN", "kase")

    assert kb._resolve_kase_argv() == [str(path_hermes)]


def test_resolve_kase_argv_hermes_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare KASE_BIN does not accept current-directory shadow executables."""
    import sys
    import kase_cli.kanban_db as kb

    (tmp_path / "kase.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_kase_argv_hermes_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved KASE_BIN batch shim is not used as worker argv[0]."""
    import sys
    import kase_cli.kanban_db as kb
...
def test_resolve_kase_argv_hermes_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved KASE_BIN command names do not delegate cwd search to Popen."""
    import sys
    import kase_cli.kanban_db as kb
...
def test_resolve_kase_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m kase_cli.main`.

    Pins the correct module name (NOT `kase` — there is no top-level
    `kase` package). Regression for #23198: the original PR shipped
    `python -m kase` which fails with `No module named kase` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_kase_argv()
    assert argv == [sys.executable, "-m", "kase_cli.main"]


def test_resolve_kase_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `kase_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m kase_cli.main --version`
    ...
    assert "Kase" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
