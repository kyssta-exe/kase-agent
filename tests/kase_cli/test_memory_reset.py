"""Tests for the `kase memory reset` CLI command.
...
- Profile-scoped reset (uses KASE_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake KASE_HOME with memory files."""
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `kase memory reset` is run.
    """
...
class TestMemoryReset:
    """Tests for `kase memory reset` subcommand."""
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's KASE_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_kase_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
