...
def test_registry_defaults_to_kase_home(tmp_path, monkeypatch):
    from plugins.google_meet.node.registry import NodeRegistry

    # _isolate_home already set KASE_HOME to tmp_path/.kase; the
    # registry default path must live inside that tree.
    r = NodeRegistry()
    r.add("x", "ws://x", "t")
    expected = Path(tmp_path) / ".kase" / "workspace" / "meetings" / "nodes.json"
...
def test_cli_status_pings_via_node_client(capsys, monkeypatch):
    ...
    class _FakeClient:
        ...
        def ping(self):
            return {"type": "pong", "display_name": "kase-meet-node"}
