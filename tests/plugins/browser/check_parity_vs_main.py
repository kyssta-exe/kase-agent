...
Run from the PR worktree:
    ...
    cd ~/.kase/kase/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.kase/kase``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.kase/kase
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a kase checkout"
)
...
# Isolated KASE_HOME for the config write.
