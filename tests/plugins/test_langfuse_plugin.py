...
class TestDiscovery:
    def test_plugin_is_discovered_as_standalone_opt_in(self, tmp_path, monkeypatch):
        ...
        # Isolated KASE_HOME so we don't read the developer's config.yaml.
        home = tmp_path / ".kase"
        home.mkdir()
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_hermes_config(self, monkeypatch):
        ...
        # Drop any cached import of kase_cli.config.
        sys.modules.pop("kase_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "kase_cli.config" not in sys.modules, (
            "langfuse plugin imported kase_cli.config — regression toward "
...
# Kase logs.  The fix in `_get_langfuse()` validates the documented
