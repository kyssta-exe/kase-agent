...
binary download are exercised manually by `kase secrets bitwarden
...
def hermes_home(tmp_path, monkeypatch):
    """Point Kase at an isolated home directory."""
    home = tmp_path / ".kase"
    home.mkdir()
    monkeypatch.setenv("KASE_HOME", str(home))
    # Some modules cache get_kase_home; clear if needed.
    import kase_constants
    if hasattr(kase_constants, "_KASE_HOME_CACHE"):
        kase_constants._KASE_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from kase_cli.env_loader import _apply_external_secret_sources
