...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    kase_state = ModuleType("kase_state")
    kase_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "kase_state", kase_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Kase"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Kase"
