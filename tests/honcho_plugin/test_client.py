...
from kase_cli.profiles import _get_default_kase_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "kase"
        assert config.workspace_id == "kase"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.kase/honcho.json
        monkeypatch.setenv("KASE_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"kase": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="kase"
        ):
            result = config.resolve_session_name("/home/user/kase/subdir")
        assert result == "kase"
...
class TestResolveConfigPath:
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        ...
        assert _get_default_kase_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_kase_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("KASE_HOME", None)
            result = resolve_config_path()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".kase".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".kase"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_default_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=True):
            ...
            os.environ.pop("KASE_HOME", None)
            assert resolve_active_host() == "kase"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"HERMES_HONCHO_HOST": "kase.coder"}):
            assert resolve_active_host() == "kase.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("kase_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "kase.coder"
    ...
    def test_custom_profile_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("kase_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "kase"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            # Temporarily remove kase_cli.profiles to simulate import failure
            saved = sys.modules.get("kase_cli.profiles")
            sys.modules["kase_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "kase"
            finally:
                if saved is not None:
                    sys.modules["kase_cli.profiles"] = saved
                else:
                    sys.modules.pop("kase_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="kase.coder")
        assert config.host == "kase.coder"
        assert config.workspace_id == "kase"  # shared workspace
        assert config.ai_peer == "kase.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="kase")
        assert config.host == "kase"
        assert config.workspace_id == "kase"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "kase": {"aiPeer": "kase", "peerName": "alice"},
                "kase.coder": {
                    "aiPeer": "kase.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="kase.coder", config_path=config_file,
        )
        assert config.host == "kase.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "kase.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="kase.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "kase.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kase": {"enabled": True, "aiPeer": "kase"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kase": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_hermes_config_timeout_override_used_when_config_timeout_missing(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("kase_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("kase_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_hermes_request_timeout_alias_used(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("kase_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
