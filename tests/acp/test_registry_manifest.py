"""Tests for ACP Registry metadata shipped with Kase."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["name"] == "Kase"
    assert data["description"]
    assert data["repository"] == "https://github.com/Kyssta/kase"
    assert data["website"].startswith("https://kase.kyssta.com/")
    assert data["authors"] == ["Kyssta"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"kase[acp]=={data['version']}"
    assert uvx["args"] == ["kase-acp"]
