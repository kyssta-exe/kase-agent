"""Tests for acp_adapter.server — KaseACPAgent ACP server."""
...
from acp_adapter.server import KaseACPAgent, HERMES_VERSION
...
def agent(mock_manager):
    """KaseACPAgent backed by a mock session manager."""
    return KaseACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "kase"
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Kase' interactive model/provider setup in a terminal. "
                    "Use this when Kase has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Kase provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = KaseACPAgent(session_manager=manager)

        with patch(
            "kase_cli.models.curated_models_for_provider",
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "KaseACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("KaseACPAgent")
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kase_cli.config.load_config", lambda: {
...
class TestPrompt:
    ...
    async def test_prompt_propagates_hermes_session_id_env(self, agent, monkeypatch):
        ...
        via ``KASE_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("KASE_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "KASE_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("KASE_SESSION_ID") is None, (
            "KASE_SESSION_ID must be restored after the agent call so "
    ...
    async def test_prompt_restores_prior_hermes_session_id(self, agent, monkeypatch):
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("KASE_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("KASE_SESSION_ID") == "outer-sess"
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["kase-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["kase-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["kase-acp", "mcp-srv"]
