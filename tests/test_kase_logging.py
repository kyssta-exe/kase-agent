"""Tests for kase_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    kase_logging._logging_initialized = False
    ...
    kase_logging._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    kase_logging._logging_initialized = False
    kase_logging.clear_session_context()


@pytest.fixture
def hermes_home(tmp_path, monkeypatch):
    """Provide an isolated KASE_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_kase_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["KASE_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, hermes_home):
        log_dir = kase_logging.setup_logging(hermes_home=hermes_home)
        assert log_dir == hermes_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, hermes_home):
        kase_logging.setup_logging(hermes_home=hermes_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, hermes_home):
        kase_logging.setup_logging(hermes_home=hermes_home)
        kase_logging.setup_logging(hermes_home=hermes_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, hermes_home):
        ...
        kase_logging.setup_logging(hermes_home=hermes_home, force=True)
    ...
    def test_custom_log_level(self, hermes_home):
        kase_logging.setup_logging(hermes_home=hermes_home, log_level="DEBUG")
    ...
    def test_writes_to_agent_log(self, hermes_home):
        ...
        test_logger = logging.getLogger("test_kase_logging.write_test")
    ...
    def test_warnings_appear_in_both_logs(self, hermes_home):
        ...
        test_logger = logging.getLogger("test_kase_logging.warning_test")
    ...
    def test_info_not_in_errors_log(self, hermes_home):
        ...
        test_logger = logging.getLogger("test_kase_logging.info_test")
    ...
    def test_explicit_params_override_config(self, hermes_home):
        ...
        kase_logging.setup_logging(hermes_home=hermes_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, hermes_home):
        kase_logging.setup_logging(hermes_home=hermes_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, hermes_home):
        kase_logging.setup_logging(hermes_home=hermes_home, mode="cli")
        root = logging.getLogger()
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, hermes_home):
        ...
        kase_logging.setup_logging(hermes_home=hermes_home)
        kase_logging.set_session_context("abc123")
    ...
    def test_clear_session_context(self, hermes_home):
        ...
        kase_logging.setup_logging(hermes_home=hermes_home)
        kase_logging.set_session_context("xyz789")
        kase_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, hermes_home):
        ...
        def thread_a():
            kase_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            kase_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        kase_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        kase_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = kase_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = kase_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in kase_logging.COMPONENT_PREFIXES
        ...
        # under that prefix).
        assert ("gateway", "hermes_plugins") == kase_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = kase_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == kase_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = kase_logging.COMPONENT_PREFIXES["cli"]
        assert "kase_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == kase_logging.COMPONENT_PREFIXES["cron"]
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, hermes_home):
        kase_logging.setup_logging(hermes_home=hermes_home)
        kase_logging.setup_verbose_logging()
    ...
    def test_idempotent(self, hermes_home):
        kase_logging.setup_logging(hermes_home=hermes_home)
        kase_logging.setup_verbose_logging()
        kase_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        kase_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = kase_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        kase_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("kase_cli.config.is_managed", return_value=True):
                kase_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, hermes_home):
        level, max_size, backup = kase_logging._read_logging_config()
