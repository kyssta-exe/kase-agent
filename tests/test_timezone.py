"""
Tests for timezone support (kase_time module + integration points).
...
def _reset_kase_time_cache():
    """Reset the kase_time module cache (replacement for removed reset_cache)."""
    kase_time._cached_tz = None
    kase_time._cached_tz_name = None
    kase_time._cache_resolved = False


# =========================================================================
# kase_time.now() — core helper
# =========================================================================

class TestKaseTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_kase_time_cache()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="kase_time"):
            result = kase_time.now()
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != Kase tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hermes_tz(self):
        """Already-aware datetimes should be normalized to Kase tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Kase tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Kase tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Kase is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Kase tz
        ...
        # Use a Kase timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _kase_now's wall time — this would
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Kase timezone"
        )
