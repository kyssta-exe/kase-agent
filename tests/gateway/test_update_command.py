...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade kase" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _kase_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._kase_home", tmp_path), \
        ...
        with patch("gateway.run._kase_home", tmp_path):
            ...
            # project_root will be the real kase dir (which HAS .git).
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when kase is not on PATH and kase_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "kase update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m kase_cli.main when kase not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._kase_home", hermes_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        assert "Starting Kase update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m kase_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "kase_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_kase_bin_prefers_which(self, tmp_path):
        """_resolve_kase_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_kase_bin

        with patch("shutil.which", return_value="/custom/path/kase"):
            result = _resolve_kase_bin()

        assert result == ["/custom/path/kase"]

    @pytest.mark.asyncio
    async def test_resolve_kase_bin_fallback(self):
        """_resolve_kase_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_kase_bin
        ...
        assert result == [sys.executable, "-m", "kase_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_kase_bin_returns_none_when_both_fail(self):
        """_resolve_kase_bin returns None when both strategies fail."""
        from gateway.run import _resolve_kase_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._kase_home", hermes_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/kase" if x == "kase" else "/usr/bin/setsid"), \
