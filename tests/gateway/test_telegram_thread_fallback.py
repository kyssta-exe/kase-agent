...
Supergroup forum topics route with ``message_thread_id``. Kase-created
...
async def test_send_typing_attempts_api_call_for_dm_topic_reply_fallback():
    """Kase-created DM topic lanes should still attempt scoped typing.
...
async def test_send_uses_reply_fallback_for_hermes_dm_topics():
    """Kase-created Telegram DM topics route with thread id plus reply anchor."""
    adapter = _make_adapter()
    call_log = []
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in Kase-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []
