...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.kase."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /kase, /btw, /stop,
        ...
        for expected in ("/kase", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@kase`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@kase"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@kase")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /kase subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /kase btw."""
        command = {
            "command": "/btw",
    ...
    async def test_legacy_hermes_prefix_still_works(self, adapter):
        """Backward compat: /kase btw foo must still route to /btw foo.

        Old workspace manifests only declared /kase as the single slash.
    ...
    async def test_legacy_hermes_freeform_question(self, adapter):
        """/kase <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/kase",
...
class TestSlashEphemeralAck:
    ...
    async def test_legacy_hermes_slash_stashes_context(self, adapter):
        """Legacy /kase <subcommand> also stashes context."""
        command = {
            "command": "/kase",
            "text": "help",
            "user_id": "U_H",
    ...
    async def test_freeform_hermes_question_does_not_stash_context(self, adapter):
        """Free-form /kase <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/kase",
