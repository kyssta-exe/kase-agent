"""Shared fixtures for the kase test suite.
...
2. **Isolated KASE_HOME.** KASE_HOME points to a per-test tempdir so
   code reading ``~/.kase/*`` via ``get_kase_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".kase"`` instead of the canonical
   ``get_kase_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No KASE_SESSION_* inheritance** — the agent's current gateway
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    "KASE_SESSION_CHAT_ID",
    "KASE_SESSION_CHAT_NAME",
    "KASE_SESSION_THREAD_ID",
    "KASE_SESSION_SOURCE",
    "KASE_SESSION_KEY",
    "KASE_GATEWAY_SESSION",
    "HERMES_PLATFORM",
    "HERMES_MODEL",
    ...
    "KASE_BACKGROUND_NOTIFICATIONS",
    "HERMES_EXEC_ASK",
    "KASE_HOME_MODE",
    "KASE_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.kase/kanban.db instead of the per-test KASE_HOME.
    "KASE_KANBAN_DB",
    "KASE_KANBAN_BOARD",
    "KASE_KANBAN_HOME",
    "KASE_KANBAN_WORKSPACES_ROOT",
    "KASE_KANBAN_LOGS_ROOT",
    "KASE_KANBAN_TASK",
    "KASE_KANBAN_WORKSPACE",
    "KASE_KANBAN_RUN_ID",
    "KASE_KANBAN_CLAIM_LOCK",
    "KASE_KANBAN_DISPATCH_IN_GATEWAY",
    "HERMES_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and KASE_HOME to per-test tempdirs so code that
    reads ``~/.kase/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 3. Redirect KASE_HOME to a per-test tempdir. Code that reads
    #    ``~/.kase/*`` via ``get_kase_home()`` now gets the tempdir.
    #
    ...
    (fake_kase_home / "memories").mkdir()
    (fake_kase_home / "skills").mkdir()
    monkeypatch.setenv("KASE_HOME", str(fake_kase_home))
    ...
    #    ~/.kase/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import kase_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_kase_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal kase config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``kase-gateway`` process
...
# correlated with a ``tests/kase_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> kase-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart kase-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting kase/python patterns are also blocked.
    """
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch kase-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m kase_cli.main".
                if (
                    "kase" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live kase-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting kase/python could hit the live gateway. "
