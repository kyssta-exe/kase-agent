"""
Tests for mcp_serve — Kase MCP server.
...
def _isolate_kase_home(tmp_path, monkeypatch):
    """Redirect KASE_HOME to a temp directory."""
    monkeypatch.setenv("KASE_HOME", str(tmp_path))
    try:
        import kase_constants
        monkeypatch.setattr(kase_constants, "get_kase_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking kase_state schema."""
