...
There are two ways to add a platform to the Kase gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.kase/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Kase code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `kase gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `kase_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Kase core.
...
And add it to the `kase-gateway` composite:

```python
"kase-gateway": {
    "includes": [..., "kase-your-platform"]
}
```
...
## 12. Status Display (`kase_cli/status.py`)
...
## 13. Gateway Setup Wizard (`kase_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ kase_cli/ toolsets.py \
  --include="*.py" -l | sort -u
