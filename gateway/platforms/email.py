"""
Email platform adapter for the Kase gateway.

Allows users to interact with Kase by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "kase" "version" "{_hermes_version}" '
            '"vendor" "Kyssta" '
            '"support-email" "noreply@kyssta.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        msg_id = f"<kase-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
