"""Projects codex app-server events into Kase' messages list.

The translator that lets Kase' memory/skill review keep working under the
...
Kase' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Kase
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Kase' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Kase stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Kase already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Kase' messages store text only.
        text_parts: list[str] = []
