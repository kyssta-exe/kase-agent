...
Owns one Codex thread per Kase session. Drives `turn/start`, consumes
...
# Kase' tools.terminal.security_mode → Codex's permissions profile id.
...
class CodexAppServerSession:
    """One Codex thread per Kase session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Kase' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Kase' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own kase-tools callback we
            # auto-accept — the user already approved Kase' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Kase approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Kase returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort Kase version string for codex's userAgent line."""
    try:
