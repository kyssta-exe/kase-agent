...
# (bridged to this env var in kase_cli/main.py, gateway/run.py, and
# cli.py) or `HERMES_REDACT_SECRETS=false` in ~/.kase/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Kase — used by
    ``kase config``, ``kase status``, ``kase dump``, and anywhere
...
def redact_sensitive_text(text: str, *, force: bool = False, code_file: bool = False) -> str:
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical kase log line
