...
def _state_file() -> Path:
    return get_kase_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.kase/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``kase update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``kase curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `kase curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Kase' background skill CURATOR. This is an "
    ...
    "into ~/.kase/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: kase-config-*, kase-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.kase/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.kase/logs/curator/``)
    ...
    authored skill data in ``~/.kase/skills/``.

    ``ensure_kase_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_kase_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every kase install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: kase curator status
        keep an umbrella stable: kase curator pin document-tools
    ...
    lines.append("full report: kase curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: kase curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.kase/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.kase/skills/.archive/` for "
            "safety and can be restored via `kase curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.kase/skills/.archive/`. "
            "Restore any via `kase curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `kase curator restore <name>`")
    lines.append("- All archives live under `~/.kase/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `kase curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `kase curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``kase model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `kase model` → auxiliary
