...
from kase_constants import get_config_path, get_skills_dir, is_termux
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``HERMES_PLATFORM`` or
            ``KASE_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``kase`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.kase/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``kase`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against KASE_HOME, not cwd
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.kase/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    kase = metadata.get("kase") or {}
    if not isinstance(kase, dict):
        kase = {}
    return {
        "fallback_for_toolsets": kase.get("fallback_for_toolsets", []),
        "requires_toolsets": kase.get("requires_toolsets", []),
        "fallback_for_tools": kase.get("fallback_for_tools", []),
        "requires_tools": kase.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Kase metadata, VCS, virtualenv/dependency, and cache
