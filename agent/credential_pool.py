...
from kase_cli.config import get_env_value, load_env
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``kase model`` / ``kase auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Kase process
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        ``resolve_kyssta_runtime_credentials``, it writes fresh tokens to
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``kase`` invocation that defaults to the active provider
        (e.g. setup wizard, ``kase auth status``) would land on
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("kase_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Kase
                ...
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_kyssta_runtime_credentials(
                    min_key_ttl_seconds=DEFAULT_AGENT_KEY_MIN_TTL_SECONDS,
                    inference_auth_mode=(
                        auth_mod.KYSSTA_INFERENCE_AUTH_MODE_LEGACY
                        if force
                        else auth_mod.KYSSTA_INFERENCE_AUTH_MODE_AUTO
                    ),
                )
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("kase_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Kase process
                ...
                if auth_mod._is_terminal_nous_refresh_error(exc):
                    ...
                    try:
                        with _auth_store_lock():
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_kyssta_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_kyssta_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Kase profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # resolve_kyssta_runtime_credentials(), making this entry's
            ...
            # re-authed via `kase model` / `kase auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `kase model` ->
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `kase auth remove <provider> <N>` is stable across all source types.
    try:
        from kase_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Kase PKCE)
        ...
        for source_name, creds in (
            ("kase_pkce", read_hermes_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `kase auth add nous --label <name>`).  Fall back to the
        ...
        # Kase auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from kase_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.kase/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `kase auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `kase auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Kase auth store.  Without this gate the removal is instantly
        ...
        # Kase owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``kase model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``kase auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from kase_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.kase/.env over os.environ — the user's config file is the
    # authoritative source for Kase credentials. Stale env vars from parent
    ...
    # Honour user suppression — `kase auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.kase/.env.
    ...
    try:
        from kase_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.kase/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "kase_pkce"}
        )
    ]
    if len(retained) == len(entries):
        return False
