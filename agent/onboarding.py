...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Kase starts and finds ``~/.openclaw/``.

    Points users at ``kase claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``kase claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Kase, run "
        "`kase claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `kase claw cleanup` (renames it to ~/.openclaw.pre-migration — "
