...
Every Kase request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Kase and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=kase",
        "client=kase-client-v<__version__>",
    ]

The version is sourced live from ``kase_cli.__version__`` so it auto-aligns
...
``kase_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    """Return the current Kase release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``kase_cli`` cannot be imported (should
...
def kase_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=kase-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=kase-client-v{_hermes_version()}"


def kyssta_portal_tags() -> List[str]:
    """Return the canonical list of Nous Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(kyssta_portal_tags())``).
    """
    return ["product=kase", kase_client_tag()]
