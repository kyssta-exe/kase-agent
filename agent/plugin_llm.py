...
Plugins built on Kase often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Kase
...
:class:`~kase_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Kase' main config
...
handles every provider, fallback chain, and per-task override Kase
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`kase_cli.plugins.PluginContext`
