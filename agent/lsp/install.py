...
appropriate.  All installs go to a Kase-owned bin staging dir,
``<KASE_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``kase lsp
  status``.
...
# ``<KASE_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def hermes_lsp_bin_dir() -> Path:
    ...
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".kase")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = hermes_lsp_bin_dir().parent  # <KASE_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a kase-owned target dir.
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``kase lsp status`` CLI to give users a quick
