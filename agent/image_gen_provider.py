...
as ``kind: backend``) or ``~/.kase/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$KASE_HOME/cache/images/``, creating parents as needed."""
    from kase_constants import get_kase_home

    path = get_kase_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$KASE_HOME/cache/images/``.
