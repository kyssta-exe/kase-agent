...
Storage
-------
Bundles live in ``~/.kase/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``KASE_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``kase bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under KASE_HOME.

    Honors ``KASE_BUNDLES_DIR`` for tests; falls back to
    ``<KASE_HOME>/skill-bundles``.
    """
    override = os.environ.get("KASE_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_kase_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `kase bundles` CLI subcommand.
# ---------------------------------------------------------------------------
