...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.kase/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Kase triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://kase.kyssta.com",
    "X-Title": "Kase",
...
_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://kase.kyssta.com",
    "X-Title": "Kase",
    "User-Agent": f"Kase/{_HERMES_VERSION}",
}
...
# in lockstep with kase_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from agent.portal_tags import kyssta_portal_tags as _kyssta_portal_tags


def _nous_extra_body() -> dict:
    """Return a fresh Nous Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``kase_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _kyssta_portal_tags()}
...
# ``_nous_extra_body()`` or import ``kyssta_portal_tags`` directly.
...
_KYSSTA_DEFAULT_BASE_URL = "https://inference-api.kyssta.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_kase_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Kase)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``kase_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.kase/auth.json for an active Nous provider.
    ...
    if pool_present:
        ...
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _KYSSTA_DEFAULT_BASE_URL),
...
def _kyssta_base_url() -> str:
    """Resolve the Nous inference base URL from env or default."""
    return os.getenv("KYSSTA_INFERENCE_BASE_URL", _KYSSTA_DEFAULT_BASE_URL)
    ...
    try:
        ...
        creds = resolve_kyssta_runtime_credentials(
            ...
            inference_auth_mode=(
                KYSSTA_INFERENCE_AUTH_MODE_LEGACY
                if force_refresh
                else KYSSTA_INFERENCE_AUTH_MODE_AUTO
            ),
        )
    except Exception as exc:
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``kase auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``kase_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from kase_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Kase auth store.
    ...
    try:
        from kase_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from kase_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from kase_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from kase_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from agent.kyssta_rate_guard import nous_rate_limit_remaining
    ...
    # auto-detects the caller's tier via check_kyssta_free_tier().  Fall back to
    # _NOUS_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _NOUS_MODEL
    try:
        from kase_cli.models import get_nous_recommended_aux_model
    ...
    if runtime is not None:
        ...
        base_url = str((nous or {}).get("inference_base_url") or _kyssta_base_url()).rstrip("/")
    return (
        OpenAI(
            api_key=api_key,
            base_url=base_url,
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `kase setup` or `kase model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_azure_foundry(
    *,
    ...
    :func:`kase_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from kase_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second kase
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``kase config aux reset``)."""
...
def _recoverable_pool_provider(resolved_provider: str, client: Any) -> Optional[str]:
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.kyssta.com"):
        return "nous"
    ...
    try:
        ...
        if normalized == "nous":
            ...
            creds = resolve_kyssta_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("HERMES_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("HERMES_NOUS_TIMEOUT_SECONDS", "15")),
                inference_auth_mode=KYSSTA_INFERENCE_AUTH_MODE_LEGACY,
            )
            if not str(creds.get("api_key", "") or "").strip():
                return False
            _evict_cached_clients(normalized)
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `kase model` but the
    #    old OPENAI_BASE_URL lingers in ~/.kase/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: kase model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.kase/.env",
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from kase_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: kase auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: kase model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: kase model -> xAI Grok OAuth — SuperGrok Subscription)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from kase_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: kase doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("kase_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from kase_cli.models import _should_use_copilot_responses_api
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(_kyssta_portal_tags())
    if merged_extra:
    ...
    *,
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `kase model`."
                )
    ...
    try:
        ...
        client_is_nous = (
            ...
            or base_url_host_matches(_base_info, "inference-api.kyssta.com")
        )
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        client_is_nous = (
            resolved_provider == "nous"
            or base_url_host_matches(_client_base, "inference-api.kyssta.com")
        )
        if _is_auth_error(first_err) and client_is_nous:
            refreshed_client, refreshed_model = _refresh_nous_auxiliary_client(
                cache_provider=resolved_provider or "nous",
