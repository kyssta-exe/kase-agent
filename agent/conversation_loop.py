...
from agent.kyssta_rate_guard import (
...
from kase_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Kase tool use: "
    ...
    return (
        ...
        f"context, but Kase needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Kase config, set `model.ollama_num_ctx: 65536` "
...
def run_conversation(
    ...
    # ``kase logs --session <id>`` can filter a single conversation.
    from kase_logging import set_session_context
    ...
    # are reused.  The system prompt is Kase's territory; plugins
    ...
    # all run inside Codex). Default Kase path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Kase internals.
        #
        # Kase invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Kase tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if agent.provider == "nous":
                    try:
                        from agent.kyssta_rate_guard import clear_nous_rate_limit
                        clear_nous_rate_limit()
                    except Exception:
                        pass
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: kase login --provider nous")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.kyssta.com")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `kase doctor` for credential-chain diagnostics, or")
                    ...
                    from kase_constants import display_kase_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Kase-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: kase config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: kase config set ANTHROPIC_API_KEY \"\"")
                ...
                # providers (DeepSeek, Kimi, MiMo, Kase).  A 429 can
                ...
                # which Kase' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Kase' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`kase",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if _provider in {"openai-codex", "xai-oauth"} and status_code == 401:
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `kase auth` to re-authenticate.", force=True)
                            else:
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok Subscription) from `kase model`.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: kase setup", force=True)
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("KASE_KANBAN_TASK")
        if _kanban_task:
            try:
