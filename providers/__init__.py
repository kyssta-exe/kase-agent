...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with kase)
2. User plugins: ``$KASE_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$KASE_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$KASE_HOME/plugins/model-providers/`` if it exists."""
    try:
        from kase_constants import get_kase_home

        d = get_kase_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple KASE_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$KASE_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with kase.
    ...
    # 2. User plugins — under $KASE_HOME/plugins/model-providers/<name>/.
