# Kase Security Policy

This document describes Kase's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/Kyssta/kase/security/advisories/new)
or **security@kyssta.com**. Do not open public issues for
security vulnerabilities. **Kase does not operate a bug
bounty program.**
...
- Environment details (`kase version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Kase access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Kase's documentation
...
Kase supports two OS-level isolation postures. They address
...
Kase supports this in two ways:

- **Kase's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Kase's in-process heuristics
...
Kase filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Kase
itself. Bugs in Kase's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Kase does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Kase's own documentation, or reasonable
  operator expectations would predict — including cases where
  Kase has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  kase-home security, and similar. Reports against those
...
- Kase includes supply-chain guards for MCP server
